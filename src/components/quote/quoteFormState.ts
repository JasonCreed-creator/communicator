...
export function applyFieldRules(prev: QuoteFormState, key: string, value: unknown): QuoteFormState {
  ...
  if (key === 'target') {
    ...
    // 100명 이상 + LED면 4K 스케일러가 시스템에 자동 포함 — 옵션 중복 청구 방지.
    if (tv >= 100 && next.displayType === 'led') {
      const { scaler4k: _omit, ...rest } = next.options || {}
      next.options = rest
    }
  }
  if (key === 'guarantee') {
  ...
  if (key === 'displayType' && value === 'led') {
    if (prev.target < 100) next.options = { ...next.options, scaler4k: true }
    else {
      const { scaler4k: _omit, ...rest } = next.options || {}
      next.options = rest
    }
  ...
  if (key === 'displayType' && value === 'projector') {
    // 화면중계는 LED 전용 — 프로젝터 전환 시 해제 (엔진도 미과금 이중 방어)
    const { screenRelay: _omitRelay, ...rest } = next.options || {}
    next.options = rest
  }
  return next
}
...
export const OPT_CATALOG: OptCatalogItem[] = [
  ...
  { id: 'scaler4k', icon: '🖥️', group: null, pureOnly: false,
    ko: { label: '4K 스케일러/KVM', price: '250만원', detail: '4K급 LED 운용 시 필수 — 외부 스케일러 + KVM' },
    en: { label: '4K Scaler/KVM', price: 'KRW 2,500,000', detail: 'Required for 4K LED — external scaler + KVM' } },
  { id: 'screenRelay', icon: '📡', group: null, pureOnly: false, ledOnly: true,
    ko: { label: '화면중계', price: '250만원', detail: '카메라 2대 + 스위칭 + 중계 오퍼레이터 (1일) — LED 전용' },
    en: { label: 'Live Screen Relay', price: 'KRW 2,500,000', detail: '2 cameras + switching + operator (1 day) — LED only' } },
  { id: 'fullRecording', icon: '🎥', group: null, pureOnly: false,
    ko: { label: '전체 녹화·편집', price: '350만원', detail: '전 세션 풀 녹화 + 세션별 편집본 (스케치 영상과 별개)' },
...
export function toggleOption(
  ...
  if (opt?.ledOnly && prev.displayType !== 'led' && !prev.options[id]) {
    return { state: prev, notice: t.relayLockToast }
  }
  if (id === 'scaler4k' && prev.displayType === 'led' && prev.options.scaler4k) {
    return { state: prev, notice: t.ledLockToast }
  }
  if (id === 'scaler4k' && prev.target >= 100) {
  ...
  next[id] = !next[id]
  return { state: { ...prev, options: next }, notice: null }
}
