 ...
 */
function ClientLinkButton({ target, label }: { target: ClientLinkTarget | null; label: string }) {
  const [state, setState] = useState<'idle' | 'copied' | 'failed'>('idle')
  if (!target) return null
  if (target.kind === 'settings') {
    return (
      <Link to={CLIENT_LINK_SETTINGS_PATH} onClick={stop} className="btn btn-ghost btn-sm shrink-0" title="행사 설정 ② 담당자 — 발주처 링크 복사·발급">
        {label}
      </Link>
    )
  }
  ...
  return (
    <span className="flex shrink-0 flex-col items-end gap-1">
      <button type="button" onClick={copy} className="btn btn-ghost btn-sm" title="발주처 컨펌 화면 주소를 복사합니다 — 메신저로 다시 전하세요">
