...
describe('DoD-16 온보딩 완료 = onboarded_at 파생 (v1.4.1)', () => {
  it('(a) onboarded_at이 null이면 내부 라우트가 위저드로 리다이렉트된다', async () => {
    const p = mockProvider()
    p.resetOnboarding()
    expect((await p.getOnboardingStatus(PROJECT_ID)).onboarded_at).toBeNull()
  ...
  it('(b) 완료 처리 시 onboarded_at 타임스탬프가 기록되고 completed가 파생된다', async () => {
    ...
    const status = await p.getOnboardingStatus(PROJECT_ID)
  ...
  it('(c) 이미 완료된 프로젝트의 재완료 시도는 409 CONFLICT', async () => {
    const p = mockProvider()
    const err = await p.completeOnboarding(PROJECT_ID).then(
      () => null,
      (e: unknown) => e,
    )
