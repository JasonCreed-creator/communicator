...
// DoD-8: S9 운영계획서 — mock 데이터로 섹션(v1.3부터 ③큐시트 추가, 프로그램 다음 배치 / v2.5부터
// ⑦비상 대응 신설, Phase 3.16d — 이 파일이 단언하는 섹션 7종은 그대로이고 ⑦비상 대응 회귀 커버는
// plan-v25.test.tsx가 별도로 맡는다) 전부 렌더 + 섹션별 진행률 + 제작물 리스트 표가 가이드
// 스펙에서 자동 생성 + 인라인 편집 권한 게이팅·왕복 (CLAUDE.md v1.4 §7 DoD-8). 픽스처 초기화
// 단위 = 이 파일 — 시나리오 순서대로 실행된다.
//
