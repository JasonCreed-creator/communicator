...
// DoD-7: 지시(instruction) 발행 흐름 — 지시 발행 → 담당자 화면 '지시됨' 뱃지 →
...
// PM(usr-pm)이 발행한 지시가 이 변수로 다음 테스트들에 전달된다.
let issuedItemId = ''

describe('DoD-7 지시 발행 흐름', () => {
  it('(a) pm이 보드에서 지시 발행 폼으로 발행하면 보드에 지시됨 뱃지로 나타난다', async () => {
    renderRoute('/board/design')

    const heading = await screen.findByRole('heading', { name: '지시 발행' })
    ...
    await userEvent.type(within(form).getByLabelText('지시 내용'), '입구 사이니지 시안 요청.')
    ...
    await userEvent.click(within(form).getByRole('button', { name: '지시 발행' }))
  ...
  it('(b) design 사용자 홈에 받은 지시로 해당 항목이 노출된다', async () => {
    mockProvider().switchUser('usr-design')
    renderRoute('/')

    const widget = (await screen.findByRole('heading', { name: '받은 지시' })).closest('div')!.parentElement!
    expect(within(widget).getByText('입구 사이니지')).toBeTruthy()
    expect(within(widget).getByText('사이니지')).toBeTruthy()
  })

  it('(c) S3 상세에 지시 카드(브리프+스펙 칩)가 렌더된다', async () => {
    renderRoute(`/items/${issuedItemId}`)

    expect(await screen.findByRole('heading', { name: '지시 카드' })).toBeTruthy()
  ...
  it('(e) pm이 아닌 사용자에게는 지시 발행 UI가 노출되지 않는다', async () => {
    // usr-design으로 전환된 상태가 (b)부터 유지된다.
    renderRoute('/board/design')

    await screen.findByText('새 항목 생성')
    expect(screen.queryByText('지시 발행')).toBeNull()
  })
})
