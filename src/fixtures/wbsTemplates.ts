...
function t(
  ...
  return { phase_no, phase_name: PHASE_NAMES[phase_no], code, title, offset_start, offset_end, role, origin_role }
}

/** 모객형 37태스크 — 부록 §15 표 그대로 */
export const RECRUITING_WBS_TEMPLATE: readonly WbsTemplateTask[] = [
  t('1.1', '계약 검토 및 최종 날인', -42, -40, 'pm', 'RS'),
  t('1.2', '킥오프 (영업+운영+협력)', -38, -35, 'pm', '공동'),
  t('1.3', '클라이언트 실행 계획 미팅', -35, -33, 'pm', 'RS'),
  t('1.4', '현장답사 (주차·이동경로·교통)', -33, -28, 'ops', 'MC-PM'),
  t('2.1', '행사 기초 자료 요청 (Key Visual 등)', -33, -30, 'pm', 'RS'),
  t('2.2', '기초 자료 수령 리마인더', -28, -26, 'pm', 'RS'),
  t('2.3', '기초 자료 수령', -26, -23, 'pm', 'RS'),
  t('2.4', '자료 수령 후 협력사 전달', -23, -22, 'pm', 'RS'),
  t('2.5', '랜딩페이지 디자인·개발 1차', -22, -18, 'design', 'MC-PM'),
  t('2.6', '랜딩페이지 1차 수정·내부 검토', -18, -17, 'design', 'MC-PM'),
  t('2.7', '랜딩페이지 최종 컨펌·URL 오픈', -17, -16, 'pm', 'MC-PM'),
  t('2.8', '제작물 (배너·렌탈장비·기념품·현수막)', -13, -5, 'design', 'MC-PM'),
  t('3.1', '리드젠 서베이 문항 설계·시스템 구축', -25, -22, 'reg', 'RO'),
  t('3.2', '서베이 링크 전달·검수', -22, -20, 'reg', 'RO'),
  t('3.3', '서베이+랜딩 통합 테스트', -20, -18, 'reg', 'RO'),
  t('3.4', '대시보드 최초 세팅·전달', -18, -17, 'reg', 'RO'),
  t('3.5', '실시간 리드 관리 시트 세팅', -17, -16, 'reg', 'RO'),
  t('4.1', '리드 수집 시작', -16, -5, 'reg', 'RO'),
  t('4.2', '타겟 일치/불일치 실시간 협의', -16, -3, 'pm', 'RS'),
  t('4.3', '불일치 리드 적격 확정 후 전달', -16, -3, 'reg', 'RO'),
  t('4.4', '1차 참석 확인 (전화·알림톡·메일)', -8, -5, 'reg', 'MC-AT'),
  t('4.5', '2차 참석 확정·노쇼 방지', -3, -1, 'reg', 'MC-AT'),
  t('4.6', '데일리 현황 공유 (내부)', -5, -1, 'reg', 'MC-AT'),
  t('4.7', '데일리 현황 공유 (고객)', -5, -1, 'reg', 'RO'),
  t('5.1', '행사 물류 배송·현장 셋팅', -3, -2, 'ops', 'MC-PM'),
  t('5.2', '현장 운영 자료 수집·테스트', -2, -1, 'ops', 'MC-PM'),
  t('5.3', '전체 리허설·테크니컬 체크', -1, -1, 'ops', 'MC-PM'),
  t('5.4', '현장 등록 데스크·출입 관리', 0, 0, 'reg', 'MC-AT'),
  t('5.5', 'VIP 케어·연사 관리·프로그램 운영', 0, 0, 'ops', 'MC-PM'),
  t('6.1', '최종 쇼업 리드 리스트 정리·전달', 1, 2, 'reg', 'MC-AT'),
  t('6.2', '쇼업 리드 raw data 납품', 2, 3, 'reg', 'RO'),
  t('6.3', '전체 행사 결과 보고서 작성·전달', 3, 10, 'ops', 'MC-PM'),
  t('6.4', '운영 견적서 확정·전달', 3, 5, 'pm', 'MC-PM'),
  t('6.5', '세금계산서 발행·고객 입금 확인', 5, 15, 'pm', 'RS'),
  t('6.6', '운영비 협력사 정산 (이윤 포함)', 15, 20, 'pm', 'RS'),
  t('6.7', '고객사 회계 처리 마감', 20, 30, 'pm', 'RS'),
  t('6.8', '프로젝트 회고·개선 사항 정리', 10, 20, 'pm', '공동'),
]
...
const GENERAL_EXTRA_TASKS: readonly WbsTemplateTask[] = [
  t('3G.1', '참석 대상 명단 확정', -20, -15, 'reg', null),
  t('3G.2', '초청장 발송·회신 관리', -14, -5, 'reg', null),
]
