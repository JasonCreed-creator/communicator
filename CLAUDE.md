...
> v2.11 변경 핵심(2026-09-25): **Phase 3.23 UX 개편 적용** — 캔버스 "커뮤니케이터 UX 개편" v4(3턴 렌더 검수) 승인 → 사용자 결정: 대비 토큰 2건 어둡게(`--ink-cap` #756f64 · `--negative` #c42f2f) · 주황 채운 버튼 바탕 = `--accent-deep` · 디자인 보드 갤러리 보기 · **바로 구현**(범위 [A] — PR 8개 연속, 검증 통과 시 머지). PR-1 = 기반(토큰·`keep-all` 줄바꿈·기한 표기 `오늘`/`n일 지남`·화면 코드 제거). 정본 = 디자인지시서 **v1.4 §7-2**. DataProvider 131 불변.
