...
> v2.11 변경 핵심(2026-09-25): **Phase 3.23 UX 개편 적용** — 캔버스 "커뮤니케이터 UX 개편" v4(3턴 렌더 검수) 승인 → 사용자 결정: 대비 토큰 2건 어둡게(`--ink-cap` #756f64 · `--negative` #c42f2f) · 주황 채운 버튼 바탕 = `--accent-deep` · 디자인 보드 갤러리 보기 · **바로 구현**(범위 [A] — PR 8개 연속, 검증 통과 시 머지). PR-1 = 기반(토큰·`keep-all` 줄바꿈·기한 표기 `오늘`/`n일 지남`·화면 코드 제거) · PR-2 = 홈 '오늘 할 일' · PR-3 = 디자인 보드 '다음 행동' 표·차례 칩·갤러리. 정본 = 디자인지시서 **v1.4 §7-2**. DataProvider 131 불변.
...
- **Phase 3.23 — UX 개편 적용 (캔버스 v4, 사용자 지시 2026-09-25 "페이지 UI UX를 클로드디자인을 통해서 업그레이드" → 3턴 시안 → "바로 구현 시작")**
  ...
  - PR 순서(사용자 승인 "제안 순서대로"): **PR-1 기반** → PR-2 홈 '오늘 할 일' → PR-3 디자인 보드(표 '다음 행동' + 갤러리) → PR-4 항목 상세·큐시트 → PR-5 행사 목록 → PR-6 견적 목록·옵션 → PR-7 정산보드 → PR-8 행사 설정·온보딩. 같은 브랜치에서 차례로 — 검증(tsc·vitest·build·`deploy:check`·demo 4단·스모크 ③) 통과 시 머지 후 다음
...
- **Phase 4.5 — 항목 고치기·지우기** (사용자 지시 2026-09-25 "이미 등록한 항목을 수정/삭제하는 기능도 만들어야 함 — 지금은 항목추가로 쌓이기만 해", 설계서 v2.10 §8)
  ...
  - 화면 = S3 **항목 관리 카드**(`src/components/item/ItemManageCard.tsx` · `DeleteItemDialog.tsx`): 메타 열 맨 아래(정형 문서는 본문 맨 아래) — 헤더 주 액션 2개(3.17b) 불변. 권한 없는 역할에는 카드 없음 · 종료 행사는 비활성 + 이유 · 지운 뒤 결과 → '{영역} 보드로'(공통 = 홈)
