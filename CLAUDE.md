...
- **Phase 4.7 — 협력사 견적서 불러오기(엑셀)** (사용자 지시 2026-09-25 "협력사 견적 불러오기 등 아직 구현 안 된 내용들 싹 진행" → 묶음 3, 설계서 v2.11 §19.5a·§8.5)
  ...
  - 화면 = S-10 `src/components/settlement/VendorQuoteImport.tsx`(옛 "Phase 4.7에서 열립니다" 버튼 자리 — 게이트 뒤에 숨기지 않음): 이력·확인 대기 건수 → 대화상자(파일·협력사 → 확인 큐: 부가세 · 행별 포함·제목·버킷 · '확인 필요' · 비어 있는 버킷 한 번에 채우기 · 공급가 대조) → 확정 · 나중에 · 버리기. mock의 확인 큐는 메모리(픽스처 상태 불변)
