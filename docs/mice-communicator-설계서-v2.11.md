...
| Method·액션 | 권한 | 동작 |
|---|---|---|
| GET /api/notify | 공개 | `{slack, cron}` — 공용 채널·크론 비밀이 설정됐는가(값은 내보내지 않는다) |
...
| POST `test` {project_id} | 그 행사 pm | 행사 채널(없으면 공용)로 시험 한 줄. 채널 없음 409(행사 설정 ③ 안내) · Slack 거부 502 |
