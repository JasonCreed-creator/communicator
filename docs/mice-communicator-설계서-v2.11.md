...
| S3 | 항목 상세 | 지시 카드(브리프·스펙 칩, v1.2)·버전 이력(최신 뱃지)·미리보기·코멘트 스레드·컨펌 이력·Drive 폴더 링크 — **큐시트 항목은 파일 대신 정형 에디터 렌더(v1.3: 행 편집·드래그 정렬·대본 전문)** · **(v2.9.3) 업로드가 막힌 상태(컨펌대기·승인·확정·파트너 첫 제출 전)는 업로드 폼 대신 이유·다음 할 일(헤더 '새 버전 업로드' 비활성) · 버전마다 저장 위치(실서버: Drive / 임시·저장 안 됨) · 실서버 Drive 미연결 경고 상자 · 발주처 링크 0개면 발송 폼에 경고** · **(v2.10) 항목 관리 카드 — 메타 열 맨 아래(정형 문서는 본문 맨 아래), 헤더 주 액션 2개 불변: 고치기(pm·영역 담당 — 제목·카테고리·마감 / pm은 담당·제작 가이드, 정형 문서는 카테고리 잠금) · 지우기(pm — 항목 이름 입력 확인 → 결과(Drive 보관 여부) → 보드로). 권한 없는 역할에는 카드가 없고, 종료 행사는 비활성 + 이유** | 버전 업로드, 전이, (pm) 컨펌 발송(큐시트=스냅숏 자동), 항목 고치기·지우기 |
