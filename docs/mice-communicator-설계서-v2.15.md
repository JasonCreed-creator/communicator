...
| 항목 | 내용 |
|---|---|
| 문서 상태 | **v2.15 — Slack 메시지로 행사 만들기(인테이크) + 견적서 첨부 + 행사 코드 자동(Phase 6.2 · §10 S0·S6① · §8.7 · §9)**(2026-09-26, 사용자 지시 "행사 기본정보를 슬랙 메시지를 통해 불러와 기록하는 시스템 구축(행사 만들기 시) · 여기 견적서도 첨부할 수 있게(파일 혹은 링크) · 행사 코드는 행사명에 따라서 자동기입" + "나 이제 잘꺼니까 잘 해놔"(진행 위임 — 범위 게이트·브리프 생략, 판단은 PROGRESS 결정 로그)): ① 온보딩 ① 머리에 **'Slack 메시지에서 불러오기'** — 메시지 링크(봇 `conversations.history/replies` — 권한 `channels:history`·`groups:history` 추가 필요) 또는 글 붙여 넣기 → **라벨 규칙**(행사명: · 일시: · 장소: · 인원: …)이 먼저, **AI(Claude, feature `project_intake`)**가 빈 칸만(키·한도가 있을 때 — 실패해도 규칙 결과로 답한다) → 행사명·일시·장소·인원·주최·주제·대상·유형·메모를 폼에 **주황으로 채운다**(사람이 확인·수정 뒤 저장) · 기록 = `projects.intake`(링크·시각·보낸 사람 표시 이름·방식·채운 칸 — **원문 저장 없음**) · 선택하면 그 글의 스레드를 행사 Slack 스레드로(설정 ③과 같은 칸) ② **견적서 첨부**(`projects.quote_attachment`) — 파일은 Drive 행사 폴더 `02_견적·정산/견적서`(서버 `PUT /api/drive?action=project-file` · 4MB) · 링크는 https 주소만 · Slack 글의 첨부 파일은 서버가 봇으로 내려받아(`files:read`) 같은 폴더에 · 행사 하나에 하나 · 내부 화면(온보딩 ①·설정 ①)에만 ③ **행사 코드 자동** — 행사명 단어 이니셜(영문 대문자 · 한글 초성 로마자 · ㅇ은 모음) 최대 4자 + 연도 두 자리(행사일 → 이름 속 20xx → 올해) · 겹치면 -2 · 새 행사(자리표시 `EVT-…`)에서만 자동, 사람이 고치면 꺼짐('행사명에서 다시 만들기'로 복귀) · 견적 핸드오프의 코드 제안도 같은 규칙. mock(데모) = 붙여 넣은 글을 라벨 규칙으로 그 자리에서 읽는다(링크·AI·파일 저장은 실서버). DataProvider **v15.6**(`Project`·`ProjectPatch`의 `intake`·`quote_attachment` 필드만 — 131 불변). 직전 **v2.14 — AI(Claude)로 협력사 견적서 PDF·사진 읽기(Phase 4.8 · §19.5b · §8.6 · §10 S-10)**(2026-09-26, 사용자 질문 "시스템에 클로드 API 붙여볼 수 있어?" → 사용자 선택 "협력사 견적서 PDF·사진만 먼저"(AI가 꼭 필요한 한 곳만) · 모델 = **Claude Sonnet 5** · 키 = **개인 Anthropic 계정으로 먼저 시험** · 사용 범위 = **로그인한 내부 사용자 + 1인 하루 30회**): 정산보드 '협력사 견적서 불러오기'가 엑셀에 더해 **PDF·사진(JPG·PNG·WEBP)**을 받는다 — 서버 함수 `api/ai`가 Claude로 표를 옮겨 적고(구조화 출력 · 개인정보 칸 없음 · 합계를 계산해 채우지 않음) 결과는 **엑셀과 같은 확인 큐**(제안만 저장 → 담당자 확인 → 확정 RPC가 저장된 제안의 금액으로 발주 항목)로 간다 · 검산은 우리 코드(어긋나면 경고, 막지 않음) · 확인 큐 머리에 'AI가 읽음' 배지 + 원본 대조 안내 · 키 = 서버 env `ANTHROPIC_API_KEY`만(VITE_ 판 금지) · 권한·한도 = SQL `ai_usage_claim`(그 행사 pm · 종료 안 된 행사 · KST 하루 한도 — Claude 쪽 오류는 세지 않음) · 사용 기록 `ai_usage`(파일 이름·금액 없음) · 사진은 앱이 긴 변 2000px로 줄여 보낸다(3MB 상한). DataProvider **v15.5**(`VendorQuoteParsed.format`에 'ai' · `reader` 필드만 — 131 불변). 운영계획서 점검·멘트 초안·견적 분배/WBS 제안 등 다른 AI 자리는 **아이디어로만 기록**(사용자 — "꼭 필요한 게 아니면 추가하지 않아도 됨"). 직전 **v2.13.2 — 16:9 장표형 운영계획서(Phase 3.24 PR-C · §23.7 · §10 S9-D)**(2026-09-26, 사용자 선택 "운영계획서 출력 = 16:9 장표형 인쇄"(2026-09-26 범위 게이트) + "다음스텝 알아서 진행"): 운영계획서(S9)와 같은 조립 데이터를 **16:9 장표**(1280×720 CSS px = 338.67mm×190.5mm, 파워포인트 16:9 한 쪽)로 싣는 전체 화면 `/plan/deck` — 표지 · 목차 · 운영 요약(핵심 숫자 6칸) · 01 행사 개요 · 02 제작물 · 03 공간·설치 · 04 프로그램 · 05 참가자 · 06 조직·인력 · 07 안전·비상. **새 입력 칸 없음** — 앱에 있는 표·숫자·목록만(운영가이드 현장 운영 표 섹션이 04~07장의 중심). 인쇄 = 이름 붙은 쪽 `@page deck`(A4 운영계획서 인쇄 규칙과 분리) · 표·글은 칸 폭·글자 수로 줄 수를 어림해 넘치기 전에 다음 장으로(고르게 나눔 · 구분 줄 '(이어서)'). DataProvider **v15.4**(`PlanData.guide` 필드만 — 131 불변). 목표·성공 기준 · 확인 요청 · 계약 범위 대응표(사양만)는 입력 칸이 필요해 다음 단계. 직전 **v2.13.1 — 시나리오 멘트 원고형 · 운영 보드 문서 요약 · 큐시트 칸 이름 · 문서 머리 통일(Phase 3.24 PR-B · §23.6 · §23.1 · §10.2)**(2026-09-26, 같은 캔버스 시안 ①②④ — 사용자 선택 "멘트 원고형"): 시나리오 = MC가 그대로 읽는 원고(세션 머리 시각 크게 → 블록마다 시각·구분 → 괄호 지시문 → 멘트 크게) · 비상 예비 멘트(`scenario_blocks.kind`에 `emergency` 추가 — 세션·시각 없음) · 연사 자리(대괄호·○○○)는 '연사 확인 대기' · 운영 보드 = 카드마다 그 유형의 숫자 + 유형마다 다른 표 칸 · 큐시트 칸 = 시각 · 큐 · 구분 · MC·진행 · 조명 · 영상 · 음향(데이터 불변) · 시나리오·운영가이드 머리 = 큐시트 PR-4b와 같은 모양(빌더 안 두 번째 머리·발송 버튼 없음). DataProvider **v15.3**(타입 값 1개 — 131 불변). 직전 **v2.13 — 운영가이드 현장 운영 섹션(Phase 3.24 PR-A · §23.5 · §23.1 · §10.2)**(2026-09-26, 사용자 지적 "운영보드 네 항목의 세부내용 포맷이 다 똑같아" + 실물 운영계획서(44장) 제시 "이 정도 수준" → 문서 3종 실무화 — 캔버스 시안 5장 승인 · 범위 [B] · 운영가이드 섹션 = **현장 운영과 관련되는 전부**(사용자 선택)): 운영가이드 = **현장 운영 12섹션**(설치·철거 일정 · 현장 인력·콜타임 · 무전·지휘 체계 · 역할 분담 ●/○ · D-day 진행표 · 구간별 체크리스트 · 등록 운영(대기 시간 자동 계산) · VIP 의전 · 안전관리 · 존별 운영 · 비상 대응 · 연락망) — 앞의 9종은 **표로 채우는 섹션**(`guide_sections.data` jsonb, type = kind) · 표 섹션은 저장 때 content를 data에서 다시 만든다(운영계획서 ⑦·발송 스냅숏·인쇄 무변경) · 새 문서 시드는 행사 일시·장소·프로그램표·담당자 수로 뼈대를 채운다 · **옛 문서는 그대로 열리고 '뼈대 추가'로 빠진 섹션만 끼운다**(기존 섹션 순서·내용 불변) · 시안의 '연락망 전화번호는 인쇄에만'은 R-O6과 충돌해 **구현하지 않음**(이탈). DataProvider **v15.2**(필드만 — 131 불변). 시나리오 원고형·운영 보드 카드 요약·큐시트 칸 이름은 PR-B. 직전 **v2.12 — Slack 봇 전환 + 의뢰 확인 버튼(Phase 6.1 · §9 · §8.4 · §10 S2·S3·S6③·S-13)**(2026-09-26, 사용자 결정: 웹훅 → **봇**(같은 Slack 앱) · **DM을 쓰지 않는다 — 행사마다 채널의 스레드 하나에 봇이 답글로 남기고 할 일이 생긴 사람을 @멘션** · 의뢰(제작 요청·검토 요청)에는 **'확인했어요' 버튼 하나**(표식 — §5 무변경, 의뢰 한 건마다 기록) · 시안 올림은 멘션 없이 기록만): 보낼 곳 = 행사 스레드(`projects.slack_thread_url` — 행사 설정 ③에 스레드 링크) → 행사 웹훅 → 공용 웹훅 → 없음 · 멘션 대상 = `profiles.slack_user_id`(이메일로 찾아 적어 둠 · 담당자 화면에서 고침) · 확인 기록 `request_acks` · 버튼 응답 `api/slack-interact`(서명 검증) · 리마인드 + 항목 마감 D-1 · 24시간 확인 없는 의뢰. DataProvider **v15.1**(필드만 — 131 불변). 직전 **v2.11 — 협력사 견적서 불러오기(Phase 4.7 · §19.5 · §8.5 · §10 S-10)**(2026-09-25, 사용자 지시 "협력사 견적 불러오기 등 아직 구현 안 된 내용들 싹 진행" → 묶음 3): 엑셀(.xlsx)만(PDF·사진은 다음 단계) · 파서 = 견적서 임포트와 같은 것(§22.2 A·B·C형 — 브라우저 동기 리더) · **제안만 저장**(`settlement_imports.parsed` = 행·버킷 제안·부가세 판정) → 담당자 확인 → 확정 RPC가 **저장된 제안의 금액**으로 발주 항목(status='ordered') 생성 · 버킷은 원가 버킷만 제안 · 부가세는 확실할 때만 제안하고 모르면 묻는다(추측 금지) · 원본 = Drive 행사 폴더 `02_견적·정산/협력사 견적서`. DataProvider **v15 = 131메서드**(`importVendorQuote`·`listVendorQuoteImports`·`confirmVendorQuoteImport`·`discardVendorQuoteImport`). 실서식 보정은 실샘플 수령 후(**가정**). 직전 **v2.10.1 — Slack 알림(Phase 6 · §9 · §8.4 · §10 S1·S6③)**(2026-09-25, 사용자 지시 "아직 구현 안 된 내용들 싹 진행" → 묶음 2): 알림 원천 = 기존 activity_log(새 표 1개 = 선점 기록 `notification_log`) · 즉시 = 앱이 사건 직후 `api/notify`에 신호만(fire-and-forget) · 매일 = Vercel cron(KST 9시대, Hobby 하루 1회) D-1 리마인드·미등록 파일 묶음 · 보낼 곳 = 행사 채널(`projects.slack_webhook_url`, 행사 설정 ③) → 공용(env `SLACK_WEBHOOK_URL`) → 없음(no-op) · 홈 '리마인드' 버튼 = Slack(시간당 1회). DataProvider **v14.1**(ProjectPatch 필드 1개 — 127 불변). 이메일(Resend)은 Phase 6b 그대로. 직전 **v2.10 — 항목 고치기·지우기(Phase 4.5 · §6.1·§7.8·§8·§8.3·§10 S3)**(2026-09-25, 사용자 지시 "이미 등록한 항목을 수정/삭제하는 기능도 만들어야 함 — 지금은 항목추가로 쌓이기만 해" · 지우기 범위 = **모든 상태 + 항목 이름 입력 확인**(사용자 선택)). DataProvider **v14 = 127메서드**(`updateDeliverable`·`deleteDeliverable`) · SQL RPC 2개(`update_deliverable`·`delete_deliverable`) · Drive `archive-item`(지운 항목 폴더 → 행사 `99_archive`). 직전 **v2.9.3 — 실사용 결함 5건 정정(Phase 4.3.1 · §10 S3)**(2026-09-25 — 실서버 작동 테스트 중 발견, 표시만: 업로드 잠금 안내 · 거부 문구의 상태 이름 · 발송 전 발주처 링크 경고 · 버전 저장 위치 · Drive 미연결 경고 상자 — 전이표·DataProvider 125·DB 불변). 직전 **v2.9.2 — 시험용 입구(§12.1 · Phase 4.3) + 회사 도메인 하위 경로(§18a-2 · Phase 4.4)**(2026-09-25, 사용자 지시: 로그인은 Okta SSO로 맨 마지막 — 그 전까지 "아무나 들어갈 수 있는 게이트"(위험 고지 후 완전 개방 선택) · "도메인은 mkt.rememberapp.co.kr/leadgen/communicator로 교체"(연결 = 회사 CloudFront → Vercel)). DataProvider 125 불변. 직전 v2.9 확정 — **Drive 저장소 연결(Phase 5)**(2026-09-24, 사용자 지시: "작업물 저장소는 이 폴더 — 파트별 폴더링 · 파일 끌어놓기·폴더 업로드·Drive에 직접 올린 뒤 링크로도 작업", 범위 게이트 [A] · 사용자 결정 3건: 한 세션 완료 · **폴더 소유 계정 OAuth** · 코드 완성까지(실키는 다음)). ① 저장소 루트 = 사용자 지정 폴더(`MICE Communicator`, 내 드라이브) — 루트 `00_견적서`·행사 폴더(§7.1 표준 트리)·`99_archive`(§7.1b) ② 업로드 3경로 = 끌어놓기(파일·폴더) · 파일/폴더 선택 · **Drive 링크 등록**(§7.2b — 루트 안 파일만, 행사 폴더 안 = 참조 · 루트 안 다른 곳 = 복사 · 밖 = 403) ③ 서버 = Vercel Function `api/drive` 1개(§8.3) — **4MB 조각 중계 업로드**(Vercel 요청 4.5MB 한도 · 브라우저→Google 직접 PUT은 CORS 불가) · 서명 URL 스트림 프록시(§7.4) · **§7.5 2단계 확정 구현**(06 복사 성공 후 final) · 인박스 = **폴더 목록 비교**(§7.3 이탈 — Changes API는 연결 계정 Drive 전체 변경을 읽어야 함) ④ 인증 = OAuth(갱신 토큰은 앱 '연결하기' → Supabase Vault) · 서비스 계정은 **공유 드라이브 전용**(저장 용량 없음 — Google 공식 문서 2026-09-24 확인) ⑤ 행사 삭제 → 폴더를 루트 `99_archive`로 이동(§4-1c 이탈 2 해소) ⑥ **DataProvider v13.1**(`UploadVersionInput.drive_link`·`onProgress` 필드 추가만 — 125메서드 불변). 직전 v2.8 — **행사 하드 삭제 + 새 출발**(2026-09-07, 사용자 지시: 데모 행사·임시 담당자 전삭제 / 앞으로 만든 행사는 관리자 권한으로 삭제). §4-1c 신설 — 권한 축 = 전역 `app_role='admin'`(프로젝트 pm 아님) · 종료는 선행 조건 아님 · 견적은 `set null`로 분리 보존 · 주소록·협력사 무관 · 구현 = `security definer` RPC `public.delete_project`(projects에 delete RLS 정책은 두지 않음) · 연쇄 정정 3건(ProjectContext 저장 id 검증 · mock currentUser 멤버십 0 폴백 · 행사 0건 빈 상태 유도) · 이탈 2건(삭제 자체의 activity_log는 함께 사라짐 · Drive 트리는 남음). **DataProvider v13 재동결(125메서드)**. 직전 v2.7 — **Phase 4 Supabase 이식 정합**(2026-09-07, 사용자 결정 5건: [B] 3단 분할 · 폴링 유지(Realtime 보류) · profiles.title/phone/org + client_contacts.phone 추가 · sheet_status 매핑 추가 · **서버 함수 = Vercel Functions**(Edge Functions 대체)). ① 스키마 구현 정본 = `supabase/migrations`(17개, 멱등) + 통합 `setup.sql` + 생성 `seed.sql`(§18-3) — §4 요약과 다른 지점은 §4 머리말 표에 명시(profiles가 앱의 사람 정본·auth_user_id 분리, FK → profiles, landing_daily_metrics 열 TS 정합, app_config·sheet_source_rows 신설) ② 토큰 경로(`/c`·`/p`)·랜딩 리드·시트 반영·다단계 쓰기 = **security definer SQL RPC**(§6.2·§8 — anon은 표 권한 0·함수 execute만, 로컬 Postgres 85항목 증명) ③ 견적 서버 재계산·시트 읽기 = **Vercel Functions `api/`**(§8·§12 — 같은 레포·같은 배포, D-Day 추가 단계 = Vercel env) ④ 내부 로그인 = 이메일 매직링크 + `app_config.allowed_email_domains` 트리거 게이트(§12) ⑤ DataProvider **v12(124메서드) 그대로** — 새 메서드 0. 직전 v2.6 — UI/UX 고도화(패턴 정본) + 등록 구글 시트 연동 + 행사 유형 4분류(§25 증분). §25 증분(Phase 3.18, 2026-08-29): `projects.format`(conference/dms/exhibition)·`psa_enabled` 신설 — **format 권한은 3가지로 한정**(온보딩 시드·견적 모델·복합 게이트 구성요소), 상시 모듈 게이트는 기존 축 유지. 판매 플래너(dms·exhibition 판매형 도구) 신설, conference 견적 경로 **무접촉**. **DataProvider v11 재동결**(v10은 3.17c에서 소진). 초청제 모드·PSA 모듈은 미착수(§25.6·§25.5). **시각안 없이 진행** — 사용자 지시로 생략, 대체 게이트는 스크린샷 검수. 앞선 v2.6 — (2026-08-28 핸드오프 채택, 2026-08-29 챗 실측 검수 반영). ① 공통 패턴 정본 = 디자인지시서 §7-1(배지 의미 4단계+중립·표 정본·빈 상태 5종·시각화 어휘·인쇄·외부 지면) ② 등록 시트 연동 §24 — 시트가 정본·앱은 읽기만, 자동 감지는 하되 **반영은 항상 사람 확인 후**, 필드 소유 분리, 하드 삭제 금지, 동시 접속 낙관적 잠금(§24.3 R-S1~R-S4) ③ **체크인 배치 = B안(사이드바 S-12 별도 화면)** — 3.17 구현은 A안이었고 이는 Code 판단, 3.17.1에서 B안으로 복원 ④ **DataProvider v10 재동결(10메서드 추가 · 120메서드)**, importVendorQuote는 **v13 예약**(v11은 §25 증분, v12는 §4-2b 담당자 마스터에서 소진 — 2026-08-29). 직전 v2.5 확정 — **운영보드 재구성: 문서 유형 우선 + 시나리오·운영가이드 빌더** (2026-08-28, 시각안 3화면·구조 결정 5가지 전부 승인 — 계기: 사용자 데모 실측 피드백 "운영보드에 큐시트·시나리오·운영가이드가 들어가야 하고 항목별 전용 빌더가 필요"). ① 운영보드 1면 = 유형 카드 4종(큐시트/시나리오/운영가이드/기타 제작물) — "카테고리가 빌더를 결정한다" 원칙의 보드 레벨 확장, 유형 선택 시 빌더 인라인 ② 시나리오 빌더(프로그램표 뼈대 자동·진행 블록·큐시트로 내보내기)·운영가이드 빌더(존/역할/비상/연락망 4섹션·원본 연동 stale 확인) 신설 — 정형 테이블 §4-27, 컨펌·스냅숏은 큐시트 규약 재사용 ③ S9 ⑦비상 대응 섹션 신설·②시나리오 펼침·③존운영 확장(§10.2) ④ **DataProvider v9 재동결(8메서드 · 110메서드)**, importVendorQuote는 v10 예약 순연. 구현 = Phase 3.16(mock·3.15.1 머지 후). 직전 v2.4.1 확정 — **패치: 3.15 머지본 챗 감수(2026-08-27, 조건부 보완) 반영** — ① §15.3b 주최형 R&R 4카드·§15.3c 규약 카드 3종 정의(감수 M4 — 설계 공백 보완, 가정) ② projects에 파트너 안내 필드 2종(partner_guide_url·partner_contact_email — DataProvider v8.1, 필드 추가만) ③ 데모 아티팩트 charset 선두 보장 규약(§13b — 감수 M1, 미선언 서빙 백지 실증) ④ 폴리시 P1~P6은 CLAUDE.md 무개정·지시문 3.15.1로 수행. 직전 v2.4 확정 — **주최형(파트너) 확장 + 견적서 임포트** (2026-08-27, 시각안 4화면·구조 결정 7가지 전부 승인). ① 프로젝트 성격 축 `kind`(대행형/주최형) 신설 — 주최형은 파트너 N곳이 무로그인 링크 `/p/{token}`으로 제출하고 우리가 검토(기존 상태머신 방향 반전 재사용), 파트너 간 완전 격리·계약액 비노출(§21) ② 직접 설계한 견적서 xlsx 업로드 → 자동 인식 → 확인 큐 → 요소 분배 4종(§22, 실서식 3형 계약). DataProvider **v8 재동결(16메서드 · 102메서드)**, WBS 3번째 템플릿 "주최형" 12건(§15.3). 구현은 Phase 3.15(mock 우선·서버 0). 직전 v2.3 확정 — **서버 스프린트: 키 최후 주입 실행 개정(기능 무변경)** (2026-08-27, 범위 게이트 승인). Phase 4(Supabase)·5(Drive)·6(알림)을 운영 자격증명 없이 D-Day(8/31 월, 첫 출근일) 전에 전부 구현·검증하고, D-Day에는 §20 런북의 자격증명 주입(서버 3키·Slack 웹훅·Drive OAuth)만으로 실전 투입 가능 상태를 만든다. 사전 검증은 개발용 무료 Supabase 프로젝트, API 키는 신형 체계(sb_publishable/sb_secret — §12, 웹검증 2026-08-27) 채택. 직전 v2.2 확정 — **정산보드(S-10) 신설** (2026-08-23, 내부정산 실물 13건 분석 기반·시각안 승인). 마진 식(항목 마크업 + PCO 기획료 + RSVP 운영비, 리드젠 제외)을 실물 2건에서 원 단위 검산하고 §19에 정본화한다. §4-23 테이블 4종 + §4-24 계약 R-S1~R-S10, DataProvider **v7 재동결(11메서드 · 86메서드)**. 직전 v2.1 — **랜딩보드(S-3) 정본화 + 랜딩 스코프 계약 + 가격 상수 v1.1 정의** (2026-08-23). 코드가 선행한 Phase 3.13 랜딩보드를 §4-19~§4-22·§8·§10에 정본으로 흡수하고, `listLandingPages`·`createLandingPage`가 현재 행사가 아닌 사용자 첫 멤버십으로 스코프되던 결함을 계약으로 못박는다(§4-21). LED 오퍼레이팅·중계 단가 분리(§17.4)와 골든 데이터셋 출처 규약(§17.3)을 확정. 직전 v2.0: **견적 Configurator(jsx-easy-shift) 단일 플랫폼 통합** (2026-08-22, 시각안 3화면 승인 · 읽기 분석 보고 기반): 견적 모듈 S-2 · 견적→행사 핸드오프 · 새 Supabase 프로젝트 · 인프라 전환 절차. 직전 v1.5: **다중 행사(프로젝트 셀렉터·행사 목록) + 행사 설정 메뉴(개요·담당자 입력) 확장** (2026-08-22, 시각안 3화면 승인). 직전 v1.4.1: v1.4(유형별 WBS·R&R, 2026-08-22 시각안 승인)에 **Phase 3.6·3.7 구현 해석 정본화** 패치: projects.onboarded_at 확정(사용자 승인 2026-08-22) · 임박/지연 배타 산식 · 일반형 28건 파생 규칙 · 재전개 보존 규칙 · 큐시트 스냅숏 mock 규약 (Code PROGRESS 열린 질문 ①~⑤ 종결) |
...
└──────────────┬─────────────────────────┘   └──────────────┬─────────────────┘
        ...
        └ Edge Functions (Deno) — API 계약 §8
               ...
               ├──> Anthropic Claude API (v2.14 — 협력사 견적서 PDF·사진 읽기, 서버 함수 api/ai만)
...
- 동결 이력: v1(35메서드, Phase 1) → v2(41, v1.2 승인) → v3(53, v1.3·v1.4 승인) → v3.1(v1.4.1 — 필드 추가만) → v4(v1.5 승인 — 다중 행사 5메서드) → **v5(v2.0 승인 — 견적 모듈: `listQuotes`·`getQuote`·`createQuote`·`saveQuoteVersion`·`finalizeQuote`·`createProjectFromQuote`·`exportQuoteXlsx`·`listComplianceCards`·`updateComplianceCard` **9메서드** 추가, `Project`에 모객 필드(guarantee_pax·targeting·kpi_show_rate·quote_id), `WbsTask.target` 추가. 기존 시그니처 불변)** → **v6(v2.1 승인 — 랜딩보드 8메서드: `listLandingPages`·`getLandingPage`·`createLandingPage`·`updateLandingPage`·`publishLandingPage`·`deleteLandingPage`·`listLandingMetrics`·`submitLandingLead`)** → **v6.1(v2.1 정정 — 스코프 결함 해소: `listLandingPages(projectId)`·`createLandingPage(projectId, input)`로 시그니처 변경. 나머지 6메서드는 landingId로 프로젝트를 역참조하므로 불변)** → **v7(v2.2 승인 — 정산보드 11메서드: `getSettlementBoard`·`createSettlementBoard`·`rebaseSettlementBoard`·`createSettlementBucket`·`updateSettlementBucket`·`deleteSettlementBucket`·`createSettlementItem`·`updateSettlementItem`·`deleteSettlementItem`·`listVendors`·`upsertVendor` = **86메서드**. 기존 시그니처 불변. 업로드 파싱(`importVendorQuote`)은 서버 의존이라 v8 예약 슬롯으로 남긴다 — §19.5)**. → v8(102)·v9(110)·v10(120)·v11(120, 3.18 — 메서드 0)·**v12(124, §4-2b 담당자 마스터)**·**v13(125, §4-1c 행사 하드 삭제 — `deleteProject`. 사용자 승인 2026-09-07)**·**v13.1(125 불변 — §7.2b `UploadVersionInput.drive_link`·`onProgress` 필드 추가만. 사용자 승인 2026-09-24, Phase 5 범위 게이트 승인이 동결 해제 승인을 겸함)**·**v14(127, v2.10 항목 고치기·지우기 — `updateDeliverable`·`deleteDeliverable`. 사용자 승인 2026-09-25. `importVendorQuote`는 v15 예약 — Phase 4.7이 소진)**·**v14.1(127 불변 — §9 `ProjectPatch.slack_webhook_url` 필드 추가만. 묶음 2 범위 승인이 동결 해제 승인을 겸함, v13.1 전례)**·**v15(131, v2.11 협력사 견적서 불러오기 — `importVendorQuote`·`listVendorQuoteImports`·`confirmVendorQuoteImport`·`discardVendorQuoteImport`. v7부터 예약해 온 `importVendorQuote` 슬롯을 Phase 4.7이 소진. 묶음 1→2→3 범위 승인(2026-09-25)이 동결 해제 승인을 겸함 — v14 항목이 이미 "v15 예약 — Phase 4.7이 소진"으로 예고)**·**v15.1(131 불변 — §9 v2.12 `ProjectPatch.slack_thread_url` · `PersonInput`/`PersonWithAssignments.slack_user_id` · `DeliverableDetail.request_acks` 필드 추가만. Phase 6.1 착수 지시(2026-09-26)가 동결 해제 승인을 겸함, v13.1·v14.1 전례)**. ·**v15.2(131 불변 — §23.5 v2.13 `GuideSection.data` · `GuideSectionInput.data`(표 섹션 데이터) + `GuideSectionKind` 9종 추가만. 시안 승인 "승인 → PR-A 착수"(2026-09-26)가 동결 해제 승인을 겸함, v15.1 전례)**·**v15.3(131 불변 — §23.6 v2.13.1 `ScenarioBlockKind`에 `emergency` 추가만. 같은 시안 승인 + "병합하고 다음스텝 진행"(PR-B)이 동결 해제 승인을 겸함)**·**v15.4(131 불변 — §23.7 v2.13.2 `PlanData.guide`(첫 운영가이드의 섹션 — 연락망 제외) 필드 추가만. 사용자 "모든 권한 허용하니 질문 그만" · "다음스텝 알아서 진행"(2026-09-26)이 동결 해제 승인을 겸함)**·**v15.5(131 불변 — §19.5b v2.14 `VendorQuoteParsed.format`에 `'ai'` 값 · `VendorQuoteParsed.reader?`(AI가 읽었다는 표시) 추가만. `VendorQuoteImportInput`·메서드 시그니처 불변 — PDF·사진은 같은 `importVendorQuote`가 받는다. 사용자 선택 "협력사 견적서 PDF·사진만 먼저"(2026-09-26)가 동결 해제 승인을 겸함)**·**v15.6(131 불변 — §10 S0 v2.15 `Project.intake`·`Project.quote_attachment`(선택 필드) · `ProjectPatch.intake`·`ProjectPatch.quote_attachment` 추가만. 사용자 지시 3건 + "나 이제 잘꺼니까 잘 해놔"(2026-09-26 — 진행 위임)가 동결 해제 승인을 겸함, v15.4 전례)**. 다음 예약 슬롯은 없다 — 새 메서드는 다시 사용자 승인부터. v5부터는 MockProvider와 SupabaseProvider가 동시에 이 인터페이스를 구현한다(Phase 4 — **v2.7: SupabaseProvider는 v12 124메서드를 무수정 구현, `src/providers/supabase/`**). 매 해제는 사용자 승인+본 문서 개정 동반이 조건 — **v6은 이 조건을 어기고 코드가 선행했다(2026-08-22 Phase 3.13). v2.1이 사후 정본화하며, 재발 방지 규칙은 §4-21 말미에 둔다**
...
> | 토큰·견적 참조 FK의 on-delete | 미기재 | `approvals.decided_via_token`·`settlement_boards.quote_id` = **set null** · `comments.author_token` = **cascade** | 2026-09-07 dev 실측(3단): 행사 cascade 삭제가 이 FK에 막혔다. 회수(`revoked_at`)·확정은 삭제가 아니라 운영 경로 무영향. 코멘트는 `comments_author_present` check 때문에 null 불가 — 사후 승인 대상 |
>
...
create table projects (
  ...
  slack_thread_url text,              -- v2.12 행사 스레드 링크(봇이 답글로 알림)
...
-- 협력사 견적서 업로드 (v2.2 스키마 확정 → v2.11 구현, 스키마 불변) — §19.5·§19.5a
create table settlement_imports (
  id           uuid primary key default gen_random_uuid(),
  board_id     uuid not null references settlement_boards(id) on delete cascade,
  file_name    text not null,
  drive_file_id text,                             -- 원본 보존 — Drive 행사 폴더 02_견적·정산/협력사 견적서 (v2.11)
  vendor_id    uuid references vendors(id),
  parsed       jsonb,                             -- 파싱 결과 = 제안(행·버킷 제안·부가세 판정 — §19.5a). 확정 금액의 출처
...
### 7.8 (v2.10) 항목 삭제와 Drive — 파일은 지우지 않는다
- 항목을 지우면(pm, §8 `DELETE /deliverables/{id}`) DB 행은 지우고, **항목 폴더**(디자인 `05_산출물/디자인/{항목}` · 운영 `04_운영/{항목}`)는 **그 행사 폴더의 `99_archive/`로 옮기며 이름 끝에 `(삭제됨 YYMMDD)`**를 붙인다 — 행사 삭제(§7.7)와 같은 규약을 행사 안에서 한 단계 낮춰 쓴다. `99_archive`는 인박스 스캔 제외 파트(§7.3)라 옮긴 파일이 인박스에 다시 뜨지 않는다.
- 서버(`api/drive` `archive-item`)가 다시 확인한다: 요청자 = 그 행사 pm · **항목이 DB에서 이미 지워졌다**(살아 있는 항목의 폴더는 409) · 폴더에 **이 항목의 표식**(`appProperties.communicator_deliverable_id`)이 있고 그 행사 폴더 안에 있다(밖 403). 공통 영역 항목은 파트 폴더(`01_기획`·`02`·`03_회의록`)에 바로 두므로 **폴더를 옮기지 않는다** — 다른 항목 파일이 딸려 가기 때문이다.
- 폴더 밖에 있는 버전 파일(§7.2b 행사 폴더 안 참조로 등록한 파일 · 공통 영역 파일)은 제자리에 둔다. 버전 행이 사라지면 스캔이 그 파일을 '처음 보는 파일'로 인박스에 올리므로, `delete_deliverable`이 **처리됨(dismissed) 인박스 행**으로 남겨 아는 파일로 둔다.
- 보관은 best-effort — 실패·미연결이어도 삭제는 끝나고 폴더는 제자리에 남는다. 화면은 결과(보관함으로 옮김 / 원래 자리에 남음)를 삭제 확인 창에서 알린다.
...
> **(v2.7) 실행 자리 매핑** — 아래 표의 경로는 논리 계약이고, 구현은 세 자리로 나뉜다: ① 단일 표 CRUD = PostgREST(RLS) ② 한 트랜잭션이 필요한 다단계 쓰기 = SQL RPC(`…1600_rpc_core.sql`: add_member·remove_member·remove_person·delete_project·complete_onboarding·replace_wbs_tasks·seed_project_sets·save_scenario_blocks·save_guide_sections·finalize_quote·review_partner_submission·transition_deliverable·upload_version·request_approval·link_inbox_file) ③ 토큰 경로·랜딩 리드·시트 반영 = SQL RPC(`…1700_rpc_portals.sql`) — (v2.10) 항목 고치기·지우기 = SQL RPC `update_deliverable`·`delete_deliverable`(`…20260925000100_deliverable_edit_delete.sql`) ④ TS 엔진·외부 API가 필요한 것 = **Vercel Functions** `api/quote-recalc`(POST /quotes·/quotes/{id}/versions의 서버 재계산)·`api/sheets`(§24 시트 probe·preview·rows). 오류 규약은 RPC가 `raise exception 'CODE: 메시지'`(errcode P04xx)로 던지고 프론트 `mapPgError`가 ProviderError로 옮긴다.
...
| PATCH /deliverables/{id} | pm·역할-영역 일치(담당·가이드 = **pm**) | 항목 고치기 (v2.10) — **보낸 키만** 바꾼다(빈 칸 = null): 제목·카테고리·마감 / pm: 담당·브리프·참고 링크·규격 4종. 상태·영역·파트너는 이 경로로 바뀌지 않는다(상태 = §5 전이표 경로만). 큐시트·시나리오·운영가이드로/에서 카테고리 전환 P0409(빌더 데이터가 카테고리에 매여 있다) · 빈 제목·카테고리 P0422 · 멤버 아닌 담당 P0422 · 종료 행사 P0409. 구현 = RPC `public.update_deliverable(p_deliverable, p_patch jsonb)`, 로그 `deliverable.updated {fields}` |
| DELETE /deliverables/{id} | **pm** | 항목 지우기 (v2.10) — **모든 상태**(사용자 결정 — 화면은 항목 이름 입력으로 확인). 버전·컨펌·코멘트·큐·시나리오 블록·가이드 섹션 cascade, WBS 연결 set null, 이 항목으로 등록된 인박스 파일은 닫고(dismissed) 버전 파일은 처리됨 인박스 행으로 남긴다(§7.8). 종료 행사 P0409 · 없는 항목 P0404. 구현 = RPC `public.delete_deliverable` → 반환 `drive_folder_id`로 §7.8 보관, 로그 `deliverable.deleted {title, category, area, status}` |
...
| POST `archive-item` | 그 행사 pm | (v2.10) 지운 항목의 폴더를 행사 폴더 `99_archive`로(§7.8) — 항목이 DB에 남아 있으면 409 · 이 항목 표식 폴더만(공통 파트 폴더는 `not_item_folder`로 건너뜀) · 행사 폴더 밖 403 |
...
### 8.4 (v2.10.1) Slack 알림 API — Vercel Function `api/notify` (Phase 6, §9)

| Method·액션 | 권한 | 동작 |
|---|---|---|
| GET /api/notify | 공개 | `{slack, bot, cron}` — 공용 채널·봇 토큰(v2.12)·크론 비밀이 설정됐는가(값은 내보내지 않는다) |
| GET /api/notify + `Authorization: Bearer CRON_SECRET` | Vercel cron | 밀린 사건 + D-1 리마인드 + 미등록 파일 묶음. 비밀 미설정 503 · 불일치 401 |
| POST `drain` | 로그인 세션 **또는** 살아 있는 발주처·파트너 링크 토큰 | 밀린 사건 선점 → 채널별 전송 → sent·failed·skipped 기록. 요청은 Slack 실패와 무관하게 200(본 동작을 막지 않는다) |
| POST `test` {project_id} | 그 행사 pm | (v2.12) 행사 스레드(봇) → 행사 채널 → 공용 채널로 시험 한 줄. 보낼 곳 없음 409(행사 설정 ③ 안내) · Slack 거부 502(봇 초대·채널·토큰 문구) |
| POST `remind` {project_id, target: delayed·approval} | 그 행사 멤버 | 홈 리마인드 — 목록 선점(시간당 1회, 두 번째 409) → 전송. 목록이 비면 `{sent:false}` |

| (v2.12) POST /api/slack-interact | Slack(서명 `X-Slack-Signature` · 5분 창) | 의뢰 카드 '확인했어요'(`action_id=ack`, 값 = card_id) → 누른 사람을 주소록에서 찾고(Slack ID → 없으면 봇 `users.info` 이메일로 찾아 적어 둠) → 멘션된 사람이면 확인 기록 + `response_url`로 카드 교체(버튼 → "✓ 이름 확인함 · 시각") · 아니면 그 사람에게만 보이는 안내. 서명 틀림·시간 밖 401 · 비밀 없음 503 · 항상 200(Slack 3초) |

SQL(`…20260925000200_notifications.sql`): 표 `notification_log`(선점 키 · RLS on · 정책 없음 · service만) · service 전용 `notify_claim_events`·`notify_claim_reminders`·`notify_claim_manual`·`notify_mark` · 첫 설치 때 최근 2일 사건은 `skipped`로 표시(배포 직후 첫 신호가 지난 사건을 쏟지 않게) · 30일 지난 선점 기록은 매일 정리. 서버 env: `SLACK_WEBHOOK_URL`(공용) · `CRON_SECRET` · `APP_BASE_URL`(선택). DataProvider v14.1 — `ProjectPatch.slack_webhook_url` 필드만(발송·시험·리마인드는 인터페이스 밖 연동 층 `src/lib/notify/`).

(v2.12) SQL(`…20260926000100_slack_bot.sql`): `projects.slack_thread_url` · `profiles.slack_user_id`(pm 열 권한 추가) · 표 `request_acks`(카드 1장 = card_id, 항목마다 1행 — 받는 사람·채널·ts·확인 시각·확인한 사람 · RLS = 멤버 읽기만, 쓰기는 service) · `notify_claim_events` 재정의(내부검토 요청 + 행사 스레드 + 받는 사람 `recipients` + 카드 필드) · `notify_claim_reminders` 재정의(받는 사람 + 항목 마감 D-1 + 24시간 미확인 의뢰 — 카드마다 한 번) · service 전용 `notify_record_card`·`notify_ack_card`·`notify_set_slack_user` · `list_people`에 Slack ID. 서버 env 추가: `SLACK_BOT_TOKEN` · `SLACK_SIGNING_SECRET`(둘 다 서버 전용). 서버가 요청하는 곳 = `https://slack.com/api/…`(봇) · `https://hooks.slack.com/…`(웹훅·버튼 응답)뿐. DataProvider **v15.1** — `ProjectPatch.slack_thread_url` · `PersonInput.slack_user_id` · `PersonWithAssignments.slack_user_id` · `DeliverableDetail.request_acks` 필드만(131 불변).

### 8.5 (v2.11) 협력사 견적서 불러오기 API (Phase 4.7, §19.5a)

| Method·Path | 권한 | 동작 |
|---|---|---|
| POST /projects/{id}/vendor-quote-imports | pm(종료 행사 409) | 엑셀(.xlsx)을 **앱이 읽어**(파서 = §22.2) 제안만 저장 — `settlement_imports` 한 행(status='parsed' · parsed · questions). 정산보드 없음 409 · .xlsx 아님·항목 표 없음·없는 협력사 422. **항목은 만들지 않는다**(PostgREST + RLS `settlement_imports_write`) |
| GET /projects/{id}/vendor-quote-imports | 멤버 | 가져오기 목록(최신순 — 협력사 이름·확정 때 만든 항목 수) |
| POST /vendor-quote-imports/{id}/confirm | pm | RPC `confirm_vendor_quote_import` — {vat_included, vendor_id?, rows:[{index, bucket_id, title?}]}. 고른 행마다 발주 항목(status='ordered') · **금액은 저장된 제안에서** · 부가세 포함이면 `round(v/1.1)` + 원본 보존 · 원가 없는 버킷·다른 보드 버킷·같은 행 두 번·없는 행·빈 목록 422 · parsed 아님 409 · 로그 = 파일 이름·건수(금액 없음) |
| POST /vendor-quote-imports/{id}/discard | pm | status='discarded'(항목 없음). parsed 아님 409 |
| PUT /api/drive?action=`settlement-file`&import_id&name | 그 행사 pm(JWT) | (Drive 연결 시) 원본을 행사 폴더 `02_견적·정산/협력사 견적서`에 한 번에 업로드(4MB 초과 413 · id 형식 400) → service 전용 `settlement_import_set_file`로 기록. 판정 = `drive_settlement_file_check`(pm · 확인 대기 · 종료 행사 아님 — 확정 뒤 409). 실패해도 가져오기는 그대로 |
...
| Method·Path | 권한 | 동작 |
|---|---|---|
| GET /api/ai | 누구나 | `{enabled, model, daily_limit}` — 키·저장소 env가 있는가 · 모델 이름 · 하루 한도. **키 값은 어떤 응답에도 없다** |
| POST /api/ai?action=`vendor-quote` | 그 행사 pm(JWT · 종료 행사 409) | 본문 `{project_id, file_name, media_type, data_base64}`(PDF·JPEG·PNG·WEBP · 원본 3MB 이하). 순서 = ① 키 없음 503(엑셀 안내 — 흉내 내지 않는다) ② 형식 400 · 크기 413 · **내용 서명**(확장자만 바꾼 파일) 400 — 여기까지 한도를 쓰지 않는다 ③ `ai_usage_claim`(사용자 JWT) — 권한 403 · 없는 행사 404 · 종료 409 · **하루 한도 429** ④ Claude(구조화 출력) ⑤ 결과 검사 → `docFromAiVendorQuote`(검산은 우리 코드) ⑥ `ai_usage_finish`(service). 응답 `{doc, model, usage:{used, limit}}` — **저장하지 않는다**(앱이 받은 doc으로 §8.5 가져오기를 그대로 탄다). Claude 쪽 오류(키·과부하·연결) = 503/502 + `failed`(한도에서 뺀다) · 거절·길이 초과·견적서 아님·항목 없음 = 422 + `unreadable`(한도에 센다 — 호출 비용이 났다) · 응답 모양 틀림 = 502 + `failed` |

SQL(`…20260926000400_ai_usage.sql`): 표 `ai_usage`(RLS on · 정책 없음 · authenticated 권한 없음 — 앱이 자기 기록을 지워 한도를 우회할 수 없다) · `ai_usage_claim(p_project, p_feature, p_limit)`(authenticated · definer — pm · 종료 안 된 행사 · 사람마다 트랜잭션 잠금 · KST 하루 · 'failed' 제외) · service 전용 `ai_usage_finish`(claimed 줄만 한 번). 서버 env = `ANTHROPIC_API_KEY`(필수) · `ANTHROPIC_MODEL`(기본 `claude-sonnet-5`) · `AI_DAILY_LIMIT`(기본 30, 1~500) — 전부 서버 전용. 최대 실행 시간 120초(`vercel.json` functions — SDK 제한 시간 100초).

---

### 8.7 (v2.15) 행사 인테이크 API (Phase 6.2, §10 S0)

| Method·Path | 권한 | 동작 |
|---|---|---|
| GET /api/intake | 누구나 | `{slack, ai, daily_limit}` — 봇 토큰·AI 키가 설정됐는가(값 없음) |
| POST /api/intake?action=`read` | 로그인한 내부 사용자(JWT → 주소록) | `{link}` 또는 `{text}` (+ `project_id`) → ① 링크면 봇이 글 한 개를 읽는다(`conversations.history` latest=oldest=ts · 답글 링크는 `conversations.replies`) — 봇 토큰 없음 503(글 붙여 넣기 안내) · `missing_scope` 403(**어느 권한을 더할지 그대로**: channels:history · groups:history · files:read → 재설치) · `not_in_channel` 403(앱 추가 안내) · 못 찾음 404 ② 글 정리(Slack mrkdwn → 읽기 글 · 사람 멘션은 '@담당자') ③ **라벨 규칙**(`src/lib/intake/eventBrief` — 서버·mock 공용) ④ AI 키가 있고 글이 20자 이상이면 `ai_usage_claim(project_id|null, 'project_intake')` → Claude(구조화 출력 · 개인정보 칸 없음 · 오늘 날짜를 알려 연도 없는 날짜를 해석) → 검사 → **라벨 값이 이긴다**(mergeBrief). AI 실패·거절·한도(429)는 규칙 결과 + `ai_note`로 답한다(인테이크를 막지 않음) · 권한 오류(403·404·409)만 그대로 ⑤ 응답 `{source, message{permalink, posted_at, posted_by, text, channel, thread_ts}|null, files[{id,name,mimetype,size,looks_like_quote}], links[{url,label,looks_like_quote}], fields, filled, method ai|rules, ai_note}`. **서버는 저장하지 않는다**(글 원문도) — 앱이 사람 확인 뒤 `updateProject` |
| POST /api/intake?action=`slack-file` | 로그인 + Drive 쪽 판정(그 행사 pm · 종료 안 됨) | `{project_id, file_id}` → `files.info` → 4MB 초과 413 → 봇 토큰으로 내려받기(`files.slack.com`만 · HTML이 오면 권한 없음 403) → `projectFileOp`(행사 폴더 `02_견적·정산/견적서`) → `{file_id, file_name, url, mimetype}`. 기록은 앱이 `quote_attachment`로 |
| PUT /api/drive?action=`project-file`&project_id&name | 그 행사 pm(JWT → `drive_project_file_check`) | 본문 = 파일 바이트(4MB 이하 · 빈 파일 400 · id 형식 400 · 종료 409) → 행사 폴더 `02_견적·정산/견적서` → `{file_id, file_name, folder_id, url}` · 로그 `drive.project_file`. 인박스는 `drive_known_file_ids`(quote_attachment 포함)로 이 파일을 안다 |
...
> **v2.12 운영 반영 완료(2026-09-26)** — 운영 DB 22번째 마이그레이션 적용 · 운영 `/api/notify` = `{bot:true}` · 서명 없는 버튼 요청 401 · 실발송(행사 스레드 등록 → 테스트 → 제작 요청 카드 → '확인했어요' → 카드 교체) 사용자 확인.

> **(v2.15) 봇 권한 추가 필요 — 인테이크(§8.7)**: 글을 링크로 읽으려면 봇 토큰 스코프에 `channels:history`(공개 채널) · `groups:history`(비공개 채널) · `files:read`(첨부 파일 내려받기)를 더하고 워크스페이스에 **다시 설치**해야 한다(Slack 앱 → OAuth & Permissions → Bot Token Scopes). 그 전까지 서버는 `missing_scope`를 받으면 무엇을 더할지 그대로 알리고, 글을 복사해 붙여 넣는 길은 언제나 열려 있다. 봇이 채널 멤버여야 읽을 수 있다(앱 추가). 봇 토큰 값은 바뀌지 않는 것이 보통이나 바뀌면 `SLACK_BOT_TOKEN` 교체(**가정** — 재설치 때 확인).
...
| (v2.10.1) 새 지시(지시 발행 — §5 부수 효과 "담당자 알림") | ● (행사마다 묶음) | — |

- Slack: 프로젝트별 Incoming Webhook 1개(설정 화면에서 등록). 메시지 포맷: `[행사코드] 이벤트 — 항목명 (링크)`.
- **(v2.10.1) 구현(Phase 6)** — 원천은 새로 만들지 않는다: **activity_log가 이미 사건을 적는다**(`version.uploaded`·`approval.requested`·`approval.decided`·`partner.submitted`·`deliverable.requested`). 서버 함수 `api/notify`(§8.4)가 아직 알리지 않은 최근(2일) 사건을 **선점**(`notification_log`에 키를 먼저 넣은 쪽만 보낸다 — 동시에 두 번 불려도 한 번)해 보낸다.
  - **즉시**: 앱(SupabaseProvider)이 사건 직후 `api/notify`에 신호만 보낸다(기다리지 않음 · 묶음 0.8초 · keepalive). 발주처 `/c`·파트너 `/p`는 로그인이 없어 그 링크 토큰으로 신호한다. 놓친 신호는 매일 크론이 거둔다.
  - **매일(Vercel cron `0 0 * * *` = KST 9시대 — Hobby는 하루 1회·지정 시각의 1시간 안, 2026-09-25 Vercel 문서 확인)**: 밀린 사건 + D-1 리마인드(컨펌 기한 — 아직 컨펌대기인 것만 · 마일스톤 · 파트너 마감 — 아직 미제출) + 미등록 파일 묶음. 키에 KST 날짜를 넣어 같은 날 중복 전달(크론은 best-effort·중복 가능)에도 한 번만. 인증 = `CRON_SECRET`(Vercel이 `Authorization: Bearer`로 보낸다).
  - **보낼 곳** = 행사 채널(`projects.slack_webhook_url` — 행사 설정 ③, pm) → 없으면 공용 채널(서버 env `SLACK_WEBHOOK_URL`) → 둘 다 없으면 **보내지 않는다**(선점 행은 `skipped` — 나중에 채널을 연결해도 옛 사건이 한꺼번에 쏟아지지 않는다). 서버는 `https://hooks.slack.com/services/…` 주소로만 POST한다(행사 설정 값도 같은 규칙 — 그 밖은 422).
  - 한 번 보낼 때 같은 채널의 줄을 한 메시지로 묶고(20줄 상한 — 넘으면 `…외 N건`), 새 지시는 행사마다 한 줄로 묶는다(주최형 WBS 전개가 수십 건을 만든다). 지워진 항목의 사건은 보내지 않는다. 링크 = 앱 주소 + `?project={id}`(받은 사람이 다른 행사를 보고 있어도 그 행사로 열린다 — ProjectContext가 받고 주소에서 지운다). 앱 주소 = `APP_BASE_URL` → 요청 출처 → Vercel 운영 도메인(+`VITE_BASE_PATH`).
  - **홈 '리마인드'·'컨펌 독촉' 버튼** = 이 행사 채널로 지연 태스크·컨펌 대기 목록(상위 5 + 건수). 같은 행사·같은 대상은 **한 시간에 한 번**(서버가 막는다 — 409 문구). mock은 보내는 흉내 없이 사실만 알린다.
  - 금액은 싣지 않는다(§19.7) — 선점 함수가 금액 열을 읽지 않고(행사 코드·이름, 항목 제목, 사람·파트너 이름, 날짜, 건수만), 문구는 화이트리스트 필드만 쓴다. 본문 = `{text, unfurl_links:false, unfurl_media:false}`. Slack 특수문자(`&`·`<`·`>`)는 이스케이프(제목으로 멘션·링크 위조 불가).
...
- **(v2.12) 봇 전환 + 의뢰 확인 버튼(Phase 6.1 — 사용자 결정 2026-09-25~26)** — 웹훅(채널 글)은 받는 사람 폰에 푸시가 가지 않고(Slack 기본 알림 = DM·멘션·키워드만) 사람을 지정할 수 없다 → **봇**(같은 Slack 앱에 권한 추가). **DM은 쓰지 않는다**(회사 운영 방식 — 채널에서 모두가 보도록) — 행사마다 **채널의 스레드 하나**가 행사 기록이고, 봇은 모든 글을 그 스레드의 답글로 남기며 할 일이 생긴 사람만 **@멘션**한다(멘션 = 그 사람 폰 푸시).
  - **보낼 곳** = 행사 스레드(서버 `SLACK_BOT_TOKEN` + `projects.slack_thread_url` — 행사 설정 ③에 Slack '링크 복사' 주소, pm) → 행사 웹훅(예비) → 공용 웹훅 → 없음. 스레드 링크 = `https://{워크스페이스}.slack.com/archives/{C|G…}/p{16자리}`(답글 링크면 `thread_ts`가 스레드) 또는 `app.slack.com/client/…/thread/{채널}-{ts}` — DM 채널(D)은 받지 않는다(`src/lib/slackThread.ts`, 앱·서버 공용). 비공개 채널은 채널 세부정보 → 에이전트 및 앱 → 앱 추가가 먼저(2026-09-26 실측: 채널 매니저의 `/invite`가 "초대 권한 없음"으로 막힌 사례 — 앱 추가 탭 경로 사용).
  - **멘션 대상(결정 ② 표)**: 새 지시 → 항목 담당자 · 내부검토 요청(`status.transitioned` → internal_review — v2.12에서 알릴 사건에 추가)·파트너 제출 → PM · 발주처 승인·수정요청 → 담당자 + PM(수정요청은 발주처 **공유** 코멘트 앞 150자 인용) · D-1 리마인드 → 컨펌 기한 = PM · 마일스톤 = 그 영역 담당(전체면 PM) · 파트너 마감 = PM · **항목 마감 D-1**(v2.12 — 지시됨·초안·수정요청인 우리 항목) = 담당자 · 시안 올림·컨펌 발송·미등록 파일 = 멘션 없음(기록만). 멘션은 `profiles.slack_user_id` → 없으면 봇이 이메일로 찾아(`users.lookupByEmail`) **비어 있을 때만** 적어 둔다 → 못 찾으면 `이름(Slack 미연결)`(오류 없음). 담당자 화면(S-13)에서 직접 고친 값이 이긴다. 이메일은 찾는 데만 쓰고 본문에 싣지 않는다.
  - **의뢰 카드**: 새 지시(제작 요청 — 담당자) · 내부검토 요청·파트너 제출(검토 요청 — PM)은 줄 대신 **카드**(Block Kit — 마감·규격·종류·위치·가이드 앞 150자 / 버전·메모·파일 이름) + 버튼 `확인했어요`(채움 — 값 = card_id) · `앱에서 열기`(링크). 같은 전달에서 같은 담당자의 새 지시가 여럿이면 카드 한 장(`n건 모두 확인했어요`). 승인·반려·착수 같은 **작업 버튼은 Slack에 두지 않는다**(앱 '다음 단계' 한 곳 — 두 곳에 상태가 생기면 어긋난다). 시안 이미지는 싣지 않는다(Slack 이미지 블록은 공개 주소가 필요 — 파일은 서명 주소로만 나간다).
  - **확인했어요**: 확인은 **상태가 아니라 표식**(§5 전이표 무변경) — 의뢰 한 건마다 `request_acks`에 기록(한 항목에 제작 요청 확인과 검토 요청 확인이 따로 생긴다). **멘션된 사람의 확인만 센다** — 다른 사람이 누르면 그 사람에게만 보이는 안내, 카드는 그대로. 확인하면 카드의 버튼 자리에 "✓ 이름 확인함 · 시각"(채널의 모두가 본다). 앱은 디자인 보드 '다음 행동'·항목 상세 '다음 단계' 아래에 한 줄(`Slack 제작 요청 확인함 · 이름 날짜` / `아직 확인 안 함 · n일째` — 지금 상태에 맞는 의뢰만).
  - **24시간 확인 없는 의뢰** = 매일 크론이 카드마다 **한 번만** 같은 스레드에 받는 사람 멘션 + 원래 카드 링크 답글(스레드 안에 스레드는 없다 — 새 카드를 또 올리지 않는다, 누를 버튼은 원래 카드의 것 하나).
  - **실패**: 봇 오류(봇이 채널에 없음·채널 없음·토큰)는 `failed` + 한국어 사유, 본 동작은 그대로. 버튼 응답 함수 = `api/slack-interact`(§8.4 — Slack 앱 Interactivity Request URL과 같은 경로). 채널 단위 슬래시 명령·DM·발주처 Slack은 없다(발주처는 Phase 6b 이메일).
...
| S-00 | 제품 런처 (2026-09-04, 도메인 루트 `/`) | 사이드바·프로젝트 셀렉터 없는 중립 지면 — 제품 카드 2장(**견적 컨피규레이터** → `/quotes` · **MICE 커뮤니케이터** → `/home`), 카드마다 요약·기능 3줄·사용 대상 캡션, 하단에 발주처·파트너 링크(`/c`·`/p`) 안내와 옛 견적 주소 자동 이동 안내. 도메인 `rmb-mice.com`이 붙으면 이 화면이 첫 화면이다 | 카드 클릭=진입(카드 전체가 링크, accent 버튼 0개 — §5 CTA 원칙). 자동 리다이렉트 없음(딥링크 불변). 사이드바 로고 = 런처 복귀(제품 갈아타기) |
...
| S0 | 새 행사 위저드 (v1.3→v1.5 재정의) | **행사 설정 ①②③과 동일한 폼 컴포넌트**를 3단계로 배치: ①행사개요(필수 4: 행사명·코드·시작일·장소) ②담당자(내부 담당자 — **(v2.9.1) 역할 칸 4개 + 주소록 인물 카드: 카드를 누르거나 역할 칸으로 끌어놓아 배정, 미리 배치 없음(생성자 PM만)** · 주소록에 없는 사람은 직접 입력·전자명함, PM 1명 필수 / 발주처 담당자·토큰 선택) ③유형·확인 — **(v2.6 §25) format 4카드**(컨퍼런스 일반형·컨퍼런스 모객형·DMS·전시회) + PSA 체크박스 + 시드된 kind·event_type 세부 토글(노출·수정 가능), WBS 전개 예고 · **(v2.11.3 — Phase 3.23 PR-8, 디자인지시서 v1.4 §7-2.12) 화면 재배치**: 카드 한 장(960) · 머리 줄 = '새 행사 만들기' + '단계마다 저장돼요' + **[나중에 하기]**(→ 행사 목록 — 1단계에 저장 안 한 입력이 있으면 먼저 확인, 행사는 지우지 않고 '먼저 확인할 행사'에 남는다) · 단계 제목(행사 기본 정보 / 담당자 배정 / 유형 고르고 확인) · 진행 줄 = 가로 단계 줄 + 'n단계 중 k단계 · 필수 4개 중 m개 입력' + 막대(1단계는 저장 전 입력을 센다) · ① = 넓은 2열(무엇을 / 언제·어디서) + 선택 항목 6개 접기(값이 있으면 펼침) + [다음: 담당자] | 완료 = onboarded_at 기록 + **format 프리셋 일괄 전개**(WBS·R&R·컴플라이언스·tier) + R&R 시드. 미완료 행사는 목록에 남고 진입 시 행사 설정으로 유도(차단 아님) · **(v2.15 — Phase 6.2) ① 머리에 'Slack 메시지에서 불러오기' 카드**(링크 또는 글 → 라벨 규칙 + AI → 폼 칸 주황 채움 · 결과 줄 = 읽은 방식 배지 · 채운 칸 n · 읽지 못한 칸 이름 · 원문 보기 · 첨부·링크 개수 · 기록 `projects.intake` · 선택 시 스레드 등록) · **행사 코드 자동**(새 행사 자리표시 코드일 때 행사명·행사일을 따라감 · 주황 · '행사명에서 자동으로 만들었어요' · 고치면 꺼짐 · '행사명에서 다시 만들기') · **선택 항목 아래 '견적서' 카드**(파일 올리기 — Drive 연결 시 · 링크 붙이기 · Slack 글의 첨부 '이 파일 첨부'·'이 링크 첨부' · 현재 첨부 = 배지·이름·출처·열기·빼기). 채운 버튼은 여전히 '다음: 담당자' 하나 |
| S1 | 홈 대시보드 (`/home` — 2026-09-04 루트가 S-00 런처가 되며 이동. 사이드바 "홈"·행사 목록/견적의 "행사로 이동"이 전부 `/home`) | 미결 컨펌(기한순) · D-day·마일스톤 · **지연/임박 WBS 태스크(v1.4)** · 미등록 인박스 · 영역별 진행률 바 · 최근 활동 | 인박스 연결/무시, 항목·태스크 바로가기, **(v2.10.1) '담당에게 리마인드'·'컨펌 독촉' = 이 행사 Slack 채널로 목록(시간당 1회 — mock은 사실만 안내)** |
| S2 | 영역 보드 (design/ops 공용) | 카테고리 그룹 카드: 상태 뱃지(지시됨 포함)·최신 vN·담당·마감 | 항목 생성, (pm) 지시 발행 폼, 필터(상태·담당), 상태 전이 |
| S3 | 항목 상세 | 지시 카드(브리프·스펙 칩, v1.2)·버전 이력(최신 뱃지)·미리보기·코멘트 스레드·컨펌 이력·Drive 폴더 링크 — **큐시트 항목은 파일 대신 정형 에디터 렌더(v1.3: 행 편집·드래그 정렬·대본 전문)** · **(v2.9.3) 업로드가 막힌 상태(컨펌대기·승인·확정·파트너 첫 제출 전)는 업로드 폼 대신 이유·다음 할 일 · 버전마다 저장 위치(실서버: Drive / 임시·저장 안 됨) · 실서버 Drive 미연결 경고 상자 · 발주처 링크 0개면 발송 폼에 경고** · **(v2.10) 항목 고치기·지우기: 고치기(pm·영역 담당 — 제목·카테고리·마감 / pm은 담당·제작 가이드, 정형 문서는 카테고리 잠금) · 지우기(pm — 항목 이름 입력 확인 → 결과(Drive 보관 여부) → 보드로). 권한 없는 역할에는 없고, 종료 행사는 비활성 + 이유** · **(v2.11.1 — Phase 3.23 PR-4, 디자인지시서 v1.4 §7-2.7) 화면 재배치: 머리 = [최신본 내려받기][⋯ 메뉴 — 고치기·지우기](옛 '항목 관리' 카드 대신 · 고치기 폼은 본문 맨 위) · '다음 단계' 카드 1장(5단계 레일 · 상태 제목·설명 · 올리기·내부검토 요청·(pm) 반려·컨펌 발송 — 머리의 '새 버전 업로드'를 옮겼다, 막힌 상태면 버튼 없이 이유) · 본문 폭 16:9 큰 미리보기 · 코멘트(공개 범위를 먼저 고름 — 내부 메모 / 발주처와 공유) · 오른쪽 = 버전 이력(누르면 미리보기 · 발송본 표시) · 컨펌 기록 타임라인 · 제작 가이드(비면 pm '채우기')** | 버전 업로드, 전이, (pm) 컨펌 발송(큐시트=스냅숏 자동), 항목 고치기·지우기 |
...
| S6 | 행사 설정 (v1.5 재정의 — 메뉴 2번째 상시 노출) | 탭 ①행사개요: 행사명·코드·유형·시작/종료일·시작/종료 시간·장소·예상 인원·좌석 형태·주제·주최/주관·사회자·참가 대상·기타 개요 항목(overview_items) + **(v2.0) 모객형 전용 그룹: 보장 인원·쇼업 KPI·타겟팅 5축 칩, 연결 견적 링크("견적 v3 확정 기준")** — **행사개요의 단일 원천(S9 ①은 여기서 읽기 조립, 인라인 편집 제거)** / 탭 ②담당자: **(v2.9.1) 역할 칸 4개(배정 현황 — 표 대체, 칸마다 이름·직함·이메일·전화·빼기) + 주소록 인물 카드(누르기 → 역할 버튼 · 역할 칸으로 끌어놓기)** + 직접 입력·전자명함(역할 변경 = 빼기 후 재배정) + 발주처 담당자·토큰 통합 표(발급·회수·링크 복사) + R&R 미리보기 / 탭 ③유형·연동: 유형 토글 안내·Drive·Slack(**v2.10.1 Slack 카드** — 행사 채널 등록·바꾸기·해제·테스트 보내기(pm), 저장 뒤에는 끝 토큰을 가려 표시, 상태 칩 = 행사 채널·공용 채널·꺼짐, 크론 미설정 안내, mock은 발송 없음을 명시 · **(v2.12) 행사 스레드 링크가 먼저 — 등록하면 채널·'Slack에서 열기'를 보이고 웹훅은 예비로 접힌다 · 상태 칩 = 행사 스레드·봇 준비 안 됨·행사 채널·공용 채널·꺼짐**) · **(v2.11.3 — Phase 3.23 PR-8, 디자인지시서 v1.4 §7-2.12) 화면 재배치**: 탭 이름 = 개요 · 담당자 · 유형·연동(번호 없음) · 탭 줄 오른쪽 필수 요약('필수 4개 모두 입력됨' / '필수 n개 남음 — 칸 이름' — 옛 상단 체크 스트립 대체) · 개요 = 섹션 목록(기본 정보 · 일정·장소 · 내용 · 모객 설정) + 섹션 카드 · 바꾼 칸 = accent 테두리 + '원래 …' · **고정 저장 바**(바꾼 칸이 있을 때만 — 개수·칸 이름 · 변경 취소 · 저장) → '저장됨 HH:mm' · 행사 포맷은 읽기 전용(전환은 온보딩 ③ — WBS 재전개 동반) | pm 편집(타 역할 읽기). 상단에 "세팅 완료·일자" 또는 "세팅 미완료(필수 n개)" 뱃지 · **(v2.15) ① 기본 정보 섹션 끝에 '견적서' 카드**(온보딩과 같은 컴포넌트 — pm만 붙이기·빼기, 그 밖은 읽기) |
...
| S-10 | 정산보드 (v2.2, 메뉴 "운영" 그룹 마지막) | 상단 KPI 4(마진 기준 계약액·실집행·최종 마진·마진율) + **마진 구성 3분할 막대**(항목 마크업=변동 / PCO 기획료=고정 / RSVP 운영비=고정)와 검산 블록 / 버킷 표(견적·발주·실집행·마크업·마크업률, `has_cost=false`는 "원가 없음"·`ld`는 "마진 계산 밖") / 버킷 펼침 = 발주 항목 표(항목·협력사·담당·발주·실비·상태·증빙) / **협력사 견적서 불러오기 카드**(v2.11 — 불러온 이력·확인 대기 건수 → 대화상자: 파일·협력사 → 확인 큐[부가세 질문·행별 포함·제목·버킷(원가 버킷만)·'확인 필요'·비어 있는 버킷 한 번에 채우기·공급가 대조] → 확정 = 발주 항목 N개 / 나중에 / 버리기) · **(v2.11.2 — Phase 3.23 PR-7, 디자인지시서 v1.4 §7-2.11) 화면 재배치: 머리 = 제목 + '내부 전용' · [기준 견적 갱신] [협력사 견적서 불러오기](채운 — 옛 아래 카드의 버튼을 올렸다, pm 아니면 비활성 + 이유) · 머리 아래 할 일 알림(확인 대기 견적서 → 확인하기 · 견적 초과 버킷마다 → 항목 보기 · 검산 어긋남) · KPI 4장(검산 = 최종 마진 칸 배지, 마진 구성 3분할 막대도 그 칸 — 옛 '마진 구성 · 검산' 카드 퇴역) · 버킷 표(원가 없는 버킷은 그룹행 '원가 없는 항목' 아래, 펼침 없음) · 발주 항목 = 행 끝 ⋯ 메뉴(금액·상태 입력 · 지우기) · **초과 사유 = 발주 항목 메모(note)**(버킷에 사유 칸 없음 — 스키마 불변) · 불러온 견적서 이력은 있을 때만 맨 아래** · **(v2.14 — §19.5b) 불러오기 파일 = 엑셀·PDF·사진 — PDF·사진은 AI(Claude)가 읽고 같은 확인 큐로(머리에 'AI가 읽음' 배지 + 원본 대조 안내 · 버튼 'AI로 읽기' · 읽는 중 안내 · mock은 사실 안내)** | ＋발주 항목(pm)·발주액/실비 입력(담당)·버킷 추가(pm)·기준 견적 바꾸기(pm)·협력사 견적서 올리기. **발주처에게 절대 노출되지 않는다(§19.7)**. 견적 초과는 경고만 하고 막지 않는다 |
| S9-D | 16:9 장표 (v2.13.2 · §23.7) | 운영계획서와 같은 조립 데이터를 16:9 장표로 — 표지 · 목차 · 운영 요약 · 01~07장. 사이드바 없는 전체 화면(인쇄 = 장표만) · 도구 줄 = ← 운영계획서 · 장 수 · 싣지 못한 표(어디서 채우는지) · 인쇄 · PDF | 인쇄(PDF — 크롬·엣지에서 용지 16:9 자동) · 진입 = S9 발행 줄 '16:9 장표' |
...
- **운영보드 홈(유형 우선)**: **v2.13.1(§23.6)** 카드마다 그 유형의 숫자(큐 n개 · 멘트 n/m 작성 · 섹션 n/m 채움 · 확정 n/m) + 유형마다 다른 표 칸(큐 · 운영 시간 · 대본 작성 / 세션 · 멘트 작성 · 연사 확인 · 비상 멘트 / 섹션 채움 · 현장 인력 · 무전 · 원본 갱신 / 최신 파일) · 동작 = '열기'(정형 문서는 그 행 아래 인라인 빌더) · '올리기'(파일 없는 기타 문서). (v2.5 원문) 상단 유형 카드 4종 — 큐시트 / 시나리오 / 운영가이드 / 기타 제작물(각 카드에 건수·최신 상태 요약). 카드 선택 시 해당 유형의 문서 목록 + **인라인 빌더**(별도 화면 이동 없음 — "빌더 열기" 시 목록 아래 펼침). "+ 항목 추가"의 카테고리 선택도 같은 원칙: 정형 카테고리(큐시트·시나리오·운영가이드)를 고르면 생성 직후 해당 빌더가 인라인으로 열린다(3.15.1 P7의 완성형). 기존 데이터 이관: category='큐시트' → 큐시트 카드 / 신설 2종 카테고리 추가 / 나머지 → 기타 제작물. 자동 분류·무손실.
- **시나리오 빌더**: **v2.13.1(§23.6) 멘트 원고형** — 요약 줄(멘트 n/m 작성 · 연사 확인 대기 · 큐시트로 보내기 · 인쇄 · MC 배포용) · 왼쪽 세션 목록 · 세션 머리(시각 크게 + 제목 + 프로그램표 연동) → 블록(시각 · 구분 → 괄호 지시문 → 멘트 크게 · 빈 멘트 = '멘트 쓰기' · 연사 자리 = '연사 확인 대기') · 끝에 비상 예비 멘트. 제목·상태·컨펌 발송은 항목 상세 머리와 '다음 단계' 카드(큐시트와 같은 모양). (v2.5 원문) 세션 그룹(프로그램표 연동 표시) → 진행 블록 행(시각·구분 칩[MC/영상/의전/전환/커스텀]·대본(펼침)·비고). 상단 액션 = 큐시트로 내보내기 · 인쇄 · 컨펌 발송(스냅숏). 세션 접기. 시나리오·큐시트 역할 분리 도움말(InfoTip) 명시.
- **운영가이드 빌더**: ~~섹션 카드 4종 시드~~ → **v2.13(§23.5): 현장 운영 12섹션 시드**(설치·철거 · 인력·콜타임 · 무전·지휘 · 역할 분담 · D-day 진행표 · 구간별 체크리스트 · 등록 운영 · VIP 의전 · 안전관리 = 표로 채우는 섹션 + 존별 운영[존운영 연동] · 비상 대응 · 연락망/비품), 왼쪽 섹션 목록(준비 · 당일 · 안전·공통 — 채움 상태), 옛 문서는 '뼈대 추가'(R-O9). (v2.5 원문) 섹션 카드 4종 시드(존별 운영[존운영 연동] · 역할별 체크리스트[R&R 연동] · 비상 대응 · 연락망/비품) + 섹션 추가. 연동 섹션은 원본 변경 시 "갱신 있음" 표시 → 차이 확인 후 반영(자동 덮어쓰기 금지 — 기준 견적 갱신과 동일 패턴). 개인 연락처는 화면·S9 조립 제외, 인쇄 스냅숏에만 포함 옵션.
...
- **(v2.9.2 · §12.1) 시험용 입구 — 임시, Okta SSO 전까지**: 아래 12.1 참조.
- **(v2.14) AI 키**: `ANTHROPIC_API_KEY`는 Vercel 서버 env에만(Secret) — `VITE_` 판 금지(번들에 실리면 누구나 쓴다) · 대화·코드·PR에 붙이지 않는다 · SDK(`@anthropic-ai/sdk`)는 `api/_lib/ai/claude.ts` 한 곳에서만 import(앱 번들에 없음) · 비용 방어 = ① 1인 하루 한도(`ai_usage_claim`) ② Anthropic 콘솔의 월 사용 한도(Spend limit — 키 발급 때 사람이 건다). 시험 = 개인 계정 키(사용자 결정 2026-09-26) — 회사 계정 전환은 키 교체만. 보내는 것 = 견적서 파일 한 장(협력사 견적 — 참가자 명단·연락처는 보내지 않는다) · 규칙이 사람 이름·연락처·계좌·사업자번호를 옮기지 않게 하고 스키마에 그 칸이 없다 · 서버 로그 = 기능·상태·토큰 수만(파일 이름·금액 없음). 가드 = DoD 83 ⑥.
...
### 12.1 시험용 입구 (v2.9.2 · Phase 4.3, 2026-09-25 — 임시)

**결정**: 로그인은 **Okta SSO로 맨 마지막에**(사용자 — "okta verify 사용 중, 여기 등록해서 원키로 로그인"). 그 전까지 실서버에서
내부 기능·저장소 연동을 시험하려고 "아무나 들어갈 수 있는 게이트"(사용자 지시). 위험 고지(주소만 알면 누구나 회사 Drive 폴더·DB에
쓰기 가능) 후 사용자가 **암호 없는 완전 개방**을 골랐다(대안: 공용 암호 입구[추천] · 코드 없는 1회용 링크).

- **서버** `api/gate`(Vercel Function 1개): GET = 입구 상태(열려 있으면 주소록 인물 목록 — 이름·직함·권한만, 이메일·전화 없음) ·
  POST `{profile_id}` = 입장 — 로그인한 적 없으면 auth 사용자 생성(email_confirm → 가입 트리거가 이메일로 프로필 연결) → admin
  `generateLink(magiclink)`(메일 없음) → **`token_hash`만** 응답 → 브라우저가 `verifyOtp`로 세션(검증 스크립트와 같은 경로)
- **스스로 닫히는 장치**: 서버 env `AUTH_GATE=open` **그리고** `AUTH_GATE_UNTIL`(ISO 시각)이 지금부터 **14일 이내**일 때만 열림 —
  하나라도 없거나 틀리면 닫힘(fail-safe) · 기한 지남 410 · 앱 번들은 입구를 여닫지 못한다(스위치 = 서버 env만, `VITE_` 판 없음)
- **권한**: 입구가 올려 주지 않는다 — 고른 사람의 `app_role`·행사 역할 그대로(RLS 정상 작동). 사이드바 하단 **로그아웃**(실서버 모드만)으로
  다른 사람으로 바꿔 들어가 권한 차이를 시험한다
- **감사**: 서버 로그에 `[gate] enter profile=<id> role=<role>`만(이메일·이름 없음)
- **종료 조건**: Okta SSO(§12 개정 — Supabase Pro 필요 · IdP 등록은 CLI · `signInWithSSO`) 도입 시 `AUTH_GATE`를 지우고 이 절을 폐기한다
- 계약 = DoD 64(`src/test/dod64-open-gate.test.tsx`)
...
| (v2.2) 정산보드 S-10(마진 3분할·버킷 9+추가·항목 3단 추적·부가세 자동 분리·협력사 마스터) · (v2.11) 협력사 견적서 불러오기 — 엑셀 · (v2.14) PDF·사진 — AI 읽기 | ~~(v2.2) 협력사 견적서 PDF·사진 파싱~~(v2.14 완료)·발주서 발행·협력사 단가 누적 통계 → 견적 단가 DB화 연결 |
...
- **v2.15** (2026-09-26): **Slack 메시지로 행사 만들기(인테이크) + 견적서 첨부 + 행사 코드 자동**(Phase 6.2 — §8.7 신설 · §9 봇 권한 · §10 S0·S6① · §4 projects 열 2) — 사용자 지시 3건("행사 기본정보를 슬랙 메시지를 통해 불러와 기록" · "견적서도 첨부(파일 혹은 링크)" · "행사 코드는 행사명에 따라서 자동기입") + "나 이제 잘꺼니까 잘 해놔"(위임). Code 판단: 라벨 규칙 먼저·AI는 빈 칸만(실패가 막지 않음) · 원문 미저장 · 서버는 읽기만(저장은 앱이 사람 확인 뒤) · 첨부 = Drive 행사 폴더 + 링크 주소 · 자리표시 코드에서만 자동 · SQL 26번째 · DataProvider v15.6(필드만). 새 의존성 0.
- **v2.14** (2026-09-26): **AI(Claude)로 협력사 견적서 PDF·사진 읽기**(Phase 4.8 — §19.5b 신설 · §8.6 신설 · §2 · §12 · §10 S-10) — 사용자 질문 "시스템에 클로드 API 붙여볼 수 있어?"(예: 견적서·운영계획서 분석) → 사용자 선택 "협력사 견적서 PDF·사진만 먼저" · Sonnet 5 · 개인 계정 키로 먼저 시험 · 로그인 내부 사용자 + 1인 하루 30회. 새 의존성 1(`@anthropic-ai/sdk` — 서버 함수 전용, 범위 선택이 승인을 겸함) · SQL 25번째 마이그레이션 · DataProvider v15.5(필드만). 이후 모델 질문("fable로 전환할까?")에는 유지 권고(옮겨 적기 작업 · 응답 시간 · 비용 — env 한 줄로 전환 가능).
- **v2.13.2** (2026-09-26): **16:9 장표형 운영계획서**(Phase 3.24 PR-C — §23.7 신설 · §10 S9-D) — 사용자 선택 "16:9 장표형 인쇄" + "다음스텝 알아서 진행". DataProvider v15.4(`PlanData.guide` 필드만). 새 입력 칸 0 · 새 의존성 0. 다음 단계(입력 칸 필요) = 목표·성공 기준 · 확인 요청 · 계약 범위 대응표.
- **v2.13.1** (2026-09-26): **시나리오 멘트 원고형 · 운영 보드 문서 요약 · 큐시트 칸 이름 · 문서 머리 통일**(Phase 3.24 PR-B — §23.6 신설 · §23.1 scenario_blocks.kind +emergency · §10.2 · §9 v2.12 운영 반영 완료 기록) — 같은 캔버스 시안 승인 + 사용자 선택 "멘트 원고형". DataProvider v15.3(타입 값 1개). 이탈 3: 뒤 세션 접기 · 큐 번호 칩 · 블록 세부 이름.
- **v2.13** (2026-09-26): **운영가이드 현장 운영 섹션**(Phase 3.24 PR-A — §23.5 신설 · §23.1 guide_sections.data · §10.2) — 사용자 지적 "운영보드 네 항목의 세부내용 포맷이 다 똑같아" + 실물 운영계획서 참고, 캔버스 시안 승인. 현장 운영 12섹션(표 9종 + 존별 운영·비상 대응·연락망) · content는 data에서 만든 사본(R-O7) · 모양 검사 이중(R-O8) · 옛 문서 뼈대 추가(R-O9) · DataProvider v15.2(필드만). 이탈 1: 연락망 전화번호 인쇄(R-O6 충돌 — 미구현).
- **v2.12** (2026-09-26): **Slack 봇 전환 + 의뢰 확인 버튼(Phase 6.1)** — 사용자 결정(2026-09-25 밤: 봇 · 멘션 규칙 표 · 링크 + '확인했어요' 1버튼 / 2026-09-26: **DM 지양 — 채널에서 멘션** · **행사 = 채널의 스레드 하나** · 확인 기록 = 의뢰 한 건마다 · 시안 올림은 멘션 없음).
  ① §9 봇 규약(보낼 곳 = 스레드 → 웹훅 → 공용 → 없음 · 멘션 대상 · 의뢰 카드 · 확인 표식 · 24시간 미확인 한 번) ② §8.4 `GET {slack, bot, cron}` · `test` 스레드 우선 · **`POST /api/slack-interact`**(서명 검증) · SQL 22번째 마이그레이션
  ③ §10 S6 ③ Slack 카드 = 스레드 링크(웹훅은 예비로 접힘) · S-13 담당자 Slack 칸 · S2 디자인 보드 '다음 행동'·S3 '다음 단계' 아래 확인 한 줄
  ④ §2.1 **DataProvider v15.1**(필드만 — 131 불변). 증명 = DoD 79(서버 29 · 앱 10) + 로컬 Postgres `supabase:check` Slack 봇 15항목. 실발송 1회는 3턴(행사 스레드 링크 등록 후).
- **v2.11** (2026-09-25): **협력사 견적서 불러오기(Phase 4.7)** — 사용자 지시 "협력사 견적 불러오기 등 아직 구현 안 된 내용들 싹 진행" → 묶음 3(버튼 승인 "묶음 1→2→3").
  ① §19.5a 구현 결정 표(파싱 자리 = 앱 · 가져오기 = 제안 · 버킷 제안 = 원가 버킷만 · 부가세 판정 4단 · 확정 금액 = 저장된 제안 · 공급가 대조는 막지 않음 · 원본 보존 ·
  되돌리기 · 비노출 · 권한) ② §8.5 API 5행 · SQL RPC 3개 + `drive_known_file_ids` 재정의 ③ §10 S-10 불러오기 카드(옛 "Phase 4.7에서 열립니다" 자리)
  ④ §2.1 **DataProvider v15 = 131메서드**(v7부터 예약해 온 `importVendorQuote` 슬롯 소진 + 목록·확정·버리기). 엑셀만 — PDF·사진은 다음 단계.
  증명 = DoD 69 + 로컬 Postgres `supabase:check` 정산 가져오기 16항목. **가정**: 실서식 보정은 실샘플 수령 후.
- **v2.10.1** (2026-09-25): **Slack 알림(Phase 6)** — 사용자 지시 "아직 구현 안 된 내용들 싹 진행" → 묶음 2(버튼 승인 "묶음 1→2→3").
  ① §9 구현 규약: 원천 = activity_log(사건 5종), 선점 표 `notification_log`(한 번만 보냄), 즉시 신호(앱 → `api/notify`, 기다리지 않음) +
  매일 크론(KST 9시대 — D-1 리마인드 3종·미등록 파일 묶음) ② 보낼 곳 = 행사 채널 → 공용 env → 없음(no-op·skipped), Slack Incoming Webhook 주소만
  ③ §8.4 API 5행 ④ §10 S1 홈 리마인드 버튼 = Slack(시간당 1회) · S6 ③ Slack 카드 ⑤ 알림 링크 `?project=` — 받은 사람이 다른 행사를 보던 중이어도
  그 행사로 연다. **DataProvider v14.1**(`ProjectPatch.slack_webhook_url` 필드만 — 127 불변). 이메일(Resend)·발주처 이메일 리마인드는 Phase 6b 그대로.
  증명 = DoD 68 + 로컬 Postgres `supabase:check` 알림 14항목.
- **v2.10** (2026-09-25): **항목 고치기·지우기(Phase 4.5)** — 사용자 지시 "이미 등록한 항목을 수정/삭제하는 기능도 만들어야 함 — 지금은 항목추가로 쌓이기만 해".
  지우기 범위는 사용자 선택 "모든 상태 + 이름 입력 확인". ① API 2행(§8 `PATCH /deliverables/{id}` · `DELETE /deliverables/{id}`) = SQL RPC
  `update_deliverable`(보낸 키만 · 담당·가이드는 pm · 정형 문서 종류 전환 409 · 상태 불변)·`delete_deliverable`(pm · 모든 상태 · cascade +
  WBS 연결 해제 + 인박스 닫기 + 버전 파일 처리됨 행) ② §6.1 권한 3행 ③ §7.8 항목 폴더 → 행사 `99_archive`(Drive `archive-item` — 지워진 항목·표식·위치
  서버 재확인, 공통 파트 폴더는 옮기지 않음) ④ §10 S3 항목 관리 카드. **DataProvider v14 = 127메서드**(동결 해제 근거 = 사용자 승인 2026-09-25),
  `importVendorQuote`는 v15 예약. 증명 = DoD 67 + 로컬 Postgres `supabase:check` 항목 고치기·지우기 시나리오.
- **v2.9.3** (2026-09-25): **실사용 결함 5건 정정(Phase 4.3.1)** — 실서버 작동 테스트(시험용 입구로 관리자 입장) 중 발견.
  ① 업로드가 막힌 상태(컨펌대기·승인·확정·파트너 첫 제출 전)면 S3 업로드 카드가 고르기·끌어놓기·업로드 대신 이유와 다음 할 일을 먼저 보이고,
  헤더 '새 버전 업로드'는 비활성 + 이유(3.17b 주 액션 슬롯 유지) — 예전에는 파일을 고르고 누른 뒤에야 `pending_approval`이라는 코드로 실패했다
  ② 거부 문구에 영문 상태 코드 0 — provider 가드는 `lib/uploadGate.ts` 공용 목록·문구, SQL·Drive 함수 문구는 `상태(코드)`를 화면 이름으로 치환
  ③ 발주처 링크(회수·만료 제외)가 0개인 행사에서 컨펌 발송 폼·문서 헤더 발송에 경고 + 행사 설정 ② 링크 — 발송은 막지 않는다
  (컨펌대기에서 나가는 길은 발주처의 승인·수정요청뿐이라 링크가 없으면 항목이 멈춘다 — §5 전이표 불변, '발송 회수'는 넣지 않음)
  ④ 버전 이력에 파일의 실제 위치(실서버만): Drive · 임시(저장 안 됨 — Drive 미연결 때 `pending:` 자리표시)
  ⑤ 실서버 + Drive 미연결이면 업로드 카드에 경고 상자("지금 올리는 파일은 저장되지 않습니다") + 행사 설정 ③ 링크.
  행사 설정 `?tab=members|integration` 딥링크. 표시만 — 전이표·DataProvider 125·DB·새 의존성·색 토큰 불변. 증명 = DoD 66.
- **v2.9.2** (2026-09-25): **시험용 입구(Phase 4.3) + 회사 도메인 하위 경로(Phase 4.4)** — 실서버 작동 테스트 중 사용자 지시 2건.
  ① §12.1 신설 — 로그인은 Okta SSO로 맨 마지막(Supabase Pro·CLI 등록 필요 — 문서 확인 2026-09-25), 그 전까지 로그인 없이 주소록 인물로
  들어가는 임시 입구(`api/gate`, 위험 고지 후 사용자가 완전 개방 선택 — 서버 env `AUTH_GATE`+`AUTH_GATE_UNTIL`(14일 이내)로만 열림,
  응답은 token_hash만) + 사이드바 로그아웃(실서버만) ② §18a-2 신설 — 정식 입구 = `mkt.rememberapp.co.kr/leadgen/communicator`
  (회사 CloudFront → Vercel), `VITE_BASE_PATH` 하나에서 basename·자산·API·링크 파생, `vercel.json` 접두어 규칙, Drive OAuth 복귀 주소를
  리디렉트 URI에서 파생. DataProvider 125 불변 · 새 의존성 0.
- **v2.9.1** (2026-09-24): **담당자 배정 카드(Phase 3.22)** — 사용자 지시("미리 배치하지 말고 인물카드를 만들어서 인물을 클릭하거나 드래그앤드랍으로 설정할 수 있도록"). ① §10 S0 ②·S6 ② 담당자 = 역할 칸 4개(배정 현황, 표 대체) + 주소록 인물 카드(셀렉트 피커 대체) — 배정은 카드 누르기(역할 버튼 4개) 또는 역할 칸으로 끌어놓기 ② **미리 배치 없음** — 새 행사는 생성자 PM만(§8 불변). 먼저 제안된 '사람별 기본 역할 자동 포함'(DataProvider 필드 추가 동반)은 사용자가 방향을 바꿔 취소 ③ §4-2b 배정 피커 = 인물 카드 ④ DataProvider 125 불변(addMember·removeMember) ⑤ 화면 규격 = 디자인지시서 §7-1.8(역할은 도트·좌측 바로만, 열 수는 놓인 칸 폭 기준 컨테이너 쿼리).
...
### 4-1c. 행사 하드 삭제 (v2.8 증분 · Phase 4.1 — 사용자 승인 2026-09-07)

> 계기: 사용자 지시 — "기존 행사들을 다 지우고 임시 담당자들도 다 삭제하고 새롭게 시작하고 싶어.
> 그리고 앞으로 생성된 행사의 경우 삭제가 가능하도록(관리자 권한설정) 하고 싶어."
> 데모 픽스처 8행사가 실사용을 가리고 있었고, 되돌릴 수 있는 유일한 행사 단위 조작은 `closeProject`
> (종료 = 플래그 뒤집기)뿐이라 지울 방법이 아예 없었다.

**결정 1 — 권한 축은 `app_role='admin'`이다(프로젝트 역할 pm이 아니다).**
행사 안의 다른 파괴적 조작(멤버 제거·항목 삭제·랜딩 삭제)은 전부 그 행사의 pm이 한다. 삭제만 축이 다르다:
① 되돌릴 수 없고 ② 행사 하나가 통째로 사라지므로 "그 행사 안에서의 지위"로 판정하면 순환이다.
그래서 §6.1에 pm 열이 `—`인 행이 처음 생긴다. 이는 **admin이 sales와 갈라지는 첫 지점**이기도 하다 —
그 전까지 코드·RLS의 모든 분기는 `admin·sales` 한 쌍(`app.is_quote_user()`·`canUseQuotes`)이었다.
**기존 `admin || sales` 술어는 손대지 않고 새 술어를 추가한다**(`app.is_admin()` / `canDeleteProject`).

**결정 2 — 종료(closed)가 삭제의 선행 조건은 아니다.**
`assertWritable`·`app.require_writable`을 경유하지 않는다. 종료 행사를 지우는 것이 정상 동선인데
종료 가드를 태우면 "종료했더니 삭제가 막힌다"가 된다. 오삭제 방어는 상태가 아니라 **확인 단계**로 한다
(화면에서 행사명 타이핑 확인 — §10 S-1·S6).

**결정 3 — 견적은 지우지 않고 분리한다.**
`quotes.project_id`·`quote_imports.project_id`는 이미 `on delete set null`이다(§4-19). 견적은 금액 원본이자
골든 벡터(DoD 21)의 근거라 행사와 수명을 같이하지 않는다. 삭제 후 그 견적은 사이드바
"견적만 있음 · 행사 미생성" 그룹으로 되돌아가고 `createProjectFromQuote`로 다시 쓸 수 있다.

**결정 4 — 주소록·협력사 마스터는 무관하다.**
`profiles`(§4-2b)·`vendors`(§4-23)는 행사 비종속이다. 행사를 지우면 `project_members`만 cascade로 사라지고
사람은 남는다 — 배정이 0이 되므로 그 뒤에는 `removePerson`이 409를 내지 않는다. 이것이 "임시 담당자 삭제"의
정상 경로다(주소록 삭제 자체는 §4-2b 그대로).

**삭제 범위** — `projects` 행 1개를 지우면 FK cascade로 따라가는 표(2026-09-07 로컬 Postgres 16 실측):
`project_members`·`project_invites`·`client_contacts`·`client_tokens`·`deliverables`(→`versions`·`approvals`·
`comments`·`cues`·`scenario_blocks`·`guide_sections`)·`milestones`·`activity_log`·`unregistered_files`·
`rsvp_contacts`·`attendees`·`sheet_connections`·`sheet_source_rows`·`program_sessions`·`wbs_tasks`·
`role_charters`·`compliance_cards`·`landing_pages`(→`landing_daily_metrics`)·`settlement_boards`
(→`settlement_buckets`·`settlement_imports`·`settlement_items`)·`partner_tiers`·`partners`(→`partner_tokens`)·
`psa_slots`·`psa_requests`(→`psa_meetings`). **RESTRICT FK는 스키마 전체에 0건**이므로 막히는 지점이 없다.

**구현 자리 = `security definer` SQL RPC `public.delete_project(p_project uuid)`**(§8).
`projects`에는 **delete RLS 정책을 두지 않는다** — 정책을 열면 가드 없는 맨 삭제가 생긴다.
정책 부재 + definer RPC 단일 경로가 이중 방어다(authenticated의 직접 DELETE는 0행 매칭으로 조용히 실패).
전례는 `public.remove_person`(§4-2b) — 최상위 엔티티를 가드와 함께 지우는 같은 형태다.

**이탈 1(§12 감사 규약)** — 삭제 자체의 `activity_log`는 남지 않는다. 로그 행이 `project_id`에 매여 있고
...
그 전제 위에 서 있던 두 곳이 깨진다. 둘 다 이 증분에서 함께 고친다:
- `ProjectContext`가 localStorage의 선택 id를 **검증하지 않는다**(코드 주석이 "mock 단계에선 행사 삭제가
  없어 무효 id가 생기지 않는다"고 명시). 현재 행사를 지우면 죽은 id가 저장된 채 남아 새로고침 후에도
  모든 화면이 404가 된다 → **목록 대조 후 폴백**으로 정정.
- `MockProvider.currentUser()`가 멤버십이 하나도 없으면 `forbidden`을 던진다 → 행사를 전부 지우면 모든
  화면이 403이 되고 새 행사를 만들 경로조차 없다. SupabaseProvider는 이미 최소 권한(`reg`)·빈 `project_id`로
  폴백하므로(`ctx.ts`) **mock을 서버 쪽에 맞춘다**.
- 화면 계층: 행사 0건일 때 `ProjectProvider`가 "표시할 행사가 없습니다."로 막다른 길을 만든다 →
  §10 S-1의 빈 상태(첫 행사 만들기 CTA)로 유도한다. §4-2c 진입점 원칙(빈 상태에서 다른 화면 이름만
  안내로 주지 않는다)이 그대로 적용된다.

**DataProvider v13 재동결 = 125메서드**(1메서드 추가): `deleteProject(projectId): Promise<void>`.
**동결 해제 근거 = 사용자 승인 2026-09-07**. 기존 124메서드 시그니처 불변. `importVendorQuote`는 **v14 예약**.

### 4-2b. 담당자 마스터 (v2.7 증분 · Phase 3.20 — 사용자 승인 2026-08-29)

> 계기: 사용자 실측 — "담당자를 사전 등록된 담당자에서 배정할 수 있게. 매번 입력하는 번거로움 없이.
> 등록 옵션을 만들고 등록된 담당자는 수정/삭제 가능하도록."

**저장소는 이미 있었다.** `users`(프로필)는 이메일로 한 사람에 모이고 `addMember`가 행사 간에
재사용해 왔다 — 없던 것은 그것을 **보여주고 고치는 경로**다. 그래서 새 테이블을 만들지 않고
기존 `users`를 주소록으로 정본화한다.

| 규칙 | 내용 |
|---|---|
| 신원 키 | **이메일**(대소문자 무시). 중복 등록·중복 수정 모두 409 |
| 스코프 | **전역**. 행사와 무관하다 — `listPeople()`은 projectId를 받지 않는다 |
| 배정 | 기존 `addMember`가 그대로 한다. 새 배정 메서드를 만들지 않는다 |
| 수정 파급 | 프로필이 하나이므로 **그 사람이 올라간 모든 행사와 발주처(`/c`) 화면에 함께 반영**된다. 화면이 이 사실을 한 줄로 밝힌다 |
| 삭제 | **배정된 행사가 하나라도 있으면 409**(사용자 결정 2026-08-29). 메시지에 행사명을 담아 화면이 "먼저 여기서 빼세요"를 그대로 말한다. 행사 담당자가 조용히 사라지는 쪽이 훨씬 나쁘다 |
| 본인 삭제 | 현재 로그인 사용자는 삭제 불가(409) |
| 권한 | 등록·수정·삭제는 pm. 목록 조회는 멤버 전원 |

**진입점**(§10): 주소록은 전역이므로 사이드바 **전역 그룹**(행사 목록 옆)에 S-13으로 둔다.
행사 스코프 화면(행사 설정) 안에 전역 데이터 편집을 넣으면 "이 행사만 바뀐다"는 오해가 생긴다.
행사 설정 ②에는 **배정 피커**만 둔다. **(v2.9.1) 피커 = 주소록 인물 카드** — 누르기 또는 역할 칸으로 끌어놓기(Phase 3.22, 디자인지시서 §7-1.8).

**DataProvider v12 재동결 = 124메서드**(4메서드 추가): `listPeople`·`createPerson`·`updatePerson`·
`removePerson`. **동결 해제 근거 = 사용자 승인 2026-08-29**(요청 + 구조 결정 2건). `importVendorQuote`는
**v14 예약**(만들지 말 것 — v13은 §4-1c 행사 삭제가 소진했다, 2026-09-07 정정).

### 4-2c. 진입점 정정 (Phase 3.20 — 사용자 실측 지적 3건)

사용자가 "파트너를 등록·수정·삭제할 수 없다 / 상품을 정의할 수 없다 / 발주처·파트너가 보는 화면이
어디인지 모르겠다"고 보고했다. **셋 다 기능은 있었고 진입점이 틀렸다.**

| 지적 | 실제 위치 | 정정 |
|---|---|---|
| 파트너 CRUD | `PartnerRosterEditor` — 행사 설정 **②담당자** 탭 | 파트너 보드(S-11)에서 직접 관리. 빈 상태의 "설정으로 가라"를 그 자리에서 여는 CTA로 |
| 상품(등급) CRUD | `PartnerTierEditor` — 행사 설정 **③유형·연동** | 판매 플래너 ①에서 바로 생성·관리. 등급 0건이면 그것이 첫 화면 |
| `/c`·`/p` 화면 | 토큰 발급·복사만 있고 **여는 경로 없음** | 토큰이 있는 자리에서 새 탭으로 연다. "받은 사람은 로그인 없이 본다"를 함께 밝힌다 |

**교훈(§10 진입점 원칙 보강)**: 빈 상태에서 **다른 화면 이름을 안내로 주지 않는다.** 그 자리에서
할 수 있게 하거나, 최소한 그 화면을 여는 CTA를 준다. 화면 이름만 적힌 안내는 사용자가 그 문구를
읽고도 못 찾는다 — 이번이 그 실증이다.
...
- **(가) 도메인이 이미 다른 Vercel 프로젝트에 붙어 있을 때**(§18-5의 `rmb-mice.com` 전제):
  ...
  **DNS 변경이 없다**(Vercel 내부 이전). 전파 대기도 없다. 되돌리려면 반대로 하면 끝이다.
  이전 뒤 `rmb-mice.com` 첫 화면은 S-00 제품 런처다 — 옛 Configurator 사용자는 런처에서 "견적
...
### 18a-2. 회사 도메인 하위 경로 — `mkt.rememberapp.co.kr/leadgen/communicator` (v2.9.2 · Phase 4.4, 2026-09-25)

**결정(사용자 지시 2026-09-25 "도메인은 이걸로 교체")**: 정식 입구 = 회사 도메인의 하위 경로. 도메인은 회사 소유 그대로 —
**구매·DNS 변경 없음**. 연결 방식 = **회사 CloudFront → Vercel**(사용자 선택 — 대안 'S3 정적 업로드 유지'는 배포마다 수동 업로드·
서버 기능 교차 호출이 필요해 기각). 조사 사실(2026-09-24·25 실측): 그 주소는 회사 S3+CloudFront가 9/21 올린 **mock 정적 사본**을
서빙하고 있다(레포 밖 로컬 수정 빌드 — base·basename 하드코딩) — 하위 주소 새로고침·`/api` 403, 실서버 기능 없음.

| 층 | 구현 | 비고 |
|---|---|---|
| 빌드 | `VITE_BASE_PATH=/leadgen/communicator/` → Vite `base` → `import.meta.env.BASE_URL` | 비우면 '/' — 기존과 동일(루트 배포) |
| 라우터 | `BrowserRouter basename` = 기본 경로(끝 슬래시 없이) | 데모 아티팩트(HashRouter)는 무관 |
| 기본 경로 밖 진입 | `main.tsx`가 기본 경로 아래로 이동(`/` → 기본 경로, `/home` → 기본 경로+`home`) | Vercel 주소·옛 링크 보호 |
| 자산·링크 | `src/lib/basePath.ts` 단일 파생 — 로고·직인(`assetUrl`) · 서버 함수 기본 경로(`{base}api`) · 공유 링크·새 탭(`appUrl`·`externalViewUrl`) · 매직링크 복귀 | data:·외부 주소는 그대로(데모 치환 보존) |
| Vercel 규칙 | `vercel.json` rewrites: `/leadgen/communicator/api/:path*`→함수 · `…/assets/:path*`·`…/brand/:path*`→파일 · `…/(.*)`→`/index.html` (루트 폴백은 맨 뒤) + 토큰 지면·자산 헤더 동형 | 접두어는 설정 파일에만(코드는 env) |
| Drive OAuth | `DRIVE_OAUTH_REDIRECT_URI` = `https://mkt.rememberapp.co.kr/leadgen/communicator/api/drive` **필수**(함수는 Vercel 주소로 요청을 받아 회사 주소를 모른다) · 연결 뒤 복귀 주소도 여기서 뽑는다(같은 오리진이면 경로만) | GCP 승인된 리디렉션 URI도 같은 값 |
| CloudFront | 동작 1건 추가: 경로 `/leadgen/communicator*` → 원본 `communicator-rho.vercel.app`(HTTPS) · 캐시 끔 · 전 메서드 · Host 헤더는 넘기지 않음 | 회사 AWS 담당자 작업 — 요청서 `docs/회사도메인-CloudFront-연결-요청서.md` |

검증: `VITE_BASE_PATH=/leadgen/communicator/ npm run build && npm run deploy:check` — 로컬 Vercel 규칙 재현 + 실브라우저(딥링크·새로고침·
런처·옛 라우트·루트 이동·로고 경로·미처리 예외 0) + D2(접두어 아래 api→함수·assets·brand·토큰 헤더). 단위 계약 = DoD 65.
**주의(가정)**: Vercel 프로젝트가 Hobby 요금제 — 약관상 비상업·개인 전용(검증 2026-09-25). 회사 업무 도메인 운영 전 Pro 전환 여부를 결정한다.
...
- 단계: 엑셀(구조가 가장 또렷) → PDF → 사진. 엑셀만으로도 항목 단위가 성립한다. **v2.14: PDF·사진 = AI(Claude)가 읽는다(§19.5b)** — 같은 확인 큐.
- ~~파싱은 서버가 필요하므로 **Phase 4.7**로 둔다~~ → **v2.11 구현(§19.5a)**. 스키마(`settlement_imports`)는 v2.2에서 미리 만들었고(불변), 그동안 안내만 띄우던 버튼 자리에 불러오기 카드가 선다 — **게이트 뒤에 숨기지 않는다**(§10 진입점 원칙).
- 이 절이 §19.3의 항목 단위를 성립시키는 조건이다. 순서를 뒤집지 말 것.

#### 19.5a 구현 (v2.11 — 엑셀, 2026-09-25)

| 결정 | 내용 |
|---|---|
| 파싱 자리 | **앱(브라우저)** — 견적서 임포트 파서(§22.2 A·B·C형, 동기 xlsx 리더)를 그대로 쓴다. v2.2의 "파싱은 서버가 필요하다"는 §22에서 브라우저 파서가 성립해 해소됐다. 서버가 하는 일은 **확정**(한 트랜잭션)과 **원본 보관**뿐 |
| 가져오기 = 제안 | `settlement_imports` 한 행(status='parsed'). `parsed.rows` = 항목 행 전부 + (항목 표 밖일 때) 협력사 대행료·관리비 1행 + 절사·조정 1행 → **행 합 = 공급가(부가세 전)**. 0원 행·'총액 미포함'/선택 옵션 행은 기본 제외(담당자가 켤 수 있다). 파서 경고는 싣되 우리 견적서 임포트의 헤더 질문('인식하지 못한 헤더 항목 — 고객명·일시…')은 뺀다(여기 확인 큐는 부가세·버킷만 묻는다). **이 단계에서 항목은 만들지 않는다** |
| 버킷 제안 | ① 행사별 추가 버킷 이름이 섹션·항목 이름에 들어 있으면(딱 하나) 그 버킷 ② 섹션 이름 ③ 항목 이름 — 키워드 규칙은 §22.2-6 `mapSectionName` 정본. **원가 버킷만**(s1~s4·ot·at·원가 있는 추가 버킷) — s5·rc·ld는 원가가 없어 제안도 선택지도 없다(R-S4). 못 고르면 비워 두고 '확인 필요'. 대행료·절사 행은 금액이 가장 큰 제안 버킷에 붙이되 '확인 필요' |
| 부가세 판정 | 부가세 줄이 따로 있음 → 별도(확실) · '부가세 별도' 표기 → 별도(확실) · '부가세 포함' 표기뿐 → 포함으로 제안 + '확인 필요' · 표기 없음 → **제안 없음**(고르기 전에는 확정 불가). §19.4 파싱 행 그대로 |
| 확정 | RPC `confirm_vendor_quote_import` — 화면은 **행 번호·버킷·제목만** 보낸다. 금액은 저장된 제안에서 읽는다(검토한 값과 저장 값이 어긋나지 않게 — 위변조 방어가 아니라 일관성: pm은 원래 항목 금액을 직접 입력할 수 있다). 행마다 발주 항목 1개(status='ordered' · 발주액 = 견적서 금액, 부가세 포함이면 `round(v/1.1)` + 원본 `input_amount_raw` · 근거 "협력사 견적서 {파일 이름}" · `import_id`). 가져오기는 confirmed. **협력사 묶음 한 줄로 넣는 선택지는 없다**(§19.3) |
| 공급가 대조 | 고른 행 합 vs 견적서 부가세 전 합계(총액 − 부가세, 없으면 항목합 + 대행료 + 절사) — 1원 이내면 '=', 어긋나면 '≠ — 빠진 행 확인'. **막지 않는다**(§22.2-5 원칙) |
| 원본 보존 | Drive가 연결돼 있으면 행사 폴더 `02_견적·정산/협력사 견적서`에 한 번에 업로드(4MB 이하 — Vercel 요청 한도, 넘으면 보관만 건너뛴다) → `drive_file_id`. 인박스 스캔이 그 원본을 '모르는 파일'로 올리지 않게 `drive_known_file_ids`에 포함. **보관 실패는 가져오기를 막지 않는다** |
| 되돌리기 | 확정한 가져오기는 되돌리지 않는다 — 잘못 들어간 항목은 기존 항목 삭제(항목 단위)로 고친다. 확인 대기 건은 '버리기'(discarded) 또는 '나중에'(이력에서 다시 열기) |
| 비노출 | 가져오기·항목은 정산 표(내부)에만. 활동 로그는 파일 이름·건수만(금액 0 — §19.7). 토큰 경로 화이트리스트에 넣지 않는다 |
| 권한 | 불러오기·확정·버리기·원본 보관 = 그 행사 pm(종료 행사 409). 목록 = 멤버 |

- **가정**: 협력사 견적서는 우리 견적서보다 서식이 다양하다. 이번 파서는 가상 픽스처 2종(A형 음향·조명 — 부가세 줄·할인 행 / B형 기념품 — 부가세 표기 없음)으로만 검증했다 — 실샘플 10건으로 정확도를 실측한 뒤 인식 규칙을 보강한다(§19.8). 실파일은 레포에 커밋하지 않는다(R-Q4).
- **가정**: 같은 파일을 두 번 불러오면 새 가져오기가 된다(중복 판정 없음). 실사용에서 중복 확정이 문제 되면 파일 해시 판정을 더한다.

#### 19.5b AI 읽기 — PDF·사진 (v2.14, 2026-09-26)

| 결정 | 내용 |
|---|---|
| 자리 | 정산보드 '협력사 견적서 불러오기' 대화상자 그대로 — 파일 칸이 엑셀·PDF·사진(JPG·PNG·WEBP)을 받는다(HEIC는 받지 않고 JPG로 바꾸라고 안내). 엑셀 = 앱 파서(§19.5a) · PDF·사진 = 서버 함수 `api/ai`(§8.6)가 Claude로 읽는다. **새 화면·새 메뉴 없음** |
| 원칙 불변 | AI도 **읽기만** 한다 — 결과는 `settlement_imports.parsed`(제안)로만 저장되고 담당자 확인 → 확정 RPC(저장된 제안의 금액)를 거친다. 버킷 제안(원가 버킷만)·부가세 판정 4단·총액 미포함 기본 제외·공급가 대조(막지 않음)는 §19.5a 규칙을 **같은 코드**(`buildVendorQuote`)로 탄다 |
| 옮겨 적기 규칙 | 구조화 출력(JSON 스키마 — 섹션·항목(품명·규격·수량·단가·금액·비고·총액 포함 여부)·합계(항목 합계·대행료·비율·절사·부가세·총액)·견적일·부가세 표기) · 보이는 값만, 모르면 null(추측 금지) · 원 단위 정수, 할인 음수 · 합계 줄은 항목에 넣지 않는다 · **적혀 있지 않은 합계는 계산하지 않는다** · 사람 이름·전화·이메일·계좌·사업자번호·주소는 옮기지 않는다(**스키마에 그 칸이 없다**) · 견적서가 아니면 readable=false + 이유 |
| 검산 | Claude가 아니라 **우리 코드**(`docFromAiVendorQuote`): 섹션 소계 · 항목 합계 · 총액(적힌 공급가 + 부가세) — 적힌 합계만 대조하고, 1원 넘게 어긋나면 경고 줄(막지 않음 — §22.2-5) |
| 표시 | 확인 큐 머리에 **'AI가 읽음'**(주의 배지) + "금액을 원본 견적서와 한 줄씩 대조한 뒤 확정하세요 — 잘못 읽은 금액은 그대로 발주액이 됩니다" · 캡션 `AI 읽기(모델) · 행 n개` · 경고 첫 줄 = AI 대조 안내. 저장 모양 = `parsed.format='ai'` + `parsed.reader={kind:'ai', model}`(엑셀은 reader 없음) |
| 파일 | 사진은 앱이 긴 변 2000px JPEG로 줄여 보낸다(1.5MB 넘거나 2000px 넘을 때 — 흰 바탕) · 원본 3MB 상한(서버 함수 요청 4.5MB 안 — base64) · 서버가 내용 서명(%PDF · JPEG · PNG · WEBP)을 다시 본다 · **원본은 줄이지 않은 파일 그대로** Drive `02_견적·정산/협력사 견적서`에 형식 그대로 보관(4MB 이하 — §19.5a) |
| 권한·한도 | 그 행사 pm · 종료 안 된 행사(불러오기와 같다) · 한 사람 KST 하루 `AI_DAILY_LIMIT`(기본 30)번 — Claude 쪽 오류는 세지 않고, 읽었지만 쓸 수 없는 결과(거절·길이 초과·견적서 아님·항목 없음)는 센다. 한도 초과 문구는 "엑셀 견적서는 한도 없이 불러올 수 있습니다"로 끝난다 |
| 데모(mock) | AI 서버가 없다 — **흉내 내지 않고** "PDF·사진은 실서버에서 AI(Claude)가 읽습니다 — 데모에서는 엑셀로 시험해 보세요"(가져오기 0건) |
| 모델 | 기본 `claude-sonnet-5`(사용자 결정) · `ANTHROPIC_MODEL`로 코드 변경 없이 바꾼다 · 사고 예산을 고정하지 않는다(모델 기본) · 노력(effort) medium · 출력 상한 16,000 토큰(넘치면 "쪽을 나눠 올려 주세요") |

- **가정**: 실협력사 견적서(스캔 PDF·휴대폰 사진)로 정확도를 아직 재지 않았다 — 개인 키 시험 기간에 실물 몇 건으로 오독 유형을 모은 뒤 규칙 문장·모델을 조정한다. 실파일은 레포에 커밋하지 않는다(R-Q4).
- **가정**: 다른 모델(Opus·Fable 계열)도 같은 구조화 출력·PDF 입력 방식으로 동작한다 — 전환할 때 1건 실측으로 확인한다.
- **아이디어(미착수 — 사용자 "꼭 필요한 게 아니면 추가하지 않아도 됨")**: 운영계획서 점검(빠진 항목·모순) · 시나리오 멘트 초안 · 견적 분배·WBS 제안(사용자가 "필요할 것 같기도"라고 한 자리) · 그 밖의 '판단을 요청하는 자리'. 착수는 사용자 지시부터 — `ai_usage.feature` CHECK와 `ai_usage_claim` 기능 목록을 늘리는 개정이 함께 간다.
...
- 협력사 견적서 파싱의 정확도 목표치는 정하지 않았다. Phase 4.7 착수 시 엑셀 10건으로 실측한 뒤 정한다 — **가정**. (v2.11: 엑셀 파서는 가상 픽스처 2종으로만 검증 — 실샘플 대기)
...
scenario_blocks (
  ...
  time text null, kind text not null,             -- 'mc'|'video'|'protocol'|'transition'|'custom' · v2.13.1 §23.6 +'emergency'(비상 예비 멘트 — session_id·time null)
...
guide_sections (
  ...
  sort_order int,
  data jsonb null )                               -- v2.13 §23.5 표 섹션 데이터(type = kind) — 표 9종 필수 · emergency 선택 · 나머지 null
```
...
### 23.5 운영가이드 현장 운영 섹션 (v2.13 · Phase 3.24 PR-A · 2026-09-26)

계기 — 사용자 지적 "운영보드 네 항목의 세부내용 포맷이 다 똑같아"와 실물 운영계획서(7장 + 별첨, 44장) 제시. 운영가이드는 **현장 스태프가 들고 다니는 매뉴얼**이라
자유 글 4섹션으로는 실물 수준(설치·철거 타임라인 · 인력·콜타임 · 무전 채널 · 역할 분담 ●/○ · D-day 진행표 · 구간별 체크 · 등록 처리 용량 · 안전 5항목)을 담지 못한다.
캔버스 시안(운영 문서 3종 실무화 ③-1·③-2) 승인, 섹션 범위 = "현장 운영과 관련되는 전부"(사용자 선택).

**섹션 12종(정본 순서 — 새 문서 시드 순서이자 옛 문서에 끼울 자리의 기준)**

| 묶음 | kind | 제목 | 데이터(data.type = kind) | 뼈대 출처 |
|---|---|---|---|---|
| 준비 | setup | 설치·철거 일정 | rows{date,time,task,place,owner} · notes[] | 행사일(D-1·D-day 요일 표기) · 시작·종료 시각 · 장소 |
| 준비 | staffing | 현장 인력·콜타임 | rows{role,count,call_time,duty,channel} · extra | 첫 줄 인원 = 행사 담당자 수 · 합계 자동(인원을 적은 줄만) |
| 준비 | radio | 무전·지휘 체계 | channels{code,name,members} · chain[] · rule | 4채널 템플릿 · 지휘 흐름 첫 단계 = 대행형 '주최 요청' / 주최형 '파트너·연사 요청' |
| 준비 | raci | 역할 분담 | parties[3] · rows{area,marks[3],note} — mark = main ● / help ○ / none — | 대행형 = 주최사·우리·협력사·베뉴 / 주최형 = 파트너사·우리·협력사·베뉴(고객 주관 → 우리 주관, **가정**) |
| 당일 | dayplan | D-day 진행표 | rows{group(pre·main·post),time,segment,content,av,owner,session_id} | 사전 준비 4줄 템플릿 · 본행사 = 프로그램표 세션(시각 순, session_id) · 마무리 2줄 |
| 당일 | checklists | 구간별 체크리스트 | blocks{title,span,style(check·timeline),items{at,text}} | 오픈 준비(체크) · 리허설 구성 · 브레이크 · 클로징·퇴장(시각·할 일) |
| 당일 | registration | 등록 운영 | lines · seconds_per_person · peak_minutes · peak_arrivals · notes[] | 1인 15초 · 피크 15분(**가정** — 현장 실측으로 바꾼다) · 라인·도착 인원은 비움 |
| 당일 | vip | VIP 의전 | rows{target,arrival,route,seat,owner} | 비움(대상자 명단은 고객에게서) |
| 안전 · 공통 | safety | 안전관리 | rows{item,action,owner} | 피난·혼잡·응급·시공 안전·보험 템플릿 |
...
**등록 처리 용량** — 처리 속도 = 라인 × 60 ÷ 1인 처리 초(소수 첫째 자리) · 쌓이는 줄 = max(0, 피크 도착 − 처리 속도 × 피크 분) · 최대 대기 = 쌓이는 줄 ÷ 처리 속도(올림). 도착이 구간 안에 고르게 온다는 가정. 라인·처리 시간이 없으면 계산하지 않는다(추측 금지).

**계약 추가 (R-O7~R-O9)**

| # | 계약 |
|---|---|
| R-O7 | 표 섹션의 정본은 data다 — 저장할 때 provider가 content를 data에서 다시 만든다(초경량 마크다운 `###`·`-`만). 운영계획서 ⑦·발송 스냅숏·인쇄는 content를 읽으므로 바뀌지 않는다. 사람이 content를 따로 적어 보내도 data가 있으면 덮인다 |
| R-O8 | 모양 검사 — 표 9종은 data 필수 · data.type = kind · 마크다운 섹션(zone·role·contacts·custom)은 data 없음 · emergency는 선택. 위반은 validation(400)이고 DB CHECK(`guide_sections_data_shape`)가 이중으로 막는다 |
| R-O9 | 옛 문서는 그대로 열린다 — '뼈대 추가'는 빠진 종류만 정본 순서 자리(정본 순서가 더 뒤인 첫 섹션 앞)에 끼우고, 기존 섹션의 순서·id·내용·stale 표시는 바꾸지 않는다. 이미 있는 종류는 다시 만들지 않는다. 자동 실행 없음(사람이 누른다) |

**이탈** — 시안 ③-2의 '연락망 전화번호는 스태프 인쇄본에만'은 R-O6(개인 연락처를 문서에 적지 않음)과 충돌해 구현하지 않았다. 연락망은 기존 마크다운 섹션 그대로다(필요하면 R-O6 개정과 함께 결정).

**구현** — SQL `20260926000200_guide_structured.sql`(data 열 · kind CHECK 재정의 · 모양 CHECK · `save_guide_sections`가 data 저장) · 순수 함수 `src/lib/guideStructured.ts`(템플릿·계산·글 변환·뼈대 끼우기 — provider 2종과 빌더가 공용) · 빌더 `src/components/guide/`(섹션 목록 + 섹션 카드 + 표 읽기·고치기 부품). 증명 = DoD 80.

### 23.6 시나리오 멘트 원고형 · 운영 보드 문서 요약 · 큐시트 칸 이름 (v2.13.1 · Phase 3.24 PR-B · 2026-09-26)

**계기** — §23.5와 같은 지적("운영보드 네 항목의 세부내용 포맷이 다 똑같아"). 사용자 선택 = 시나리오 **멘트 원고형**(표형 대신 MC가 그대로 읽는 원고).

**시나리오 원고형**

| 항목 | 규칙 |
|---|---|
| 멘트 vs 지시문 | 읽는 멘트 = `mc`·`protocol`·`emergency`의 script(크게, 빈 줄 = 문단). 영상·전환·지시(`custom` — 라벨 '지시')의 script는 읽는 말이 아니라 괄호 지시문으로 보인다. note = 괄호 지시문(비상 멘트에서는 상황 이름) |
| 묶음 순서 | 프로그램표 세션(시각 크게 + 제목 + 길이·연사·비고 한 줄) → 세션 밖 진행 → 비상 예비 멘트 |
| 비상 예비 멘트 | `kind='emergency'` · session_id·time 없음 · 멘트 작성 수에 섞지 않는다 |
| 연사 자리 | 대괄호 `[연사 이름]`·`[연사]`·`[소속]`·`[직함]`과 ○○○·OOO = '연사 확인 대기'. 프로그램표에 값이 들어오면 '프로그램표 연사로 채우기'(대괄호만 — ○○○는 무엇의 자리인지 몰라 채우지 않는다) |
| 뼈대(시드) | 빈 문서에서만(R-O3). 세션마다 MC 소개 멘트 1개 — 연사 정보가 하나라도 있으면 아는 것만 채우고 모르는 칸은 대괄호, **전혀 없으면 빈 멘트**(쉬는 시간일 수 있다 — 추측 금지) + 비상 예비 멘트 3종(영상 장애 · 발표자 지연 · 음향 교체 — 일반 문장) |
| 진행 숫자 | 멘트 작성 = MC·의전 중 script가 있는 것 / 전체 · 연사 확인 대기 = 연사 자리가 남은 멘트 · 세션 = 블록이 걸린 프로그램표 세션 수 |

**운영 보드 문서 요약** — 카드 굵은 줄 = 큐시트 `큐 n개`(아래 = 큐가 가장 많은 문서의 첫–마지막 시각 · 대본 a/b) · 시나리오 `멘트 n/m 작성`(세션 · 연사 확인 대기 · 비상 멘트) · 운영가이드 `섹션 n/m 채움`(현장 인력 · 무전 · 원본 바뀜) · 기타 `확정 n/m`(컨펌대기 · 파일 없음). 표 칸은 유형마다 다르다(§10.2). 모두 읽기만 한다(R-O1). 인라인 빌더를 닫으면 숫자를 다시 읽는다.

**큐시트 칸 이름** — 시각 · 큐 · 구분 · MC·진행 · 조명 · 영상 · 음향(현장 큐시트 표기 순서). 데이터 칸(`console_light`·`console_screen`·`console_audio`)과 내보내기 규칙(§23.3 M→음향 · C→조명 · V→영상)은 그대로 — 편집 표·입력 폼·운영계획서 03·발송 스냅숏이 같은 순서.

**문서 머리 통일** — 시나리오·운영가이드도 큐시트(PR-4b)처럼 항목 상세 머리 한 줄(담당 · 마감 · 최신 vN · 날짜)과 '다음 단계' 카드 한 줄(PM 발송 — 발송 때 원고/가이드 스냅숏이 새 버전)만. 빌더 안의 두 번째 머리("시나리오 — {문서명}" + 컨펌 발송)와 메타 스트립은 걷었다. 빌더 위에는 요약 줄만(시나리오 = 멘트 n/m · 큐시트로 보내기 · 인쇄 · MC 배포용 / 운영가이드 = 섹션 n/m 채움 · 비어 있는 섹션 · 원본 바뀜 · 연락망 포함 · 인쇄 · 스태프 배포용).

**이탈** — 시안 ②의 '뒤 세션 접기'(여러 세션을 한 줄로 접는 단추)는 넣지 않았다 — 왼쪽 세션 목록이 이동을 맡는다. 시안의 '큐 Q-01' 칩은 큐시트 큐 번호 대신 대본 속 큐 표기(M-02 · C-11 — §23.3)를 모아 보인다(원고와 큐시트는 독립 편집이라 번호 연결이 없다). 블록 구분의 세부 이름(MC · 소개 · MC · 안내)은 저장할 칸이 없어 구분 이름만.

**구현** — SQL `20260926000300_scenario_emergency.sql`(kind CHECK 재정의 — 표·함수 불변) · 순수 함수 `src/lib/scenarioScript.ts`(멘트 판정 · 연사 자리 · 뼈대 · 진행 숫자 — provider 2종과 빌더가 공용) · `src/components/board/opsDocMetrics.ts`·`OpsDocTable.tsx` · `src/components/scenario/`(ScenarioBuilder · ScenarioScriptBlock · ScenarioBlockForm · scenarioGroups). 증명 = DoD 81.

### 23.7 16:9 장표형 운영계획서 (v2.13.2 · Phase 3.24 PR-C · 2026-09-26)

**계기** — 사용자가 실물 운영계획서(44장 · 16:9 · 7장 + 별첨)를 "이 정도 수준"으로 제시 → 범위 게이트에서 출력 = **16:9 장표형 인쇄** 선택(PPTX 내보내기는 라이브러리 추가라 보류) → 문서 3종 실무화(§23.5·§23.6) 뒤 "다음스텝 알아서 진행".

**원칙** — ① 새 입력 칸을 만들지 않는다: 운영계획서(S9)와 **같은 조립 데이터**(`getPlan`)만 읽는다. ② 없는 것을 지어내지 않는다: 채우지 않은 표는 장표에 싣지 않고 도구 줄의 '싣지 못한 표'에 어디서 채우는지만 알린다. 내용이 하나도 없는 장은 빈 장표 한 장('아직 채운 내용이 없습니다' + 채우는 곳)으로 남아 목차에서 빠진 장이 보인다. ③ 금액·개인 연락처는 없다(PlanData에 애초에 없다 — #RULE-NO-PRICE-TO-CLIENT · R-O6).

**장 구성**(실물 7장 구조를 앱 데이터에 맞춰 옮김)

| 장 | 장표 | 원천 |
|---|---|---|
| 표지 | 운영계획서 · 행사명 · 일시·장소 · D-day(출력 기준) · 주최·주관 · 작성 · 문서 버전 · 출력 | Project |
| 목차 | 8장 × (번호 · 이름 · 첫 장표 번호 · 실린 장표 이름) | 조립 결과 |
| 운영 요약 | 핵심 숫자 6칸 — 행사일 · 예상 인원 · 프로그램(세션 수 · 첫–끝 시각) · 현장 인력(합계 · 역할 · 무전 채널) · 제작물(확정 n/m · 규격 입력) · 등록(등록수 · 보장 대비 · 체크인) | Project · 프로그램표 · 운영가이드 인력·무전 · 제작물 · 등록 통계 |
| 01 행사 개요 | 사실 칸(행사명·일자·시간·장소·인원·좌석·주제·주최·사회·대상) + 개요 항목 | 행사 설정 개요 |
| 02 제작물 | 제작물 리스트(카테고리 · 품명 · 규격 · 수량 · 위치 · 종류 · 시안 · 상태) | 디자인 항목 가이드 스펙 |
| 03 공간 · 설치 | 설치·철거 일정(+ 시설 규정 메모) · 존별 운영(운영가이드 존 섹션이 있으면 그것만 — S9 04와 같은 규칙) | 운영가이드 setup·zone / 존운영 항목 |
| 04 프로그램 | 프로그램표 · D-day 진행표(사전 준비·본행사·마무리 구분 줄) · 구간별 체크리스트(카드 3열) · 큐시트(시각 · 큐 · 구분 · MC·진행 · 조명 · 영상 · 음향) | 프로그램표 · 운영가이드 dayplan·checklists · 첫 큐시트 |
| 05 참가자 | 등록 운영(등록 현황 3칸 + 접수 처리 용량 3칸 — 라인 × 60 ÷ 1인 처리 초 · 피크 대기) · VIP 의전 | 등록 통계 · 운영가이드 registration·vip |
| 06 조직 · 인력 | 현장 인력·콜타임(합계) · 무전·지휘 체계(채널 표 + 지휘 흐름 + 변경 규칙) · 역할 분담(●/○/— 범례) · 옛 역할별 체크리스트(있을 때) · 추진 일정(마일스톤) | 운영가이드 staffing·radio·raci·role · 마일스톤 |
| 07 안전 · 비상 | 안전관리 · 비상 대응(표 — 경고 색 머리 / 옛 문서는 글) | 운영가이드 safety·emergency |

**치수·쪽 나누기** — 장표 = 1280×720 CSS px(338.67mm×190.5mm). 틀 = 오렌지 헤어라인 4 · 머리 줄 52(행사명 · 장) · 제목 76(장 번호 · 제목 · 보조 줄 · n/m) · 본문 · 꼬리 줄 48(버전 · 출력 · n/N). 본문 예산 504px(실제 524 − 여유 20). 표 행 높이 = 16 + 20 × 줄 수(칸 폭과 글자 폭 어림 — 한글 1em · 숫자·영문 0.6em), 글 줄 25px. 넘치면 다음 장으로 — **장마다 고르게**(한두 줄만 넘기지 않음 · 장 수는 늘리지 않음) · 구분 줄·소제목은 다음 줄과 같은 장 · 묶음 한가운데서 넘어가면 '(이어서)' 구분 줄 · 마지막 행은 표 아래 메모와 같은 장. 어림이 틀리면 넘칠 수 있어 실브라우저에서 모든 장의 본문 넘침 0을 확인한다(데모 스모크 ③).

**인쇄** — 이름 붙은 쪽 `@page deck { size: 338.67mm 190.5mm; margin: 0 }` + 장표 틀마다 `page: deck` · 쪽 넘김 — A4 운영계획서(S9)의 `@page` 규칙과 섞이지 않는다. 화면 배율(zoom)·그림자·둥근 모서리는 인쇄에서 뺀다 · 색은 그대로(`print-color-adjust: exact`). 탭 제목 = `{행사명} 운영계획서 (16:9)`(PDF 기본 파일 이름). 크롬·엣지에서 용지가 16:9로 잡힌다(검증 = 헤드리스 크로미엄 PDF 960×540pt).

**데이터** — `PlanData.guide`(v15.4 필드만) = guide_zone·emergency와 **같은 항목**(빌더 데이터를 가진 첫 운영가이드)의 섹션 전부, 연락망(contacts) 제외. provider 2종이 같은 규칙.

**다음 단계(입력 칸 필요 — 설계 개정 동반)** — 01 목표·지난 행사 개선 과제·성공 기준 · 07 확인 요청(결정 시점 — 발주처 `/c`에서 답하게 할지) · 별첨 계약 범위 대응표(사양만 — 금액 비노출) · 03 공간 제원·장비 사양 표 · MC 원고(시나리오) 별첨 · 도면·연출 이미지 칸.

**구현** — 순수 함수 `src/components/plan/deck/planDeck.ts`(장 조립 · 쪽 나누기 · 목차 · 싣지 못한 표) · 부품 `DeckSlideView.tsx` · 화면 `src/pages/PlanDeckPage.tsx`(라우트 `/plan/deck` — InternalLayout 밖 · OnboardingGuard 안) · 인쇄 규칙 `src/index.css` · S9 발행 줄 '16:9 장표' 링크. 증명 = DoD 82.
...
**보류**(2026-08-29, 디자인 폴리싱·PSA 선행 후 재논의) — DMS 실전 투입 전에 다시 꺼낸다.
