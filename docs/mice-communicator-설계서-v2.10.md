...
| S1 | 홈 대시보드 (`/home` — 2026-09-04 루트가 S-00 런처가 되며 이동. 사이드바 "홈"·행사 목록/견적의 "행사로 이동"이 전부 `/home`) | 미결 컨펌(기한순) · D-day·마일스톤 · **지연/임박 WBS 태스크(v1.4)** · 미등록 인박스 · 영역별 진행률 바 · 최근 활동 | 인박스 연결/무시, 항목·태스크 바로가기 |
...
| S6 | 행사 설정 (v1.5 재정의 — 메뉴 2번째 상시 노출) | 탭 ①행사개요: 행사명·코드·유형·시작/종료일·시작/종료 시간·장소·예상 인원·좌석 형태·주제·주최/주관·사회자·참가 대상·기타 개요 항목(overview_items) + **(v2.0) 모객형 전용 그룹: 보장 인원·쇼업 KPI·타겟팅 5축 칩, 연결 견적 링크("견적 v3 확정 기준")** — **행사개요의 단일 원천(S9 ①은 여기서 읽기 조립, 인라인 편집 제거)** / 탭 ②담당자: **(v2.9.1) 역할 칸 4개(배정 현황 — 표 대체, 칸마다 이름·직함·이메일·전화·빼기) + 주소록 인물 카드(누르기 → 역할 버튼 · 역할 칸으로 끌어놓기)** + 직접 입력·전자명함(역할 변경 = 빼기 후 재배정) + 발주처 담당자·토큰 통합 표(발급·회수·링크 복사) + R&R 미리보기 / 탭 ③유형·연동: 유형 토글 안내·Drive·Slack | pm 편집(타 역할 읽기). 상단에 "세팅 완료·일자" 또는 "세팅 미완료(필수 n개)" 뱃지 |
