...
| S6 | 행사 설정 (v1.5 재정의 — 메뉴 2번째 상시 노출) | 탭 ①행사개요: 행사명·코드·유형·시작/종료일·시작/종료 시간·장소·예상 인원·좌석 형태·주제·주최/주관·사회자·참가 대상·기타 개요 항목(overview_items) + **(v2.0) 모객형 전용 그룹: 보장 인원·쇼업 KPI·타겟팅 5축 칩, 연결 견적 링크("견적 v3 확정 기준")** — **행사개요의 단일 원천(S9 ①은 여기서 읽기 조립, 인라인 편집 제거)** / 탭 ②담당자: **(v2.9.1) 역할 칸 4개(배정 현황 — 표 대체, 칸마다 이름·직함·이메일·전화·빼기) + 주소록 인물 카드(누르기 → 역할 버튼 · 역할 칸으로 끌어놓기)** + 직접 입력·전자명함(역할 변경 = 빼기 후 재배정) + 발주처 담당자·토큰 통합 표(발급·회수·링크 복사) + R&R 미리보기 / 탭 ③유형·연동: 유형 토글 안내·Drive·Slack(**v2.10.1 Slack 카드** — 행사 채널 등록·바꾸기·해제·테스트 보내기(pm), 저장 뒤에는 끝 토큰을 가려 표시, 상태 칩 = 행사 채널·공용 채널·꺼짐, 크론 미설정 안내, mock은 발송 없음을 명시 · **(v2.12) 행사 스레드 링크가 먼저 — 등록하면 채널·'Slack에서 열기'를 보이고 웹훅은 예비로 접힌다 · 상태 칩 = 행사 스레드·봇 준비 안 됨·행사 채널·공용 채널·꺼짐**) · **(v2.11.3 — Phase 3.23 PR-8, 디자인지시서 v1.4 §7-2.12) 화면 재배치**: 탭 이름 = 개요 · 담당자 · 유형·연동(번호 없음) · 탭 줄 오른쪽 필수 요약('필수 4개 모두 입력됨' / '필수 n개 남음 — 칸 이름' — 옛 상단 체크 스트립 대체) · 개요 = 섹션 목록(기본 정보 · 일정·장소 · 내용 · 모객 설정) + 섹션 카드 · 바꾼 칸 = accent 테두리 + '원래 …' · **고정 저장 바**(바꾼 칸이 있을 때만 — 개수·칸 이름 · 변경 취소 · 저장) → '저장됨 HH:mm' · 행사 포맷은 읽기 전용(전환은 온보딩 ③ — WBS 재전개 동반) | pm 편집(타 역할 읽기). 상단에 "세팅 완료·일자" 또는 "세팅 미완료(필수 n개)" 뱃지 |
