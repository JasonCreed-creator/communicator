...
- **(v2.12) 봇 전환 + 의뢰 확인 버튼(Phase 6.1 — 사용자 결정 2026-09-25~26)** — 웹훅(채널 글)은 받는 사람 폰에 푸시가 가지 않고(Slack 기본 알림 = DM·멘션·키워드만) 사람을 지정할 수 없다 → **봇**(같은 Slack 앱에 권한 추가). **DM은 쓰지 않는다**(회사 운영 방식 — 채널에서 모두가 보도록) — 행사마다 **채널의 스레드 하나**가 행사 기록이고, 봇은 모든 글을 그 스레드의 답글로 남기며 할 일이 생긴 사람만 **@멘션**한다(멘션 = 그 사람 폰 푸시).
  - **보낼 곳** = 행사 스레드(서버 `SLACK_BOT_TOKEN` + `projects.slack_thread_url` — 행사 설정 ③에 Slack '링크 복사' 주소, pm) → 행사 웹훅(예비) → 공용 웹훅 → 없음. 스레드 링크 = `https://{워크스페이스}.slack.com/archives/{C|G…}/p{16자리}`(답글 링크면 `thread_ts`가 스레드) 또는 `app.slack.com/client/…/thread/{채널}-{ts}` — DM 채널(D)은 받지 않는다(`src/lib/slackThread.ts`, 앱·서버 공용). 비공개 채널은 봇 초대(`/invite @micecommunicator`)가 먼저.
