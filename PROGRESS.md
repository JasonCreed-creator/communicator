...
- **완료: Phase 4.2.3 — 견적서 실직인 반영 + 브랜드 자산 판정 결함 수정**(2026-09-24, 사용자 지시 "견적서 출력 시 직인란에 이거 삽입해줘(누끼따야함)").
  ...
  **LibreOffice 실렌더**(국·영문, 인쇄 지름 11.6mm) · deploy:check · demo 4단 · 스모크 ③-2(내려받은 Excel 미디어 = 로고 1장·직인 0·비PNG 0) · tsc · build.
  ...
  국문 = 로고+직인·"(인)" · 영문 = 로고만·"(Seal)" 0건(주소 전체 노출) 확인. **머지 = 사용자 지시("머지 pr 모두 진행시켜")**
...
- **(Phase 4.2.3) 날짜 드리프트 테스트 3건** — `align-schedule` ④ · `dod13-wbs-view` 간트 · `dod14-wbs-urgency` 홈 큐가 2026-09-24 기준 origin/main에서도 실패
  (샘플 행사 WBS가 오늘 날짜로 '지연'이 됨). partner-board(2026-09-04)·partner-portal(2026-09-10)과 같은 처방(시계를 픽스처 가정일에 고정) 대상
...
- **(2026-09-24) Phase 4.2.3 이후**: ① ~~PR 검수 → 머지~~ → 사용자 지시로 머지(머지 = Vercel 프로덕션 자동 배포 — 운영 직인 경로가 PNG로 서빙되는지 실주소 확인)
  ② ~~영문 직인 겹침 조정 여부~~ → 영문 미삽입으로 종결 ③ 날짜 드리프트 3건 정리(후속 PR — 사용자 지시) ④ 사용자 PC Excel에서 직인 1회 확인
