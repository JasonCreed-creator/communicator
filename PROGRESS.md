...
- **(2026-09-24) Drive 실서버 작동 테스트 — [B] 3턴 승인, 새 세션에서 진행**: GCP = **회사 조직 프로젝트**(동의 화면 Internal). 1턴 = Supabase dev 3키 + 임시 PAT →
  setup 재실행(Drive 마이그레이션 포함) → `supabase:verify`(Phase 5 코드로는 **첫 실행**) → `reset-demo` 미리보기 → 담당자 등록·권한 / 2턴 = Vercel env 전환·매직링크 로그인 /
  3턴 = GCP OAuth 클라이언트 → Drive 연결 → 실사용(supabase/README §3d). 새 세션인 이유: 사용자가 Slack 플러그인을 이 세션 시작 뒤에 켜서(플러그인·커넥터는 세션 시작 때 로드).
- **(2026-09-24) 실제 담당자 등록 대기** — MICE팀 4명 + BD팀 3명(junior 1명은 이름·이메일 확정 전 보류). 연락처는 새 세션에서 Slack 프로필 조회.
  **실명·연락처는 DB에만**(공개 레포 — 코드·문서·PR·테스트 금지, 테스트는 가상 인물). 권한: 팀장 admin · PM 3명 sales · BD 3명 staff. 직함 칸 = "직함 · 담당".
...
## 4. 다음 스텝
- **(2026-09-24) Phase 3.22 이후**: ① PR(Phase 3.22) 검수 → 머지 ② **새 세션**(Slack 플러그인 로드): 담당자 7명 Slack 조회 → 작동 테스트 3턴(실서버 전환 · 담당자 등록·권한 ·
  Drive 연결·실사용) ③ 회사 도메인 사본 처리 방향 결정(미결)
