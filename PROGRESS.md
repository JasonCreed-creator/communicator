...
- **진행: Phase 6.1 Slack 봇 전환 + 의뢰 확인 버튼 — 2턴 구현 PR**(2026-09-26, 새 챗 — 사용자 "착수 · 1턴 = 메시지 시안 3장부터" → 시안 v1(DM) → 답변 3건 + **"DM 지양 — 채널에서 멘션"** → 시안 v2(채널·멘션)
  ...
  **남은 것(3턴 — 기획자님)**: 실DB setup 적용(머지 전) → 머지·재배포 → 행사 스레드 첫 글 링크를 행사 설정 ③에 붙이기(비공개 채널이면 `/invite @micecommunicator` 먼저) → '테스트 보내기' → 새 지시 1건으로 카드·버튼 확인.
...
- **(2026-09-26) Phase 6.1 — 2턴 PR 검증 → 실DB setup → 머지 → 3턴 실발송**: ① 실DB에 setup.sql 먼저(22번째 마이그레이션 — 표 1·열 2·함수 재정의, 행 수 불변 확인) ② PR 머지 → 운영 재배포(`SLACK_BOT_TOKEN`·`SLACK_SIGNING_SECRET`은 이미 운영 env)
  ③ `/api/notify` GET = `{bot:true}` 확인 ④ 기획자님: 행사 스레드 첫 글 ⋯ → 링크 복사 → 행사 설정 ③ '스레드 등록' → '테스트 보내기'(비공개 채널이면 먼저 `/invite @micecommunicator`)
