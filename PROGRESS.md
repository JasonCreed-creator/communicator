...
- **진행 중: Phase 3.12 — 데모 픽스처 리빌드화** (브랜치 `claude/gg-5jaapu`, base=main `bfccb26`).
  9/1 합류 팀원이 자기가 운영한 실제 행사(RE:BUILD 26)를 데모에서 그대로 보게 하는 데이터 전용 증분.
  **기능·스키마·타입·DataProvider 무변경 — 픽스처와 데모 기본 선택만.** 결과: vitest **322개**
  (기준 312 + DoD-26 10) · tsc(앱·데모) 클린 · vite build 성공 · `npm run demo` 4단 전부 통과
  (정적 · 라우팅 4 · 브라우저 15항목) · 금지 문자열 가드 0건 · 스크린샷 6장
...
- **Phase 3.12 — 데모 픽스처 리빌드화** (2026-08-22, 메인 단독): 기존 4행사 픽스처는 **비파괴**로 두고
  ...
  - ⑥ `prj-rebuild27` RE:BUILD 27 (진행 중 · **데모 기본 선택**, 행사일 2026-09-24는 가안):
    WBS 모객형 37건 전개 · 지연 2·임박 1 고정 · 제작물 8건(지시됨 3 · 초안 2 · 내부검토 1 · 컨펌대기 2) ·
    존 운영 1(가안) · 큐시트 1(지시됨) · 프로그램 4(가안) · 마일스톤 5 · RSVP 30 · 인박스 1 ·
    발주처 토큰 `rb27` · 견적 2버전(v1 proposed 800명 / v2 draft 700명, 베뉴 = venuedb `pie_factory`)
  ...
  - DoD-26 테스트 10건(`src/test/dod26-rebuild-fixtures.test.tsx`): (a) 종료 그룹 접힘·쓰기 409
    (b) 진행 중 첫 카드·WBS 37·지연 2·홈 집계 (c) S9 7섹션 실적 조립·견적 2버전 항등식
...
## 3. 미결
- **(열린 질문 — Phase 3.12 ①) RE:BUILD 27 견적 2건이 0원으로 산출된다.** 브리프 §2.6이 지정한
  800명·700명이 견적 엔진의 `TARGET_MAX`(500명)를 넘어 **별도 협의 모드(`isCustom`)** 로 떨어지고,
  엔진이 전 섹션 0원을 반환한다(설계상 정상 동작 — S-2가 안내 배너를 띄우고 Excel 내보내기는
  안내 문구로 거절). 브리프 §0.4 "금액은 엔진 산출값만"을 지키려면 이 결과를 그대로 저장하는 것이
  유일한 선택이라 그렇게 했다. **데모에서 금액이 보이길 원하면 두 가지 중 택1이 필요** —
  ⓐ 견적 인원을 500명 이하로 낮춰 저장 ⓑ 엔진 `TARGET_MAX` 상향(단가표 근거 필요, 별도 승인 사안)
- **(열린 질문 — Phase 3.12 ②) 참관객 레코드 703건 생성.** 브리프 §2.5는 "참관객 레코드는 20건만
  만들고 통계 카드가 703명을 보이도록 **집계 필드**를 맞춘다"고 했으나, §4-10 attendees에는 집계
  필드가 없고 통계는 레코드 수에서 파생된다(§0.5가 스키마 변경을 금지). 두 요구가 동시 충족 불가라
  **"개인정보 없이 집계 재현"을 우선**해 합성 레코드 703건을 코드로 생성했다(`참관객 0001` /
  `guest{n}@example.com` / org·phone null / 그룹은 badge_no 접두어로만 구분). 20건만 두는 쪽을
  원하면 통계 카드는 20명으로 표시된다 — 되돌리는 것은 버킷 수치 한 줄 수정
- **(열린 질문 — Phase 3.12 ③) RE:BUILD 27 WBS 1단계 시드가 브리프와 다르다.** 브리프 §1은
  "1.x 전부 done / 2.2·2.3을 지연으로"였지만, §15 템플릿 오프셋상 1단계는 D-42~D-28 =
  2026-08-13~08-27로 **오늘(2026-08-22) 기준 진행 구간**이고 마감이 지난 태스크는 1.1·1.2뿐이라
  그 조합으로는 지연이 0건이 된다(= §5 ② 테스트 불성립). 그래서 prj-partner-day와 같은 방식으로
  **"마감 지난 태스크는 완료, 단 가장 최근 2건만 미완료"** 규칙을 써 조회 시점과 무관하게 지연 2건을
  고정했다 — **9/2 이후에는 이 2건이 브리프가 지목한 2.2·2.3과 정확히 일치한다**. 1.3은 오늘 마감이라
  '진행 중'으로 두어 임박 1건도 함께 보이게 했다
- (관찰 — 기존 동작, 이번 세션 수정 안 함) **종료 행사의 S2 보드에도 '새 항목 생성'·'지시 발행' 폼이
  보인다.** 제출하면 provider가 409로 막지만 폼 자체가 숨지 않는다. 기존 종료 픽스처(④ AI 서밋)는
  산출물이 0건이라 드러나지 않던 것이 RE:BUILD 26으로 처음 보이게 됐다. 데이터 전용 증분 원칙(§0.5)에
  따라 손대지 않음 — 다음 UI 폴리시 때 `closed`면 폼 숨김 처리 검토
...
## 4. 다음 스텝
- **Phase 3.12 PR 챗 검수 → 머지** (열린 질문 ①~③ 판단 필요) → 머지 후 데모 아티팩트 재발행
...
- 2026-08-22 세션 #5: **Phase 3.12 데모 픽스처 리빌드화** (메인 단독, 브랜치 `claude/gg-5jaapu`,
  ...
  **RE:BUILD 26 운영계획서·결과보고서 PDF 2부를 추가 첨부**해, 브리프의 요약본 대신 원문에서
  ...
  스크린샷 6장. PR 발행(드래프트) → **머지는 사용자 검수 후**(열린 질문 ①~③ 판단 필요)
