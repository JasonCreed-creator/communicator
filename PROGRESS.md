...
- **완료: Phase 4.2.3 — 견적서 실직인 반영 + 브랜드 자산 판정 결함 수정**(2026-09-24, 사용자 지시 "견적서 출력 시 직인란에 이거 삽입해줘(누끼따야함)").
  ...
  **남은 것**: 영문 견적서는 주소("…Seoul, Korea")가 길어 직인 테두리가 끝 글자에 살짝 걸친다(60px도 동일) — 조정 여부 사용자 확인 대기(§3)
...
## 3. 미결
- **(Phase 4.2.3) 영문 견적서 직인 겹침** — 영문 주소("134 Teheran-ro, Gangnam-gu, Seoul, Korea")가 G:H에 걸쳐 길어 72px 직인 테두리가 끝 글자
  ("a")에 살짝 걸친다(LibreOffice 렌더 기준, 60px도 겹침). 국문은 주소가 짧아 겹침 없음. 조정안(영문만 직인을 H칸 우측 정렬·영문 주소 줄바꿈 등)은 사용자 확인 후
...
## 4. 다음 스텝
- **(2026-09-24) Phase 4.2.3 이후**: ① PR 검수 → 머지하면 운영 견적서에 직인이 찍힌다(머지 = Vercel 프로덕션 자동 배포) ② 영문 직인 겹침 조정 여부(§3)
  ③ 날짜 드리프트 3건 정리(별도 작업) ④ 사용자 PC Excel에서 직인 1회 확인
