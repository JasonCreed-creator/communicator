...
## 1. 상태 요약
- **진행: Phase 3.24 운영 문서 3종 실무화 — PR-A 운영가이드 현장 운영 섹션**(2026-09-26, 같은 챗 — 사용자 지적 "운영보드 네 항목의 세부내용 포맷이 다 똑같아" + 실물 운영계획서(44장 PPTX) 제시 "이 정도 수준으로 만드는 건 힘들까?").
  1턴 = 비교·판단(가능 — 표·숫자·목록은 앱이 채우고 판단 문장·도면·연출 이미지는 사람 몫, 자동화 7:3 **가정**) + 범위 버튼(**문서 3종 실무화부터** · 출력 **16:9 장표형 인쇄** · **[B] 이 챗 N턴**) + 시나리오 **멘트 원고형** · 가이드 섹션 **현장 운영 전부**(사용자 선택).
  2턴 = 캔버스 시안 5장 "운영 문서 3종 실무화"(비공개 — https://claude.ai/artifact/A114HQxioV6PcRDF7wMww9 · 가상 행사명·앱 예시 인물만, 올려 주신 PPT는 구조만 참고·레포 밖) → **승인 → PR-A 착수**.
  ① 운영가이드 = **현장 운영 12섹션**(설치·철거 · 인력·콜타임 · 무전·지휘 · 역할 분담 ●/○ · D-day 진행표 · 구간별 체크리스트 · 등록 운영 · VIP 의전 · 안전관리 = 표 섹션 + 존별 운영 · 비상 대응 · 연락망) — 새 문서 시드는 행사 일시·장소·프로그램표·담당자 수로 뼈대.
  ② `guide_sections.data` jsonb + kind 9종 · 표 섹션은 저장 때 **content를 data에서 다시 만든다**(S9 ⑦·스냅숏·인쇄 무변경) · 모양 검사 이중(provider 400 + DB CHECK) · 옛 문서는 그대로, **'뼈대 추가'**로 빠진 섹션만 정본 자리에(기존 순서·id·내용·stale 불변).
  ③ SQL 23번째 `20260926000200_guide_structured.sql` · 순수 함수 `src/lib/guideStructured.ts` · 화면 `src/components/guide/`(섹션 목록 + 카드 + 표 읽기·고치기) · DataProvider **v15.2**(필드만 — 131 불변) · 설계서 **v2.13**(§23.5 · 파일명 v2.13) · 디자인지시서 §7-2.13 · DoD 80.
  ④ 검증: vitest 전체(남은 실패 1 = LibreOffice 재계산 — 컨테이너 환경, main에서도 같음) · 로컬 Postgres 197/197(운영가이드 구조화 7 추가 — 컨테이너 재시작으로 `/tmp/pg-communicator` 다시 띄움) · tsc · build · deploy:check · demo 4단 · 스모크 ③(뼈대 추가 → 섹션 목록 → 등록 대기 계산 → 목록 링크 = 해시 불변).
  스모크에서 찾아 고친 결함: 섹션 목록 링크가 `#섹션` 주소라 **해시 라우팅 데모에서 화면이 이동** → 스크롤만 하도록(테스트 (i)). 이탈 1: 시안의 '연락망 전화번호는 인쇄에만' = R-O6 충돌로 미구현.
  **머지 전 필요: 운영 DB에 setup.sql(23번째 마이그레이션 — 열 1 · CHECK 2 · 함수 재정의 1, 행 수 불변)**. 다음 = PR-B(시나리오 멘트 원고형 · 보드 카드 요약 · 큐시트 칸 이름 · 머리 통일) → 운영계획서 16:9 장표.
- **완료: Phase 6.1 Slack 봇 전환 + 의뢰 확인 버튼**(2026-09-26 — PR #65 머지 `a7f2333` · 운영 DB 22번째 마이그레이션 적용을 공개 키 조회로 확인(새 표·함수 3개 존재, 공개 키 실행 거부) · 운영 `/api/notify` = `{bot:true}` · 서명 없는 버튼 요청 401 · **기획자님 실발송 확인 "완벽해!"**(행사 스레드 등록 → 테스트 → 제작 요청 카드 → 확인했어요)). 아래는 2턴 당시 기록.
- **이전: Phase 6.1 Slack 봇 전환 + 의뢰 확인 버튼 — 2턴 구현 PR**(2026-09-26, 새 챗 — 사용자 "착수 · 1턴 = 메시지 시안 3장부터" → 시안 v1(DM) → 답변 3건 + **"DM 지양 — 채널에서 멘션"** → 시안 v2(채널·멘션)
  → "진행해 — 앞으로 채널의 스레드를 통해 하나의 프로젝트로 운영, 프로젝트마다 스레드 주소 설정이 선행"). 결정은 §5 첫 항목.
  ① **보낼 곳** = 행사 스레드(봇 — 행사 설정 ③에 Slack '링크 복사' 주소) → 행사 웹훅(예비) → 공용 웹훅 → 없음. **DM 0** — 봇의 모든 글은 그 스레드 답글, 할 일이 생긴 사람만 @멘션.
  ② **의뢰 카드** = 제작 요청(담당자)·검토 요청/파트너 제출(PM) + '확인했어요'(멘션된 사람만 셈 → 카드가 "✓ 이름 확인함 · 시각"으로 바뀜 · 다른 사람은 나만 보기 안내) · 24시간 미확인 = 한 번 멘션 답글.
  ③ SQL 22번째 `20260926000100_slack_bot.sql`(스레드 링크 · Slack ID · `request_acks` · 선점·리마인드 재정의 · service RPC 3) · 서버 `api/_lib/notify/{slack,cards,interact}.ts` + **`api/slack-interact.ts`**(Slack 앱에 등록된 Request URL 그대로) ·
  앱 = 설정 ③ 스레드 카드 · S-13 Slack 칸 · 보드·항목 상세 확인 한 줄. DataProvider **v15.1**(필드만 — 131 불변). 설계서 **v2.12**(파일명도 v2.12) · DoD 79.
  ④ 검증: vitest 1446/1447(남은 1 = LibreOffice 재계산 — 이 컨테이너의 soffice 환경, main에서도 같은 실패) · 로컬 Postgres 190/190(Slack 봇 15 추가) · tsc(앱·데모) · build · deploy:check(실브라우저 포함) ·
  demo 4단 · 스모크 ③(Slack 스레드 카드·담당자 Slack 칸 — playwright 1.56.1 임시 설치, 컨테이너 chromium-1194와 맞춤) · grep 가드.
  **머지는 대기** — 실DB setup.sql이 먼저여야 한다(담당자 고치기의 `slack_user_id` 열 · 설정 ③ 스레드 저장). 이 컨테이너에는 Management API 토큰·DB 접속이 없어 적용하지 못했다.
  (코드가 먼저 배포돼도 알림 서버의 행사 조회·항목 상세는 옛 DB에서 그대로 동작하게 해 뒀다 — 새 칸 저장만 실패한다.)
  **남은 것(3턴 — 기획자님)**: 실DB setup 적용(머지 전) → 머지·재배포 → 행사 스레드 첫 글 링크를 행사 설정 ③에 붙이기(비공개 채널이면 먼저 채널 세부정보 → 에이전트 및 앱 → 앱 추가) → '테스트 보내기' → 새 지시 1건으로 카드·버튼 확인.
  **이 컨테이너에는 `.env.local`이 없다**(새 세션 — Slack 토큰은 Vercel 운영 env에만). 코드 검증은 가짜 Slack으로 했고 실발송은 운영에서.
- **이전: Phase 6.1 Slack 봇 전환 + 의뢰 확인 버튼 — 새 챗에서 착수(사용자 결정 2026-09-25 밤, 범위 [C])**. 결정 3건은 §5 첫 항목, 착수 계획은 §4 첫 항목.
...
- **완료: Phase 3.23 UX 개편 적용 — PR 8개 연속(범위 [A]) 전부 머지**(기반·홈·디자인 보드·항목 상세·큐시트·행사 목록·견적·정산보드·행사 설정·온보딩 — 마지막 = PR #61 `a90d984`, 운영 반영 확인)(2026-09-25, 사용자 지시 "페이지 UI UX를 클로드디자인을 통해서 업그레이드
  ...
  ② **디자인 트랙(Claude Design — 비공개 아티팩트 2개, 레포 밖)**: 1턴 = 21화면 캡처 + 진단(공통 결함 8건: 다음 행동 분산 · 빨강·주황 과다 · 개발 용어·화면 코드 ·
  D+/D- 혼동 · 채운 버튼 과다 · 폼 길이·폭 · 중복·막다른 카드 · 이모지) → 2턴 = 디자인 시스템 "커뮤니케이터 웜 페이퍼"(앱 토큰 그대로) + 캔버스 "커뮤니케이터 UX 개편"
  (현재 10 · 개선 9 · 공용 사이드바 — 결정: 8화면 · 앱 토큰으로 새로 · D-n/'n일 지남' · '오늘 할 일' 한 목록) → 사용자 "비주얼 미리보기가 좀 더 컸으면" → 항목 상세
  미리보기를 본문 폭 16:9로 → 3턴 = **실제 렌더 검수**(타입의 런타임 + 로컬 React UMD로 오프라인 렌더 — 표 안 반복 행이 파서에 밀려 비는 결함(게시된 홈 포함) · 한글
  음절 줄바꿈 · 가짜 입력 요소 · 체크 표시 · 메뉴 가림 · 표 열 어긋남 · 코멘트 좌측 줄 장식 → 전부 수정) + **디자인 보드 갤러리 보기**(사용자 선택) → v4 게시.
  대비 실측(흰 바탕 기준 4.5:1): ink-cap 3.62 · 빨강 배지 3.99 · 주황 버튼 흰 글자 3.07 → 사용자 결정 "둘 다 어둡게" · "바탕을 진한 주황으로" · **"바로 구현 시작"**.
  ③ **PR-1 기반**(디자인지시서 v1.4 §7-2 · DoD 70): `--ink-cap` #756f64 · `--negative` #c42f2f · `.btn-accent` 바탕 accent-deep · 흰 글자 주황 면 7곳 → accent-deep
  (외부 `/p` 헤더만 예외) · `word-break: keep-all` · 기한 표기 `dueLabel`(오늘·n일 지남)·`eventDayLabel` — 내부 8곳 적용, 발주처·운영계획서 `ddayLabel`·WBS 오프셋은 그대로 ·
  완료 태스크·마일스톤 기한 칸 `—` · 화면 코드 제거(페이지 머리 15곳 · 항목 경로 S3 · 견적 캡션 · 런처 칩 · 안내 문구 4곳). 테스트 4건 갱신 + DoD 70(20항목) ·
  스모크 ③ 교체(Phase 4.7 블록은 ③-이전으로). **머지**: [PR #53](https://github.com/JasonCreed-creator/communicator/pull/53) 미리보기 성공 → 머지 `31d570a` →
  운영 READY · 번들에 `word-break:keep-all` · `--ink-cap:#756f64` · '일 지남' 확인.
  ④ **PR-2 홈 '오늘 할 일'**(디자인지시서 v1.4 §7-2.5 · DoD 71): 요약 5칸(지연만 빨강 · 주최형은 파트너 검토 대기) + '오늘 할 일' 한 목록(출처 8종 급한 순 —
  지연 태스크·지난 마일스톤·컨펌대기·파트너 검토·정산 초과(버킷 이름만)·미등록 파일·받은 가이드·임박 · 칩 4 · 내 차례 판정 · 일정 행 8행 + 더 보기(넘겨받은 일은 늘 표시) · 미등록 파일은
  행 아래 연결·무시 · 일괄 리마인드·독촉·Drive 확인은 목록 머리 띠) + 다가오는 2주 · 영역별 확정 · 최근 활동. 옛 홈 전용 컴포넌트 3개 삭제. **기존 결함 수정**:
  인박스 '무시'가 성공해도 목록이 새로 고쳐지지 않았다(`dismissInboxFile`이 void → `useMutation.run`이 성공·실패 모두 undefined — 새 테스트가 잡음).
  테스트 7개 파일 갱신(align-home 재작성 · host-home-widget 재작성 · dod14·dod26·dod7·dod62·settlement-ui) + DoD 71(12항목).
  **머지**: [PR #54](https://github.com/JasonCreed-creator/communicator/pull/54) 미리보기 성공 → 머지 `ce7972d` → 운영 READY · 번들에 '오늘 할 일'·'다가오는 2주' 확인.
  ⑤ **PR-3 디자인 보드**(디자인지시서 v1.4 §7-2.6 · DoD 72): 카테고리 묶음 → 급한 순 한 표 6열(상태 · 항목(제목 + 카테고리·규격 한 줄) · 버전 · 담당 · 마감 · **다음 행동**)
  · 차례 칩(전체·우리 차례·발주처 차례(주최형 파트너 차례)·끝남) + 지연만 · 목록 ↔ 갤러리(최신 시안 16:9 썸네일 · 빈 자리 · 파일 표지, 보기 선택 기억) · 머리 채운 버튼
  `＋ 항목 추가` 1개 · 판정 한 곳 `designBoardRows.ts`. 운영 보드는 v2.5 유형 우선 그대로(`AreaBoardPage`가 영역으로 가른다). 공용화: `FilterChip`(홈 칩) ·
  `SegmentedToggle` · `waitingDays` → `lib/labels` · `activeClientLinks`. 상세가 `?upload=1`이면 업로드 카드로 초점. mock 자리표시 썸네일 웜 페이퍼 16:9·이름 이스케이프.
  렌더 실측 수정 3건(마감 칸 190 → 220 — 'nn일 지남' 알약이 잘림 · 규격 한 줄을 조각마다 자르던 것 → 끝에서 한 번 · 갤러리 중립 배지가 썸네일 면에 묻힘 → 흰 받침).
  테스트 3개 파일 갱신(align-board (a)(b)(d) · dod7 (a) 행 = tr · polish-3151 P6-⑥ 여러 행) + DoD 72(27항목).
  **머지**: [PR #55](https://github.com/JasonCreed-creator/communicator/pull/55) 미리보기 성공 → 머지 `5fd6f2f` → 운영 READY · 번들에 '급한 순 — 늦은 것, 우리 차례가 위로'·보기 기억 키 확인.
  ⑥ **PR-4 항목 상세**(디자인지시서 v1.4 §7-2.7 · DoD 73 — 큐시트는 범위가 커서 PR-4b로 나눔): 머리 = [최신본 내려받기][⋯ 메뉴](Phase 4.5 '항목 관리' 카드 대신 —
  설계서 §10 S3 v2.11.1 개정) · '다음 단계' 카드 1장(5단계 레일 · 상태 제목·설명 · 올리기·내부검토 요청·PM 반려/컨펌 발송 — 머리의 '새 버전 업로드'를 옮김, 막힌 상태면
  버튼 없이 잠금 캡션) · 본문 폭 16:9 큰 미리보기(사용자 요청 "비주얼 미리보기가 좀 더 컸으면") · 코멘트 공개 범위 토글(기본 내부 메모) · 오른쪽 = 버전 이력(누르면 미리보기·발송본) ·
  컨펌 기록 타임라인 · 제작 가이드(비면 pm 채우기). 부품 `components/item/` 5개로 분리(페이지 1,176 → 약 560줄). 숫자 조사 `objectParticle`(v1을·v2를 — 렌더에서 'v2을' 발견).
  버전 '최신' 흰 글자 주황 알약 → 의미 배지. 테스트 갱신 5개 파일(align-itemdetail 재작성 · dod66 ①′ · dod67 ④ 메뉴 · dod7 (c) · polish-3163) + DoD 73(12항목).
  **머지**: [PR #56](https://github.com/JasonCreed-creator/communicator/pull/56) 미리보기 성공 → 머지 `937fc69` → 운영 READY · 번들에 '발주처 답을 기다리는 중'·'항목 메뉴' 확인.
  ⑦ **PR-4b 큐시트**(디자인지시서 v1.4 §7-2.8 · DoD 74): 머리 메타 한 줄(스트립 없음) · 한 줄 다음 단계(PM = 답 기한·반려…·컨펌 발송 + 이메일 준비 중 안내) · 표 9칸(손잡이 ·
  큐 · 시간 · 구분 · 내용+대본 · 음향 · 조명 · 스크린 · ⋯) — 행 끝 메뉴(공용 `ActionMenu`) · **행 끌어 옮기기**(HTML5 DnD · 3.6c의 '드래그는 jsdom 불가'를 fireEvent 테스트 +
  스모크 실브라우저로 대체) · 표 맨 아래 `큐 추가`(다음 번호로 바로 생성·편집) · 대본 칸(고른 큐·모아 보기·편집) | 코멘트·컨펌 기록. 운영 보드 인라인 편집도 같은 표.
  렌더 실측 수정: 시간 칸 잘림(70 → 76 · 표 14px) · 표 정본 결함(머리 첫 칸만 흰 면 — 스티키 첫 열 inherit). 테스트: dod12 재작성 · polish-3151 (c) · align-itemdetail (f) + DoD 74(9항목).
  **머지**: [PR #57](https://github.com/JasonCreed-creator/communicator/pull/57) 미리보기 성공 → 머지 `11da6de` → 운영 READY · 번들에 큐 끌기 형식·'대본 모아 보기' 확인.
  ⑧ **PR-5 행사 목록**(디자인지시서 v1.4 §7-2.9 · DoD 75): 제목 `행사 목록`(사이드바와 같게) · 세 묶음 = 먼저 확인할 행사(세팅 미완료 줄 → 이어서 세팅하기 · 행사일 지난 진행 중 줄 →
  종료하기) · 진행 중 카드(행사일 가까운 순 · D-day dark 알약 · 확정 진행률 · 확인할 것 · PM · 지금 보는 행사 배지) · 종료된 행사(접힘). 카드 머리 버튼 줄(종료·삭제)을 ⋯ 메뉴로
  (행사 설정 열기 · 종료/재개 · admin 삭제…) — `ActionMenu`에 설명 줄·testId, 메뉴 안 누르기·Enter가 카드로 번지지 않게. 점선 새 행사 타일 제거. 테스트: align-settings (1)~(4) 재작성 ·
  dod58 메뉴 경로 · 제목 바뀐 4개 파일 + DoD 75(6항목).
  **머지**: [PR #58](https://github.com/JasonCreed-creator/communicator/pull/58) 미리보기 성공 → 머지 `5eb3aac` → 운영 READY · 번들에 '먼저 확인할 행사'·'행사일이 가까운 순'·'관리자만 · 되돌릴 수 없음' 확인.
  ⑨ **PR-6 견적 목록·옵션**(디자인지시서 v1.4 §7-2.10 · DoD 76): 목록 머리 버튼 5개 → 가져오기 + `＋ 새 견적` — Excel·구글 시트·**새 버전으로 고치기**는 요약 패널 아래(고른 견적에
  대한 동작) · 묶음 제목 = 행사명(`행사 연결 · 버전 n개`) / `행사 없이 견적만` · 표 6열 고정 폭(최신 위 · 정렬 화살표 퇴역) · 선 자물쇠(🔒 퇴역) · 이전 버전 대비 한 문장
  (`v2보다 1,750,000원 줄었습니다 (−1.3%)` + 바뀐 것 — 색 없음) · **뒤 버전이 있으면 고치기 비활성 + 이유**(전에는 에디터에서 저장할 때 409). 에디터 = 머리 `준비 · 견적` +
  견적서 언어 토글(`SegmentedToggle`) · 번호 단계 줄 · 옆 요약 320(30 합계 · 옵션으로 +n · 고른 옵션 · PCO 25% 안내). 옵션 = 체크박스/라디오 카드(`.ui-check`) · 막힌 옵션은
  비활성 + 이유 한 줄(전에는 누르면 알림) · 묶음 머리 `n개 고름 · 합계` · 미디어·포토월 선택 해제 · 부스 한 줄(− n ＋ · × 단가 · 소계 — `MoneyInput` 분리) · 이모지 → 선 아이콘.
  **옵션 금액은 엔진을 옵션 집합만 바꿔 돌려 읽는다**(`optionAmounts.ts` — 단가 재정의 0 · 엔진 파일 무접촉 · 합 = 엔진 옵션 합계를 테스트로 고정). `fmtMoney` 1만원 미만 원 단위
  ('0만원' 제거) · `subjectParticle`. 렌더 실측 수정 2건(미디어 3열에서 이름이 세 줄로 접힘 → 금액을 설명 아래로 · 폰에서 '구글 시트로 만들기'가 반 칸에 잘림 → 1열).
  테스트: align-landing-quote ⑦ · dod25 묶음 제목 + DoD 76(17항목) · 스모크 ③ 교체(견적 내보내기 블록의 '＋ 새 버전' → '새 버전으로 고치기', 제목 찾기 exact).
  **머지**: [PR #59](https://github.com/JasonCreed-creator/communicator/pull/59) 미리보기 성공 → 머지 `3240783` → 운영 READY · 번들에 '새 버전으로 고치기'·'행사 없이 견적만'·'견적서 언어'·'옵션으로 +' 확인.
  ⑩ **PR-7 정산보드**(디자인지시서 v1.4 §7-2.11 · DoD 77): 머리 = 제목 + '내부 전용' · [기준 견적 갱신] [**협력사 견적서 불러오기**](채운 — Phase 4.7의 아래 카드에서 올림 ·
  설계서 §10 S-10 v2.11.2) · 머리 아래 **할 일 알림**(확인 대기 견적서 → 확인하기 · 견적 초과 버킷마다 → 항목 보기 · 검산 어긋남) · KPI 4장에 '마진 구성 · 검산' 카드를 합침(검산 배지 ·
  구성 막대 주황 없음 · 참고 범위 안/밖) · 버킷 표 7열(정렬 화살표·'초과만 보기' 퇴역 · 펼침 단추 aria-expanded · 원가 없는 버킷은 그룹행 아래) · 발주 항목 7칸(역할 도트 · 상태 배지 ·
  ⋯ 메뉴 = 금액·상태 입력 · 항목 지우기) + 글자 단추 추가 · **초과 사유 = 항목 메모**(버킷엔 사유 칸 없음 — 스키마 불변) · 이력은 있을 때만. **기존 결함 수정**: 발주 항목을 지워도
  표가 새로 고쳐지지 않았다(`deleteSettlementItem` void → `useMutation.run` undefined → 보드 재조회 안 함 — 새 테스트가 잡음). 렌더 실측 수정 3건(합계 견적 칸 잘림 → 금액 칸 120 → 128 ·
  KPI 캡션이 숫자 가운데서 끊김 → 조각 단위 · 그 조각 처리가 구분자까지 묶어 375px에서 16px 가로 넘침 → 구분자를 밖으로 · 1024px에서 버킷 이름 '리…' → 표 최소 1010).
  테스트: align-settlement ①③⑤ 재작성 · settlement-ui 5건 · dod69 ④(머리 버튼) + DoD 77(12항목) · 스모크 ③ 교체(Phase 4.7 블록은 머리 버튼 기준).
  **머지**: [PR #60](https://github.com/JasonCreed-creator/communicator/pull/60) 미리보기 성공 → 머지 `c924f10` → 운영 READY · 번들에 '원가 없는 항목'·'검산 일치'·'초과는 막지 않아요'·'금액·상태 입력' 확인.
  ⑪ **PR-8 행사 설정·온보딩**(디자인지시서 v1.4 §7-2.12 · DoD 78 — 마지막 PR): 설정 탭 이름 번호 제거(`개요`·`담당자`·`유형·연동`) · 머리 배지 = 세팅 완료 positive / 미완료 attention ·
  옛 필수 4 체크 스트립 → 탭 줄 오른쪽 요약(`필수 n개 남음 — 칸 이름`) · 개요 = 섹션 목록(sticky) + 섹션 카드 4 · 바꾼 칸 = accent 테두리 + `원래 …`(칸 안내가 있으면 이어 붙임 — 유형을
  바꾸는 순간 WBS 안내가 사라지지 않게) · **고정 저장 바**(바꾼 칸이 있을 때만 — 개수·칸 이름 · 변경 취소 · 저장) → `저장됨 HH:mm` · 포맷 읽기 전용 · 행사 코드 힌트(오늘 날짜 파일 이름 예시).
  온보딩 = 카드 960 · 머리 줄 `단계마다 저장돼요` + `나중에 하기`(1단계에 저장 안 한 입력이 있으면 확인 → 행사 목록) · h1 = 단계 제목 · 가로 단계 줄 + `n단계 중 k단계 · 필수 4개 중 m개 입력`
  (1단계는 저장 전 입력을 센다) · 넓은 2열 + 선택 항목 6개 접기(값 있으면 펼침). 공용 `ProjectOverviewForm` 재작성(`layout`·`onDirtyChange`·`onRequiredFilledChange`·`nextHint`) · 설계서 §10 S0·S6 v2.11.3.
  **기존 결함 수정(새 스모크가 잡음)**: 세팅 미완료 행사를 보던 사람이 `?project=` 링크(Slack 알림 링크와 같은 경로)로 본체 화면에 들어가면 `OnboardingGuard`가 **이전 행사의 온보딩 조회 결과**로
  판정해 링크가 가리킨 화면 대신 행사 설정으로 보냈다 — ① 주소에 다른 행사의 `project`가 남아 있으면 판정을 미루고(전환 대기 — DoD 78 C1이 이 파일 전체 실행 순서에서 재현, 대기를 빼면 실패 확인)
  ② 결과에 행사 id를 붙여 현재 행사 것만 쓴다(전환은 그려졌는데 이전 조회가 늦게 온 경우 — 실브라우저 스모크가 재현). 테스트: 이름 바뀐 17개 파일 기계 치환 + align-settings (5) ·
  align-onboarding-portal ①② · dod11 · dod25 재작성 + DoD 78(14항목) · 스모크 ③ 교체(13항목 — 데모 안내 칩이 고정 저장 바 버튼을 가려 사용자처럼 닫고 시작 · Phase 3.22 블록의 탭 이름 exact).
  **머지**: [PR #61](https://github.com/JasonCreed-creator/communicator/pull/61) 미리보기 성공 → 머지 `a90d984` → 운영 READY · 번들에 '저장하지 않은 변경'·'단계마다 저장돼요'·'모두 입력됨'·
  '온보딩 3단계에서 정했습니다'·'개요 섹션' 확인, 옛 진행 문구('단계 완료 · 필수') 0.
- **이전: 남은 기능 3묶음 — 입력 없이 가능한 범위 소진, 남은 것은 기획자님 입력(§4 첫 항목)**(2026-09-25, 사용자 지시 "이미 등록한 항목을 수정/삭제하는 기능도 만들어야 함" ·
  "협력사 견적 불러오기 등 아직 구현 안 된 내용들 싹 진행시켜" → 버튼 "묶음 1→2→3 순서" · "검증 후 바로 머지" · 지우기 범위 "모든 상태 + 이름 입력 확인" →
  "니가 지금 내 입력 없이 수행 가능한 범위까지 쭉 진행해"). 묶음 1 = **Phase 4.5 항목 고치기·지우기** · 2 = **Phase 6 Slack 알림 + D-1 리마인드**(웹훅 없으면
  no-op — 실발송 시험은 웹훅 받은 뒤) · 3 = **Phase 4.7 협력사 견적서 불러오기**(가상 픽스처로 — 실서식 보정은 샘플 받은 뒤). 이번에 하지 않는 7건:
  이메일(Resend) · PDF·사진 OCR · GA Data API · 스냅숏 PDF · 발주처 `/c` 자료 올리기 · PSA·초청제·onsite 롤(열린 질문 — 추측 구현 금지) · Okta(맨 끝).
  ⑯ **3턴 완료 — Drive 연결(개인 계정)**: 사용자가 브라우저 확장(Claude in Chrome)으로 개인 GCP 프로젝트 등록(외부 · 프로덕션 게시 · 웹 클라이언트 ·
  리디렉션 = 운영 주소 `/api/drive` · 브랜딩에 홈·개인정보처리방침·도메인) → Code가 env 2개(클라이언트 ID = Config · secret = **Secret**) + Redeploy →
  `/api/drive` `configured:true` → 사용자 '연결하기'(개인 계정) → 행사 폴더 생성 → 업로드 → **Drive에 규약 이름 파일 확인**(`YYMMDD_코드_항목_v1.확장자`) ·
  `drive_enabled` 켜짐. 남은 것: 브랜딩이 가리키는 **개인정보처리방침 페이지(`/privacy`)가 아직 없다**(§3) · 실사용 1회(발주처 링크 → `/c` → 확정 사본).
  ⑰ **Phase 4.5 — 항목 고치기·지우기**(설계서 v2.10 · DoD 67): DataProvider **v14 = 127메서드**(`updateDeliverable`·`deleteDeliverable`) · SQL RPC 2개
  (`20260925000100_deliverable_edit_delete.sql` — 19번째) · Drive `archive-item`(지운 항목 폴더 → 행사 `99_archive`, 공통 파트 폴더는 안 옮김) · S3 **항목 관리 카드**
  (고치기 = pm·영역 담당 / 담당·가이드 = pm · 지우기 = pm, 항목 이름 입력 → 결과 → 보드로). 로컬 Postgres 145/145(항목 고치기·지우기 22항목 추가).
  **머지·운영 반영**: 실DB에 setup.sql 먼저 적용(함수 2개 확인 · 행 수 불변) → [PR #49](https://github.com/JasonCreed-creator/communicator/pull/49) Vercel 미리보기 성공 →
  머지 `480fb46`(사용자 지시 "검증 후 바로 머지") → 운영 배포 READY · 번들에 삭제 확인 창 표식 · `archive-item` 로그인 없이 401.
  ⑱ **Phase 6 — Slack 알림 + 매일 리마인드**(설계서 v2.10.1 §9·§8.4 · DoD 68): 원천 = 기존 activity_log(사건 5종) · 선점 표 `notification_log`(한 번만) ·
  즉시 = 앱이 사건 직후 `api/notify`에 신호만(기다리지 않음 — 발주처·파트너 화면은 링크 토큰) · 매일 = Vercel cron `0 0 * * *`(KST 9시대 — Hobby 하루 1회,
  2026-09-25 Vercel 문서 확인) D-1 리마인드 3종 + 미등록 파일 묶음 · 보낼 곳 = 행사 채널(행사 설정 ③ Slack 카드, pm) → 공용 env → 없음(no-op·skipped) ·
  홈 '리마인드'·'컨펌 독촉' = Slack(시간당 1회) · 알림 링크 `?project=` · 발송 폼에 "이메일은 아직(6b) — 링크 복사 전달" 안내. DataProvider **v14.1**
  (`ProjectPatch.slack_webhook_url` 필드만 — 127 불변). `CRON_SECRET`은 Code가 운영 env에 Secret으로 넣음(값은 `.env.local`에만). 로컬 Postgres 159/159(알림 14항목).
  **남은 입력(기획자님)**: Slack Incoming Webhook 주소 1개(공용 채널용이면 Vercel env, 행사별이면 행사 설정 ③에 직접) → '테스트 보내기'로 실발송 확인.
  **머지·운영 반영**: 실DB setup(알림 함수 4 · 최근 사건 4건 skipped 표시 · 행 수 불변) → [PR #50](https://github.com/JasonCreed-creator/communicator/pull/50) 미리보기 성공 →
  머지 `34b68bf` → 운영 READY · `/api/notify` = `{"slack":false,"cron":true}` · 로그인 없는 drain 401 · 틀린 크론 비밀 401 · 크론 등록 `0 0 * * *` 확인.
  ⑲ **Phase 4.7 — 협력사 견적서 불러오기(엑셀)**(설계서 v2.11 §19.5a·§8.5 · DoD 69): 정산보드 불러오기 카드(옛 "Phase 4.7에서 열립니다" 자리) →
  엑셀 고르기 → **앱이 읽는다**(견적서 임포트 파서 §22.2 재사용 — 서버 불필요) → 제안만 저장(`settlement_imports`: 행·원가 버킷 제안·부가세 판정) →
  확인 큐(부가세 — 모르면 반드시 고름 · 행별 포함·제목·버킷 · '확인 필요' · 한 번에 채우기 · 공급가 대조) → 확정 RPC가 **저장된 제안의 금액**으로 발주 항목
  (status='ordered', 부가세 포함이면 ÷1.1 + 원본) · 원본은 Drive 행사 폴더 `02_견적·정산/협력사 견적서`(4MB 이하 · 실패해도 가져오기는 그대로 · 인박스가
  다시 올리지 않음). DataProvider **v15 = 131메서드**(v7부터 예약해 온 `importVendorQuote` 소진 + 목록·확정·버리기). SQL `20260925000300_vendor_quote_import.sql`
  (21번째 — RPC 3 + `drive_known_file_ids` 재정의, 표·RLS 불변). 로컬 Postgres 175/175(정산 가져오기 16항목). **가정**: 가상 픽스처 2종으로만 검증 —
  **실협력사 견적서 샘플을 받으면 인식 규칙 보정**(파일은 레포에 넣지 않는다). PDF·사진은 다음 단계.
  **머지·운영 반영**: 실DB setup 먼저(3.9초 · 새 함수 3 · `drive_known_file_ids` 갱신 · 권한 확인 = anon 실행 불가 · 원본 기록 함수는 service 전용 · 행 수 불변 —
  운영에는 정산보드가 이미 1개 있어 바로 시험 가능) → [PR #51](https://github.com/JasonCreed-creator/communicator/pull/51) 미리보기 성공 → 머지 `529cb72` →
  운영 READY · 번들에 불러오기 카드 문구 있음(옛 "Phase 4.7에서 열립니다" 0) · `settlement-file` 로그인 없이 401 · `/api/drive` configured · `/api/notify` 그대로.
- **이전: 실서버 작동 테스트 [B] 3턴 — 1·2턴 완료, 3턴(Drive 연결) = 사용자 GCP 앱 등록 대기**(2026-09-25, 사용자 지시 "Phase 3.22·5 머지 후속 — 담당자 등록 +
  실서버 작동 테스트", 범위 [B] 3턴은 09-24 승인분). **실명·연락처·키·프로젝트 ref는 이 문서에 쓰지 않는다**(공개 레포 — 인원·권한·건수만).
  ① **Slack 조회 경로**: 사용자가 켠 Slack 플러그인("내 플러그인" · 커넥터 상태 "세션에서 연결됨")은 이 클라우드 세션에 오지 않았다 — 계정 플러그인 0개 ·
  세션 동기화 폴더 비어 있음 · Slack 도구 0(04:18·04:28·04:30 UTC 3회 확인). 계정 수준 플러그인 카드로 켠 뒤에도 세션 중엔 반영 없음(세션 시작 때만 로드) →
  **사용자가 Slack 프로필을 직접 복사**(버튼 선택). 추론: 켜진 쪽은 데스크톱 앱 로컬 플러그인(작성자·버전·스킬 수가 계정 카탈로그의 Slack 플러그인과 다름).
  ② **dev DB 업그레이드 — 예행연습 후 실행**: 로컬 Postgres 16에서 09-07 dev 적용본(`f799149` setup+seed)을 재현 → HEAD setup.sql ×2 = 오류 0 · 41표 행 수 불변 ·
  함수 +15/−1(`upload_version` 4→5인자) · 새 DB 로컬 검사 123/123. 실DB 사전 상태 = 예행연습과 동일(09-07판 · 행사 삭제·Drive 없음 · **Vault 켜짐** · 로그인 0 ·
  허용 도메인 제한 없음) → `supabase:remote -- setup` **2회**(4.0초·2.7초, 오류 0) → 행 수 불변(행사 8 · 산출물 124 · 참가자 1,119 · 견적 6) +
  `delete_project`·`is_admin`·`drive_*` 8함수·`drive_connection` 표·`app_config.drive_enabled`.
  ③ **`supabase:verify`**(Phase 4.1·5 코드로 첫 실행) — **83/83 통과**(09-07과 같은 83항목: DoD 1~25 흐름 · RLS 거부 3종 · 서버 재계산 · 시드 토큰 경로 ·
  정리까지 — 검증용 행사 2·견적 3·auth 3명·프로필 삭제 확인). 행사 삭제 RPC(DoD 58)·Drive는 이 스크립트 범위 밖 — 로컬 검사 123항목이 보고, 실DB는 3턴 실사용에서 1회.
  ④ **`supabase:reset-demo` 미리보기**: 시드 대조 8행사 일치 · 지울 대상 = 행사 8(cascade 19표 — 산출물 124·참가자 1,119·WBS 196 …) · 견적 6 · 협력사 8 · 데모 담당자 4 ·
  **삭제 0**(실행은 사용자 승인 후 `--yes`).
  ⑤ **실제 담당자 등록** — 7명 = admin 1(`app.promote_admin`) · sales 3 · staff 3. 이메일 = 신원 키(upsert, 재실행 무해 — 로컬에서 가상 명단 2회·권한 변경·오입력 거부 확인),
  직함 칸 "직함 · 담당"(BD팀 = Slack상 브랜드디자인팀 → "Slack 직함 끝부분 · BD팀"), 전화는 Slack에 등록된 1명만, junior 1명 보류. 로그인 0명(첫 매직링크 로그인 때
  이메일로 자동 연결). 전체 프로필 = admin 1 · sales 4 · staff 6(데모 담당자 4 포함). 명단·SQL 생성기는 스크래치 전용 — 레포에 없다.
  **2턴 차단 요인 발견**: Supabase 기본 메일은 **조직 팀원 주소로만 · 시간당 2통**(검증 2026-09-25) — 등록한 회사 주소로 매직링크가 가려면 그 주소를 Supabase 조직 팀원으로
  초대하거나 사용자 지정 SMTP가 필요하다(§3).
  ⑥ **PR #46 운영 반영 재확인**: 운영 번들(`/home`)에 3.22 표식 `application/x-communicator-person`·Phase 5 "Drive 지금 확인" 존재 · `/api/drive` 200 JSON
  (사용자 확인 04:14 UTC — merge `771711b`).
  ⑦ **`reset-demo --yes` 실행**(사용자 버튼 "지금 실행") — 시드 행사 8·견적 6·협력사 8·데모 담당자 4와 cascade 19표 삭제 → DB = 행사 0 · 견적 0 · 협력사 0 ·
  참가자 0 · 주소록 = **실제 7명만**(admin 1 · sales 3 · staff 3) · auth 사용자 0.
  ...
  **하위 경로 빌드 `deploy:check` 전 항목**(딥링크·새로고침·런처·옛 라우트·루트 이동·로고 경로·예외 0) · demo 4단(브라우저 단계는 헤드리스 셸 — 전체판 Chrome은
  ...
  ⑫ **2턴 완료 — 운영 실서버 전환 + 입구 열림 + admin 입장**: 사용자가 **프로젝트 전용 Vercel 토큰**(2026-07-30 도입된 project-scoped — 이 프로젝트 밖 요청은 거부)을
  주어 Code가 REST API로 처리(값은 `.env.local`에만, 문서·대화에 재인쇄 없음 — 사용자 요청으로 대화에 1회 출력한 예외 있음). Production env 7개
  (`VITE_DATA_PROVIDER=supabase` · Supabase URL 2 · publishable · `AUTH_GATE`·`AUTH_GATE_UNTIL`(9/28 18:00 KST) = Config, `SUPABASE_SECRET_KEY` = **Secret**) ·
  기존 `VITE_DATA_PROVIDER`(Sensitive, Production+Preview — 값은 mock으로 추정: 운영이 mock으로 떴고 알 수 없는 값이면 앱이 시작 안 함)는 **Preview 전용으로 이동**
  (PR 미리보기는 계속 mock) → 최신 운영 배포 Redeploy(32초) → `/api/gate` 열림 · 주소록 7명(admin 1 · sales 3 · staff 3) · 운영 번들에 Supabase 주소·publishable 포함 ·
  **secret 0건 · `AUTH_GATE` 이름 0건**. 헤드리스 브라우저 확인은 컨테이너 TLS(Chromium이 체인을 거부)로 불가 → 사용자 브라우저로 "작동 확인". DB: auth 사용자 1 ·
  주소록 연결 1(admin) · 사용자가 행사 1 · 항목 1 · 버전 1을 만듦(파일은 Drive 미연결이라 `pending:` 자리표시 — 저장 안 됨). Vercel 사실(2026-09-25 웹 확인):
  env 입력은 'Sensitive' 토글 → **Secret/Config 선택**으로 바뀜(2026-08-24) — Secret은 저장 후 누구도 못 읽고 빌드·런타임에는 전달.
  ⑬ **Drive 연결 계정 = 개인 계정으로 변경**(사용자: "이건 내가 원하는 방식이 아님 — 구글 드라이브는 내 개인계정과 연결되고 그 드라이브가 팀 계정과 공유되어 있어서
  직접 업로드가 가능함" → 버튼 "개인 계정으로 연결"). 확인한 사실: 저장소 폴더 `MICE Communicator`(9/24 생성)는 **회사 계정 소유 + 개인 계정에 편집 공유**
  (개인 계정으로 파일 추가 가능 — 9/24 README도 개인 계정 소유로 들어가 있음). 결정 = **개인 GCP 프로젝트 · 대상 외부(External) · 프로덕션 게시**(테스트 상태는
  7일마다 끊김 · 100명 이하는 미검수 경고만 넘기면 사용 — 웹 확인 2026-09-25) → 앱 '연결하기'도 개인 계정. 조건: 앱이 올린 파일은 회사 폴더 안에 쌓이되 **소유자는 개인
  계정**(사용자 선택 시 고지). `DRIVE_ROOT_FOLDER_ID`는 Code가 Vercel에 선반영(서버는 3개가 다 있어야 '설정됨' — 지금도 `configured:false`).
  남은 것 = 사용자 GCP 5단계(프로젝트·Drive API·Google Auth Platform 외부+게시·웹 클라이언트 · 리디렉션 `https://www.rmb-mice.com/api/drive`) → 클라이언트 ID·secret →
  Code가 env 2개 + Redeploy → 사용자 '연결하기'(개인 계정) → 행사 폴더 만들기 → 업로드 → Code가 Drive에서 규약 이름 확인.
  ⑭ **Phase 4.3.1 — 실사용 결함 5건 정정**(설계서 v2.9.3 · DoD 66 — 사용자 실측 캡처 "그림1.png — 현재 상태(pending_approval)에서는 업로드할 수 없습니다" ·
  "지금 어디에 쌓이고 있는지 파악이 안됨"): 원인 = 그 항목은 이미 발주처에 보낸 **컨펌대기**(설계상 업로드 불가 — 나가는 길은 발주처의 승인·수정요청뿐)인데
  행사에 **발주처 링크가 0개**라 아무도 결정할 수 없어 멈춰 있었다. 고친 것(표시만 — 전이표·DataProvider 125·DB 불변): ① 막힌 상태는 업로드 폼 대신 이유·다음 할 일
  (헤더 버튼은 자리 유지 + 비활성 — 3.17b 계약) ② 거부 문구에 영문 상태 코드 0(provider 가드 + SQL·Drive 함수 문구 치환) ③ 발주처 링크 0개면 발송 폼에 경고
  (발송은 막지 않음) ④ 버전 이력에 저장 위치(실서버: Drive / 임시·저장 안 됨) ⑤ 실서버 Drive 미연결 경고 상자. 사용자 승인: 3건 [A] → 저장 위치 2건 추가 승인(5건 한 번에).
  '발송 회수'(PM이 컨펌대기를 되돌리기)는 전이표 개정이 필요해 넣지 않음 — 필요하면 별도 결정.
  ⑮ **일정·순서(사용자)**: "연휴 기간 동안 테스트까지 끝내고 실전 투입" · "보안 이슈는 모든 게 작동되는지 확인한 이후로" → 기능 확인(3턴 Drive·실사용 1회)을 먼저 끝내고,
  그 뒤에 로그인(열린 입구 폐기 — Okta 또는 대안)·요금제를 다룬다. 입구는 9/28 18:00 KST에 스스로 닫힌다(연장은 env 1개).
...
- **완료: Phase 3.22 — 담당자 배정 카드(누르기·끌어놓기)**(2026-09-24, 사용자 지시: 온보딩 ② 캡처와 함께 "여기서 이미 구성원이 포함되어야 함" →
  ...
  범위 [A] 이 세션 한 번에. 온보딩 ②·행사 설정 ② 공용 `MembersEditor`: 배정 현황 표 → **역할 칸 4개**(PM·디자인·운영·등록, 칸마다 이름·직함·이메일·전화·빼기) ·
  셀렉트 피커 → **주소록 인물 카드**(이니셜·이름·직함) — 누르기 → 역할 버튼 4개 / 역할 칸으로 끌어놓기(HTML5 DnD, 새 의존성 0, 파일 끌기 무시).
  **미리 배치 없음**(생성자 PM만 — §8 불변) · 기본 역할 안은 취소 → **DataProvider 125 불변**. 열 수 = 컨테이너 쿼리(행사 설정 좁은 열 2×2 · 넓으면 4열) ·
  역할 표시 = 도트(디자인지시서 §7-1.2) · 빼기 확인 문구 "주소록에는 그대로 남습니다".
  검증: DoD 63 신설 7건 + dod19·dod53 선택자만 카드 기준으로(증명 내용 불변) · vitest 116파일 **1,142 통과** · 1 실패(LibreOffice — Calc 없는 컨테이너, 기존 환경 문제) · 1 건너뜀(로컬 Postgres 있을 때만 도는 스키마 실증) ·
  tsc · build · demo 4단 + 브라우저 검증 · 스모크 ③ = **실제 크롬 끌어놓기·누르기 배정**(33항목 통과) · 온보딩 ② 새 행사 실측 캡처(PM만 → 누르기·끌기 → 배정).
  문서: CLAUDE.md v2.8.1(Phase 3.22·DoD 63) · 설계서 v2.9.1(§10 S0②·S6② · §4-2b · §14) · 디자인지시서 §7-1.8 신설.

- **완료(코드) · 실연결 대기: Phase 5 — Drive 저장소 연결 + 업로드 3경로**(2026-09-24, 사용자 지시 "지금부터 작업물 저장소는 이 폴더
  (Drive 폴더 링크) — 각 파트별 폴더링 · 파일을 끌어놓거나 폴더에서 업로드하거나 드라이브에 직접 올린 후 링크로도 작업").
  범위 게이트 **[A] 한 번에** 승인 + 버튼 결정 3건: **연결 계정 = 폴더 소유 계정 OAuth** · **코드 완성까지(실키는 다음)** · 승인 = DataProvider **v13.1** 동결 해제 겸함.
  ① **실측이 설계를 바꾼 4건**(2026-09-24 웹·Drive 확인): 지정 폴더 = 사용자 회사 계정의 **내 드라이브** 폴더(공유 드라이브 아님) ·
  **서비스 계정은 저장 용량이 없어 내 드라이브 폴더에 파일을 못 만든다**(Google 공식) → OAuth로 · Vercel 함수 요청 본문 **4.5MB** + 브라우저→Google 직접 PUT은 **CORS로 막힘**
  → **4MB 조각 중계** · 프로덕션이 mock이라 실제로 쌓이려면 Supabase 전환 + Drive 연결 1회 필요.
  ② **Drive(MCP로 바로 반영)**: 루트에 `00_견적서`·`99_archive` 생성(MCP 연결 계정이 사용자 개인 계정 — 개인 계정 소유로 생김, 미결 참조) + 안내 문서 `00_README`.
  행사별 파트 폴더는 실제 행사가 생길 때 앱이 만든다(가짜 행사 폴더 미생성).
  ③ **서버** `api/drive.ts` 1개 + `api/_lib/drive/` 8모듈: OAuth 연결(관리자 → 구글 동의 → **Vault** 저장, 저장 전 루트 쓰기 확인) · 표준 트리(`YYMMDD_코드_행사명/01_기획…99_archive`,
  appProperties 표식으로 **멱등**) · 기존 폴더 지정 · **조각 업로드**(권한은 사용자 JWT로 `drive_upload_check`가 바이트 전에, 커밋은 `upload_version` 5인자판, 거부 시 휴지통) ·
  **링크 등록**(루트 안만 — 행사 폴더 안 참조 · 루트 안 다른 곳 복사 · 밖 403) · **서명 URL 스트림**(PDF 외 sandbox CSP·100MB·Range·구글 문서 PDF) · **인박스 스캔**(폴더 목록 비교 —
  ...
  mock에서도 업로드 3경로가 돈다(파일은 브라우저 세션 — 카드가 그 사실을 적음 · 링크 등록 버전은 Drive 새 탭, 발주처 `/c`에는 자리표시).
  ⑥ **검증**: vitest 신규 3파일 74건(DoD 61 서버 계약 37 · DoD 62 화면 12 · 링크 파서 25) · **전체 115파일 1,136 통과 · 1 실패**(main PR #44 합친 뒤) —
  실패 1건 = `koreanAmountFormula.libreoffice`: 이 컨테이너에 `libreoffice-core`만 있고 Calc가 없어 CSV 변환이 안 됨(작업 전 커밋에서도 동일 실패, 환경 문제) ·
  tsc · build · supabase:check 123/123 · demo 4단 + 브라우저 검증 + 스모크 ③ = 업로드 3경로 · deploy:check · `npm run drive:smoke`(실계정 5단계 — 키 주입 후).
  실계정 검증은 **아직 없다**(가짜 Drive 계약 테스트까지 — CLAUDE.md Phase 5 원칙).
  **남은 일(기획자님)**: Supabase 실서버 전환(4.1 3턴 — 3키) → GCP OAuth 클라이언트 + Vercel env 3종 → 행사 설정 ③ "Drive 연결하기" → `drive:smoke` (supabase/README §3d).
  **머지 = 사용자 지시("머지하고 PR", 2026-09-24)** — PR #45 → merge `600874a`. **운영 실측**(21:44 UTC): `www.rmb-mice.com/api/drive` 200 `application/json`
  `{"configured":false}`(머지 전엔 index.html) · 운영 번들에 새 화면 문구 · `/`·`/home`·`/api/quote-gsheet`·`/api/sheets`(405)·직인 PNG 무변화.

- **완료: 날짜 드리프트 테스트 3건 고정**(2026-09-24, 사용자 지시 "이어서 고치기" — PR #43 머지 후 main 기준 후속 PR). `align-schedule` ④ · `dod13-wbs-view` 간트 ·
  `dod14-wbs-urgency` 홈 큐가 샘플 행사 WBS(event_date 2026-10-22 고정 전개)의 앞쪽 마감을 실제 날짜가 넘기며 실패하던 것 — partner-board·partner-portal과
  같은 처방으로 시계를 **픽스처 가정일 2026-08-27에 고정**(Date만 가짜, 타이머는 실제). `today`를 모듈 상단에서 `new Date()`로 잡던 2파일은
  `toIsoDate(FIXTURE_TODAY)`로 — 고정 전 실제 날짜로 계산돼 조작 태스크가 어긋나지 않게. **테스트 파일 3개만 변경, 제품 코드 무변경**
- **완료: Phase 4.2.3 — 견적서 실직인 반영 + 브랜드 자산 판정 결함 수정**(2026-09-24, 사용자 지시 "견적서 출력 시 직인란에 이거 삽입해줘(누끼따야함)").
  범위 게이트 **[A]** 승인(브리프 + 보관 방식·크기 선택 1회). **사용자 결정 2건**: 보관 = **사이트 공개 경로**(`public/brand/remember-seal.png`) ·
  크기 = **72px**(60·72·84 실렌더 비교 — A4 한 장 맞춤 인쇄 지름 약 12mm).
  ① **누끼**: 원본 596×606(배경 #FBFBFB 균일 · 잉크 #FF0000 단일색)을 단일 잉크 모델 `P = a·잉크 + (1−a)·배경`의 최소제곱 알파로 분리
  (잔차 최대 1.5/255 — 모델 적합), RGB는 잉크색으로 고정해 **흰 테두리 번짐 0**, 도장 외곽 bbox 566px + 여백 2px → **571×571 정사각 투명 PNG(85KB)**
  ② **운영 결함 발견·수정**: 직인 파일이 없던 동안 `/brand/remember-seal.png`가 404가 아니라 **index.html을 200으로** 돌려줬다(vercel.json SPA rewrite) —
  `loadAssetBase64`가 상태 코드만 보고 받아 **모든 견적서에 HTML 917B가 `xl/media/image2.png`로 박혀 나갔다**(LibreOffice는 무시, Excel은 깨진 그림 가능 — 미검증).
  4.2의 "없으면 404 → 미삽입" 가정이 운영에서 틀렸던 것. 수정 = **PNG 서명 판정**(로고·직인 공통) + `deploy:check`에 로고·직인 PNG 서빙 2항목(파일을 빼면
  운영과 똑같이 `200 text/html 917B`로 실패함을 확인)
  ③ **리멤버 브랜드 전용**: 공개 직인은 `B === REMEMBER_BRAND`일 때만 — 다른 공급자 명의 견적서는 자기 `sealBase64`가 있을 때만 직인(호출부는 현재 전부 리멤버)
  ④ **데모 아티팩트 제외**: `demo/plugins.ts`가 직인을 항상 빈 data: URI로 치환 — 링크 공유 페이지이고 내려받기가 막혀 쓰일 곳이 없다(PNG data: URI 3개 유지)
  ⑤ 검증: 신규 `sealAsset.test.ts` 5케이스(자산 계약 · H7 중심 72px 실파일 · 영문 · SPA 폴백 · 화이트라벨 — **수정 전 3건 red 확인**) · `exportEstimate` 기대값 60→72 ·
  **LibreOffice 실렌더**(국·영문, 인쇄 지름 11.6mm) · deploy:check · demo 4단 · 스모크 ③-2(내려받은 Excel 미디어 = 로고 1장·직인 0·비PNG 0) · tsc · build.
  vitest **1057 통과 · 1 건너뜀 / 3 실패**(112파일) — 실패 3건(align-schedule ④·dod13 간트·dod14 홈 큐)은 **수정 전 origin/main에서도 동일하게 실패**하는 날짜 드리프트(샘플 WBS가
  오늘 기준 '지연'으로 넘어감), 이번 변경과 무관 — 범위 밖이라 별도 작업으로 분리(§3 미결)
  ⑥ **영문 견적서는 직인 미삽입**(사용자 결정 — 영문 주소 "…Seoul, Korea"가 길어 72px 직인 테두리가 끝 글자를 덮었다, 60px도 동일): 기본 레이아웃 직인 블록에
  이어 "영문견적에는 seal 도 삭제해" → 영문은 **"(Seal)" 표식도 없이** 공급자 행을 G:H 병합, 표식이 없으면 직인 앵커도 없어 직인 파일을 받지도 않으며
  주입 `sealBase64`도 찍지 않는다(`REMEMBER_BRAND.sealMarkEn` 삭제). richSupplier(원본 Configurator 레이아웃)는 원본 등가
  대상이고 앱 호출부가 없어 불변. 영문 테스트 3건(`sealAsset` 1 · `exportEstimate` 2) **수정 전 red 확인**, LibreOffice 재렌더로
  국문 = 로고+직인·"(인)" · 영문 = 로고만·"(Seal)" 0건(주소 전체 노출) 확인. **머지 = 사용자 지시("머지 pr 모두 진행시켜")** — merge `89397f3`,
  **운영 실측**: `www.rmb-mice.com/brand/remember-seal.png` → `200 image/png 86,699B`, 커밋 파일과 바이트 동일(cmp) · 로고도 `200 image/png`

- **완료: Phase 4.2.2 — 견적서 항목 행 높이 균일화**(2026-09-11, 사용자 실측 지적 "행의 높이가 들쭉날쭉하잖아" — 구글 시트 캡처 2장).
  원인: 4.2의 `fitRowHeights`가 **행마다** 줄 수로 높이를 매겨, 비고가 2줄로 접히는 행(운영 인력·보험·RSVP·쇼업·환경조성)만 34pt이고 나머지는 20pt였다.
  수정: 2패스 — 행별 줄 수를 먼저 재고, **항목 행 전부에 같은 높이**(가장 긴 행 기준, `UNIFORM_MAX_LINES=2` 상한 → 보통 34pt)를 준다.
  3줄 이상인 행만 그 행만 예외로 키운다(빈 행 10·헤더 20·섹션 제목 22·소계 20·확정 간격은 그대로). LibreOffice PDF 렌더로 전 섹션 균일 확인.
  테스트: `exportEstimate.test.ts` 행 높이 케이스를 "1줄 행(엔지니어·웹페이지)과 2줄 행(운영 인력·RSVP)이 같은 높이 = 34"로 교체. CLAUDE.md Phase 4.2·DoD 59 문구 정정
...
- **완료: Phase 4.2 — 견적서 생성 고도화(구글 스프레드시트 · 한글금액 이식형 수식 · 행 높이 감수 · 직인 자리)**(2026-09-10,
  ...
  ① **내보내기 2종 병존**: `Excel 내려받기`(기존) + `구글 스프레드시트로 만들기` — 같은 xlsx blob을 **Vercel Function `POST /api/quote-gsheet`**
  (`api/_lib/quoteGsheet.ts`)에 넘기면 JWT→`app_role` admin·sales 확인 → 서비스 계정(`GOOGLE_SHEETS_SA_JSON` 재사용, 스코프 `auth/drive`)으로
  `GOOGLE_QUOTE_FOLDER_ID` 폴더에 **시트로 변환 업로드**(multipart, `mimeType=application/vnd.google-apps.spreadsheet`) → 요청자 이메일에만 편집자 공유
  (best-effort, anyone 링크 없음) → 링크. **DataProvider 125메서드 불변** — 클라이언트는 `modules/quote/export/createQuoteSpreadsheet.ts`(saveQuoteFile과 같은 층)
  + `AuthAdapter.getAccessToken`(신설, mock=null) + 훅 `useQuoteSpreadsheet`. mock 공급자는 서버를 부르지 않고 **안내 문구**(role=alert), 자격증명 없으면
  503을 사실대로(데모 링크 흉내 없음). 결과 카드 = 새 탭 열기·링크 복사·공유 대상
  ② **한글금액 = 이식형 수식** `koreanAmountFormula.ts`: `NUMBERSTRING`(한국어 Excel 전용 — 시트 변환 시 `#NAME?`)을 TEXT·MID·VALUE·IF·ROUND·ABS만으로 짠 수식으로
  교체(16자리 패딩 → 자리별 읽기, "일십·일천·일억…만" 표기 = 기존 JS와 1:1, 0="영", 음수=절댓값). 검증 2중: 미니 평가기(테스트 전용 파서, CI) 40샘플 +
  **LibreOffice Calc 강제 재계산**(OOXMLRecalcMode=0 프로필, soffice 있을 때만 — 대조군 셀 999→2로 재계산 실증) 12샘플 전부 JS 정본과 일치
  ③ **디자인 감수(행 높이)**: ExcelJS 산출물은 명시 높이 없는 행이 전부 기본 15pt라 Excel에서 2줄 산출 내역이 잘리고, 시트는 자동 확장돼 행마다 달랐다 →
  `fitRowHeights`가 **사용 행 전부**에 내용 기준 높이(줄 수 추정 × 1.4em + 6, 최소 20 / 헤더 20 / 섹션 제목 22 / 소계 20 / 빈 행 10)를 준다. 사용자 확정 간격
  (타이틀 22.65×3·오렌지 4·베뉴 고지 18.65·옵션 안내 16·총액 27.5)과 **열 폭 그리드(2026-08-13)는 불변**. 격자선 off, 통화 서식 `"₩"#,##0`(따옴표 — Sheets 호환).
  LibreOffice 렌더(PDF→PNG) 실측: 1줄 행 20pt·2줄 행 34pt 리듬 균일, 한글금액 수식이 엔진 재계산으로 "일금 일억삼천사백팔십칠만원 정 (134,870,000원)" 표시
  ④ **직인 자리**: 공급자 행을 **G 상호 / H "(인)"**으로 나누고(다른 행은 G:H 병합 유지) `public/brand/remember-seal.png`를 H 중심 60px(≈16mm)에 앵커.
  파일이 없으면 글자만(404·0바이트 → 미삽입), **파일을 넣는 순간 자동 반영** — 규격은 `public/brand/README.md`. 로고·직인 fetch는 `loadAssetBase64` 1곳
  (데모 가드 "fetch 1건" 유지, `demo/plugins.ts` 치환 4건·없으면 빈 data: URI). 임시 직인(붉은 원)으로 위치 실측 완료
  ⑤ **부수 정정**: `partner-portal.test` 날짜 드리프트 1건(HT-2 D-37=09-08 경과 → "완료" 등장, origin/main도 실패) — partner-board와 같은 시계 고정으로 해소.
  결과: vitest **1051**(110파일, 기준선 980 — +71 = 한글금액 46·내보내기 9·헬퍼 5·UI 4·API 6·LibreOffice 1, 회귀 0) · tsc · `npm run build` · **demo 4단 + 상호작용 스모크
  ③=견적 내보내기 경로 9항목 통과** · 상시 가드 0건(HEAD 대비 불변). 정본: 설계서 §8 표 `POST /api/quote-gsheet` 행·§12·§17.3-2 · CLAUDE.md v2.7.1 Phase 4.2·DoD 59 ·
  `.env(.production).example` 3키 · brand README. **가정 2건(첫 실키 주입 시 확정)**: xlsx→Sheets 변환이 셀 위 이미지(로고·직인)를 보존 · 공유 드라이브 폴더면 파일
  소유가 드라이브에 남음(내 드라이브 폴더는 SA 용량 사용). **PR #40 머지(2026-09-10, 사용자 지시 "머지하고 커밋하고 버셀도 올려")** —
  드래프트 해제 후 merge commit(레포 관례). main 머지 = Vercel 프로덕션 자동 배포(§18a). 챗 검수는 머지 뒤 실기 테스트로 갈음 — 되돌릴 것이 나오면 main 위 후속 커밋
...
- **완료: Phase 3.21.1 — 표 줄바꿈 정본 + Vercel·도메인 실배포 완료**(2026-09-04 저녁).
  ...
  `www.rmb-mice.com/` 200 + 런처 번들, `/quotes`·`/home`·`/configurator` 200, `/c`·`/p` noindex·no-store.
  Deployment Protection은 Standard — `*.vercel.app` Preview는 Vercel 로그인 필요, 커스텀 도메인은 공개.
  API 토큰(1일 만료)은 셸 env로만 썼고 기록하지 않았다 — 사용자가 삭제할 것
  ② **표 줄바꿈 정본**(사용자 실측 지적 "단락 정렬 제대로 필요함 — 전체 페이지 전수조사"): 운영계획서 인쇄본 05 표에서
  헤더·배지·짧은 칸이 글자 단위로 끊겼다. 원인 = `.ui-th`에 nowrap이 없고(정본 `.ui-table`만 nowrap), 배지 컴포넌트에도
  없었다. **전수조사 결과 표 29개 = `.ui-table` 16(보호됨) + 일반 표 13**. 수정 = `.ui-th` 전역 nowrap · 배지 4종+인라인
  배지 span 10곳 nowrap · S9 문서형 표 3개(제작물·큐시트·프로그램)는 짧은 칸만 nowrap + 전 칸 상단 정렬 + 수량 `ui-num` ·
  나머지 일반 표(정산 항목·담당자·파트너 명부·등록 RSVP·임포트)는 짧은 칸만 nowrap. 디자인지시서 §7-1.3 **규칙 11** 신설.
  인쇄 미디어 A4 실측: 헤더 33px·배지 20px(한 줄), 본문 칸만 접힘. 결과: vitest **935**(101파일, +4 DoD 57) · tsc · build · 가드 0건
...
## 3. 미결
- ~~**(2026-09-26) Slack 비공개 채널에 봇 넣기 막힘**~~ → **해결(같은 날)**: 채널 세부정보 → '에이전트 및 앱' 탭 → 앱 추가로 들어감("이진철 님에 의해 mice-biz-team에 추가되었습니다"). `/invite`만 막혔던 것 — 화면 안내도 이 경로로 고쳤다. 아래는 당시 기록.
  `#mice-biz-team`(비공개 · 채널 매니저 = 기획자님)에서 `/invite @mice-communicator` → Slackbot "이 채널에 사람을 초대할 수 있는
  권한이 없습니다". 앱 핸들은 `@mice-communicator`(auth.test의 사용자 이름 `micecommunicator`와 다름 — 화면 문구는 이름을 박지 않도록 고침). 다음 시도 = 채널 세부정보 →
  '에이전트 및 앱' 탭 → 앱 추가. 그래도 막히면 워크스페이스 관리자 정책(비공개 채널 멤버·앱 추가 제한)으로 보고 관리자에게 요청(**추론** — Slack 도움말에 채널 매니저를 막는 설정이
  명시돼 있지 않다). 비공개 채널이라 `chat:write.public`(공개 채널 전용)으로는 우회할 수 없다.
- **(2026-09-25) 개인정보처리방침 페이지 없음** — Drive 연결용 OAuth 동의 화면 브랜딩(개인 GCP)이 앱의 개인정보처리방침 주소를 가리키지만 그 화면이 아직 없다.
  소유자 1명이 쓰는 지금은 미검수 경고만 넘기면 동작하나, 다른 사람이 연결하거나 검수를 받으려면 필요하다. 내용(수집 항목·보관·연락처)은 회사 정책 확인 후 — 추측 작성 금지.
- **(2026-09-25) 3턴 정리** — 위 "Drive 실서버 작동 테스트 3턴" 항목은 연결·폴더·업로드 확인으로 **종결**(§1 ⑯). 실사용 1회(발주처 링크 → `/c` → 확정 사본)만 남음.
...
- **(2026-09-25) Drive 실서버 작동 테스트 [B] 3턴 — 1턴 완료, 2·3턴 남음**(§1 첫 항목). GCP = **회사 조직 프로젝트**(동의 화면 Internal).
  2턴 = Vercel env 5종(`VITE_DATA_PROVIDER=supabase`·`VITE_SUPABASE_URL`·`VITE_SUPABASE_PUBLISHABLE_KEY`·`SUPABASE_URL`·`SUPABASE_SECRET_KEY`) → 재배포 →
  Supabase Auth URL(Site·Redirect) → 매직링크 로그인(admin 계정) / 3턴 = GCP OAuth 클라이언트 → Vercel env 3종 → Drive 연결 → `drive:smoke` → 실사용 1회(supabase/README §3d).
  **2턴에서 결정할 것**: ⓐ **매직링크 메일 경로** — Supabase 기본 메일은 프로젝트 조직의 팀원 주소에만 보내고 시간당 2통 한도다(그 밖 주소는 `Email address not authorized`).
  admin 1명 로그인 시험은 그 회사 주소를 Supabase 조직 팀원으로 초대하면 되지만, **팀 7명이 쓰려면 사용자 지정 SMTP가 필요**(Auth → SMTP — 코드 변경 없음, 설계서 §9 Resend
  Phase 6b와 같은 결정) ⓑ 가입 허용 도메인(`app_config.allowed_email_domains` — 지금 제한 없음. 등록 7명은 모두 같은 회사 도메인)
  ⓒ Vercel 전환 순간 운영 사이트는 로그인 필수가 된다(누구나 보던 mock 데모는 사라짐 — 데모 아티팩트·회사 정적 사본은 mock 그대로)
- ~~(2026-09-24) 실제 담당자 등록 대기~~ → **등록 완료(2026-09-25, §1 ⑤)** — junior 1명만 이름·이메일 확정 전 보류(주소록 화면 또는 같은 방식으로 추가).
  **실명·연락처는 DB에만**(공개 레포 — 코드·문서·PR·테스트 금지, 테스트는 가상 인물). 전화는 Slack에 등록된 1명만 — 나머지는 빈 칸(주소록 화면에서 입력).
- **(2026-09-25) Slack 플러그인은 데스크톱 쪽에만 켜져 있다** — 클라우드 세션(Claude Code on the web)에는 계정 수준 플러그인·커넥터만, 그것도 세션 시작 때만 온다.
  클라우드 세션에서 Slack이 필요하면(운영지침 §9 '내게쓰기' 초안 포함) 계정 수준 Slack 플러그인을 켠 뒤 **새 세션**에서 Slack 도구 노출부터 확인.
- **(2026-09-25) dev 프로젝트 = Supabase Free 플랜** — 7일간 활동이 적으면 일시정지(90일 안 복구 가능) · 백업 내려받기 불가(Pro는 7일 일일 백업, 검증 2026-09-25).
...
- **(Phase 5) 실연결 대기** — 코드는 완성, 실계정 검증 0. 순서: ① Supabase 실서버 전환(4.1 3턴 — dev 3키) ② GCP OAuth 웹 클라이언트(동의 화면 Internal 권장 · 리디렉션 `https://{도메인}/api/drive`)
  ③ Vercel env `DRIVE_ROOT_FOLDER_ID`·`GOOGLE_OAUTH_CLIENT_ID`·`GOOGLE_OAUTH_CLIENT_SECRET` → 재배포 ④ admin이 행사 설정 ③ "Drive 연결하기"(폴더 소유 계정으로 동의) ⑤ `npm run drive:smoke`.
  첫 실키에서 확정할 가정: 조각 PUT 재개 응답(308 Range)이 실제 Drive와 같다 · 스트림이 Vercel에서 4.5MB를 넘어 흐른다(문서상 스트리밍 예외) · Workspace 관리자 정책이 사내 앱 OAuth를 막지 않는다.
- **(Phase 5) 루트의 `00_견적서`·`99_archive`·`00_README`는 사용자 개인 계정 소유로 만들어졌다** — MCP 연결 계정이 개인 계정이라서. 기능엔 지장 없다(앱은 이름으로 채택).
  회사 계정 소유가 필요하면 지운 뒤 Drive 연결 때 앱이 다시 만든다(연결 콜백이 루트 공용 폴더를 보장).
- **(Phase 5 열린 질문) 세션 2파일 템플릿(CLAUDE.md·PROGRESS.md) 원본 자리가 없다** — §7.1 "행사 생성 시 템플릿 복사"는 건너뛰었다. 원본 위치(예: 루트 `90_템플릿/`)와 내용(jc-workspace-ops 표준)을 정하면 트리 생성에 붙인다.
- **(Phase 5) 파트너 포털(`/p`) 제출 파일은 여전히 이름만 기록된다**(원본 미보관 — 3.15부터). Drive로 받으려면 `PartnerSubmissionInput`에 파일 필드가 필요 = 동결 해제 대상. 첫 주최형 실전(DMS 2026) 전에 결정.
- **(Phase 5) 구글 문서·시트·슬라이드를 링크로 등록한 버전은 컨펌 발송이 안 된다**(확장자 없음 → 미리보기 포맷 규칙 §5). 보기는 PDF 내보내기로 된다. 발송까지 원하면 "PDF로 내보내 버전 등록" 버튼을 추가할지 결정.
- **(Phase 5) 5분 cron 없음** — Vercel Hobby cron은 하루 1회. 인박스는 홈 조회 시(행사당 60초 1회) + "Drive 지금 확인". 확정 복사 재시도도 스캔에 묶여 있어, 아무도 홈을 안 열면 approved가 남는다.
- **(Phase 5) 대용량 업로드 비용** — 조각 4MB = 함수 1회. 1GB 파일 ≈ 256회 호출. 상한은 `DRIVE_MAX_UPLOAD_MB`(기본 2048). 원본(PSD·영상)이 잦으면 Vercel 사용량을 보고 상한을 낮추거나 Drive 직접 업로드 + 인박스 연결을 권장.
- **(Phase 5) 파일명 vN 경합**(동시 업로드 시 이름의 vN이 실제 version_no와 한 칸 어긋날 수 있음) — Phase 4와 같은 산식이라 유지. 필요하면 커밋 후 개명으로 보정.
- **(관찰 — Phase 5) `koreanAmountFormula.libreoffice` 테스트가 이 컨테이너에서 실패**(soffice는 있는데 Calc 변환 산출물 없음 — 4.2.3 세션은 `libreoffice-calc`를 설치했었다). 변경 전 커밋에서도 동일하게 실패 — 환경 문제, 이번 변경과 무관.
- ~~(Phase 4.2.3) 영문 견적서 직인 겹침~~ → **종결**(2026-09-24 사용자 결정: 영문 견적서는 직인·"(Seal)" 표식 없음 — 공급자 행 G:H 병합. §5 결정 로그)
- ~~(Phase 4.2.3) 날짜 드리프트 테스트 3건~~ → **종결**(2026-09-24 후속 PR — 시계를 픽스처 가정일 2026-08-27에 고정, 테스트 파일만 변경)
- **(Phase 4.2.3) 직인 공개 노출** — 사용자 결정으로 공개 경로에 두었다. 로그인 전환(Phase 4 3턴) 뒤 비공개 보관(서버·권한자 한정)으로 옮길지 재판단
- **(Phase 4.2.3) Excel 실기 확인** — 직인·로고 배치는 LibreOffice로만 실측했다. 사용자 PC Excel에서 1회 확인 필요(구글 시트 변환 시 이미지 보존은 기존 가정 그대로)
- **(Phase 4.2) 구글 시트 생성은 실키 주입 전까지 503 안내다.** D-Day §20 T4b에서 `GOOGLE_SHEETS_SA_JSON` + **`GOOGLE_QUOTE_FOLDER_ID`**(공유 드라이브 폴더 권장 —
  서비스 계정을 폴더 편집자로 초대)를 Vercel env에 넣고 재배포 → 첫 실행에서 가정 2건(이미지 보존·파일 소유) 확정. 내 드라이브 폴더를 쓰면 파일이 서비스 계정
  소유·용량이 된다(15GB 상한 — 견적서는 ~100KB라 실무상 무관하나 "내 파일"로 보이지 않는다 → 요청자 편집자 공유가 그 보완).
- **(Phase 4.2) 만든 시트 링크는 DB에 남기지 않는다** — 화면 상태(세션)와 Drive 폴더에만. 견적 행에 `gsheet_url`을 붙이려면 스키마·타입·DataProvider 변경(동결 해제)이
  필요해 이번 범위에서 뺐다. 필요하면 다음 동결 해제 때 함께.
- **(Phase 4.1 3턴 — DB 쪽 완료 2026-09-25, 배포 전환 남음) Supabase 실서버 전환.** dev 프로젝트(대시보드 표시 이름은 README의 `communicator-dev`와 다른 기본 이름 —
  09-06 생성·서울 리전으로 같은 프로젝트임을 확인)에 최신 setup.sql(18개) 적용 · verify 83/83 · reset-demo 미리보기 · **admin 승격 완료**(로그인 전 `promote_admin` —
  첫 로그인 때 이메일로 연결되므로 이전의 '로그인 후 SQL 1줄' 사용자 작업은 불필요해졌다). 남은 것 = `reset-demo --yes`(승인 대기) → Vercel env 5종 → 재배포 →
  Auth URL → 로그인(메일 경로 결정 — 위 2턴 항목) → 상호작용 스모크.
...
- ~~(Phase 4.1 이탈 2) Drive 트리는 남는다~~ → **종결(2026-09-24 Phase 5)**: 삭제 뒤 행사 폴더를 루트 `99_archive`로 옮기고 `(삭제됨 YYMMDD)` — 파일은 지우지 않는다(설계서 v2.9 §7.7 · 가정 ⑤).
...
## 4. 다음 스텝
- **(2026-09-26) Phase 3.24 운영 문서 3종 실무화**: ① PR-A 검증 → **운영 DB setup.sql(23번째)** → 머지 → 운영에서 옛 운영가이드 '뼈대 추가'·새 가이드 '기본 섹션 만들기' 1회(기획자님)
  ② **PR-B**: 시나리오 멘트 원고형(세션 제목 → 시각·구분 → 멘트 전문 크게 · 지시문 괄호 · 연사 확인 표시 · 비상 예비 멘트) · 운영 보드 카드 요약(문서별 숫자 — 큐 n개·운영 시간·대본 / 멘트 작성·연사 확인 / 섹션 채움·인력·무전·원본 갱신) · 큐시트 칸 이름(시각·큐·구분·MC·진행·조명·영상·음향 — 데이터 불변) · 시나리오·가이드 머리를 PR-4b 모양으로 · Phase 6.1 마무리 기록
  ③ 그다음: 운영계획서 장 확장(요약 KPI · 목표·성공 기준 · 공간 · 참가자 운영 · 확인 요청 · 계약 범위 대응표(사양만)) → **16:9 장표형 인쇄**(사용자 선택 — 새 의존성 0). 데이터 구조 변경이 커서 착수 전 범위 게이트
  ④ 결정 대기: 연락망 전화번호를 스태프 인쇄본에만 넣을지(R-O6 개정 필요) · 주최형 역할 분담 템플릿(파트너사 열 — **가정**) 실무 확인
- **(2026-09-26) Phase 6.1** — ~~①~⑤~~ 완료(머지 · 운영 DB · bot:true · 실발송 "완벽해!"). ⑥ 담당자 이메일 ≠ Slack 이메일인 사람은 S-13 Slack 칸 — 필요할 때. 홈 '확인 안 한 지시 n' 행은 필요하면 다음 PR.
- **(2026-09-26) Phase 6.1 — 2턴 PR 검증 → 실DB setup → 머지 → 3턴 실발송**: ① 실DB에 setup.sql 먼저(22번째 마이그레이션 — 표 1·열 2·함수 재정의, 행 수 불변 확인) ② PR 머지 → 운영 재배포(`SLACK_BOT_TOKEN`·`SLACK_SIGNING_SECRET`은 이미 운영 env)
  ③ `/api/notify` GET = `{bot:true}` 확인 ④ 기획자님: 행사 스레드 첫 글 ⋯ → 링크 복사 → 행사 설정 ③ '스레드 등록' → '테스트 보내기'(비공개 채널이면 먼저 채널 세부정보 → 에이전트 및 앱 → 앱 추가)
  ⑤ 새 지시 1건 → 카드 멘션·푸시 → '확인했어요' → 카드 교체 + 앱 보드 '확인함' 한 줄. 첫 버튼 응답이 401이면 서명 비밀을 Slack 앱 설정에서 텍스트로 다시 복사(§1 메모)
  ⑥ 담당자 이메일 ≠ Slack 이메일인 사람은 S-13 Slack 칸에 멤버 ID. **이번에 넣지 않은 것**: 홈 '오늘 할 일'의 "확인 안 한 지시 n" 행(대시보드 조회 변경 동반 — 스레드의 24시간 미확인 멘션이 같은 역할을 먼저 한다, 필요하면 다음 PR)
- ~~(2026-09-25 밤 → 새 챗) Phase 6.1 Slack 봇 전환 + 의뢰 확인 버튼~~ → 2026-09-26 착수(위 항목 · 아래는 당시 계획 — DM 부분은 사용자 결정으로 채널 스레드 멘션으로 대체) — 체크인 후 **1턴 = 메시지 시안 3장**(제작요청 DM · 확인요청 DM · 채널 기록 줄, Block Kit 미리보기 HTML) → 승인 →
  **2턴 = 구현 PR 1개**(범위 [A]) → 검증 → 머지 → **3턴 = 실발송 1회**(~~토큰 주입~~ — 이미 완료, §1 참조 · 남은 준비 = 행사 채널에 `/invite @micecommunicator` + 채널 ID를 행사 설정 ③에 입력).
  ...
  ③ SQL — `notify_claim_events`가 수신자(담당자·PM) `{profile_id,email,slack_user_id}` 배열을 함께 반환 · `profiles.slack_user_id`(이메일 매칭 캐시 + 수동 덮어쓰기) · `projects.slack_channel_id` · `deliverables.acknowledged_at`·`acknowledged_by` ·
  리마인드에 '24시간 미확인 지시' 종류 추가(담당자 DM + PM 오늘 할 일 "확인 안 한 지시 n")
  ④ 화면 — 행사 설정 ③ Slack 카드: 채널 ID 입력 + 봇 초대 확인 + 테스트 보내기(웹훅 칸은 폴백으로 유지) · 담당자 S-13: Slack 연결 상태·수동 ID · 디자인 보드 '다음 행동'·항목 상세 '다음 단계'에 "확인 안 함 · n일"/"확인함 9/26" 칩
...
- **(2026-09-25 밤) Phase 3.23 UX 개편 — PR 8개 연속(범위 [A], 순서 = 제안대로)**: ① ~~PR-1 기반~~ → 머지 ② ~~홈 '오늘 할 일'~~ → 머지 ③ ~~디자인 보드~~ → 머지 ④ ~~항목 상세~~ → 머지 ④b ~~큐시트~~ → 머지 ⑤ ~~행사 목록~~ → 머지 ⑥ ~~견적 목록·옵션~~ → 머지 ⑦ ~~정산보드~~ → 머지 ⑧ ~~행사 설정·온보딩~~ → 머지(`a90d984`) — **8개 PR 전부 완료**. 다음 = 운영에서 바뀐 화면 실사용 점검(기획자님 — 특히 행사 설정 저장 바 ·
  ...
  ⑧ 행사 설정·온보딩. **머지마다 운영에 바로 배포된다**(시험 중 화면이 바뀜 — 사용자 수용). 기획자님 입력 대기 항목(아래)은 그대로 유효.
- **(2026-09-25 저녁) 묶음 진행**: ① ~~Phase 4.5 PR → 머지 → 실DB setup~~ → 완료(`480fb46` · 운영 확인) · 운영에서 항목 고치기·지우기 1회(사용자)
  ② ~~Phase 6 Slack 알림 → PR·머지~~ → 완료(`34b68bf` · 운영 확인) · **웹훅 주소를 받으면 실발송 1회**('테스트 보내기') ③ ~~Phase 4.7 협력사 견적서 불러오기~~ →
  완료(`529cb72` · 운영 확인) · **실협력사 견적서 엑셀 샘플을 받으면 보정**(기획자님) · 운영에서 불러오기 1회(정산보드가 있는 행사 — 운영에 1개 있음)
  ④ 실사용 1회(발주처 링크 → `/c` 수정요청·승인 → `06_발주처공유` 사본) ⑤ `/privacy` 페이지(내용 = 기획자님 결정) ⑥ 그 뒤 보안(입구 폐기·로그인) · 요금제 · 토큰 Revoke(사용자)
- **(2026-09-25 오후) 실전 투입까지(사용자 일정: 연휴 안에 시험 완료)**: ① **사용자 GCP 앱 등록**(개인 계정 — §1 ⑬) → 클라이언트 ID·secret 전달 → Code가 env 2개 +
  Redeploy ② 사용자 '연결하기'(개인 계정) → 행사 폴더 만들기 → 업로드 → Code가 Drive에서 규약 이름·위치 확인 · 버전 이력 저장 위치 'Drive' 확인 ③ 실사용 1회(발주처 링크
  발급 → `/c` 수정요청 → 새 버전 → 내부검토 → 발송 → `/c` 승인 → `06_발주처공유` 사본 + 확정) + 삭제 동선 1회 ④ **그 뒤 보안**(사용자 순서): 입구 폐기 + 실사용 로그인
  (Okta SSO는 Supabase Pro·IdP 등록 필요 — 대안 포함 결정) ⑤ 요금제(Vercel Hobby = 비상업 전용 · Supabase Free = 백업 없음) ⑥ PAT·Vercel 토큰 Revoke(사용자)
- ~~(2026-09-25) 작동 테스트 이후~~ → 2턴까지 완료(위 항목으로 이어짐): ① ~~PR #47 머지~~ → 완료(`982da8f`) ② ~~Vercel env(§3 목록 — 도메인 2개 제외) + Redeploy → `www.rmb-mice.com`에서
  입구로 admin 입장~~ → 완료(§1 ⑫) · admin 메뉴(행사 삭제·Drive 연결) 확인 ③ Supabase Auth URL(Site `https://www.rmb-mice.com`) ④ **3턴**(README §3d) — GCP OAuth(리디렉션
...
- **(2026-09-24) Phase 5 이후**: ① ~~PR(Phase 5) 검수 → 머지~~ → **머지 완료**(`600874a` · 운영 `/api/drive` JSON 확인) ② Supabase 실서버 전환(4.1 3턴)
  ③ Drive 연결(supabase/README §3d) → `drive:smoke` 5/5 ④ 실사용 시험 1회: 행사 생성 → Drive에 행사 폴더 확인 → 파일 끌어놓기 → Drive에 직접 올린 파일을 링크로 등록 →
  행사 폴더에 직접 올린 파일이 인박스에 뜨는지 → 컨펌 발송 → `/c` 승인 → `06_발주처공유`에 사본 + final ⑤ ~~날짜 드리프트 3건~~ → main PR #44로 종결(이 브랜치에 합침)
- **(2026-09-24) Phase 4.2.3 이후**: ① ~~PR 검수 → 머지~~ → 사용자 지시로 머지(머지 = Vercel 프로덕션 자동 배포 — 운영 직인 경로가 PNG로 서빙되는지 실주소 확인)
  ② ~~영문 직인 겹침 조정 여부~~ → 영문 미삽입으로 종결 ③ ~~날짜 드리프트 3건 정리~~ → 후속 PR로 종결 ④ 사용자 PC Excel에서 직인 1회 확인
  ⑤ (재발 방지 검토) `new Date()` 기준 픽스처 드리프트는 이번이 4·5·6번째 — 테스트 공통 시계 고정(setup 파일)으로 일괄 전환할지는 사용자 판단
...
## 5. 결정 로그
- **(2026-09-26) Phase 3.24 운영 문서 3종 실무화 — 사용자 결정 6건 + Code 판단 5건** — ① "같다"의 뜻 = **운영보드 네 항목의 세부내용 포맷**(버튼 답 — 카드 목록 줄·문서 틀이 모두 파일 항목 모양) ② 순서 = **문서 3종 실무화부터**(추천 채택) ③ 운영계획서 출력 = **16:9 장표형 인쇄**(추천 채택 — PPTX 내보내기는 라이브러리 추가라 보류) ④ 범위 = **[B] 이 챗 N턴** ⑤ 시나리오 = **멘트 원고형** ⑥ 운영가이드 섹션 = **현장 운영과 관련되는 전부**(D-day 진행표·구간 체크 · 인력·무전 · 역할 분담 · 안전 5항목 전부 선택 + "전부") → 시안 5장 승인 "승인 → PR-A 착수".
  Code 판단: ⓐ 표 섹션의 정본은 data, content는 저장 때 만드는 사본(R-O7) — S9 ⑦·스냅숏·인쇄를 건드리지 않으려고 ⓑ 새 문서에는 R&R '역할별 체크리스트'를 넣지 않음(역할 분담 표가 대신 — 옛 문서의 role 섹션은 그대로) ⓒ 옛 문서 자동 변환 없음 — '뼈대 추가' 버튼(R-O9) ⓓ 연락망 전화번호 인쇄(시안)는 R-O6 충돌로 **미구현** — 결정 대기 ⓔ 등록 1인 처리 15초 · 피크 15분 = 템플릿 기본값(**가정** — 현장 실측으로 바꾼다), 라인·도착 인원은 비워 추측 계산 없음.
- **(2026-09-26) Phase 6.1 머지** — 사용자 "머지하고 다음스텝 진행": 운영 DB 적용 여부를 공개(publishable) 키로 **조회만** 해 확인(없는 표 = PGRST205 / 있는 표 = 42501 · 새 함수 3개 = 42501 · `notify_set_slack_user`가 `language sql`이라 `profiles.slack_user_id` 열 존재도 확정) → 머지 `a7f2333` → 운영 `{bot:true}` → 실발송 "완벽해!". 사고 모델 = Opus 5.5 높음 유지 권고(실발송 디버깅·다음 보안 작업).
- **(2026-09-26) Phase 6.1 — 사용자 결정 4건 + Code 판단 6건** — 1턴 시안 v1(DM 기준) 버튼 답: ① 확인 기록 = **의뢰 한 건마다**(추천 채택 — 항목 한 칸이면 PM 확인이 담당자 확인을 덮는다)
  ② 시안 올림 = **PM 멘션 없음, 내부검토 요청 때만**(추천 채택) ③ 받는 사람 표기 = 사용자 답 "**우리 회사는 DM을 지양하고 채널에서 멘션을 통해 메시지를 남기면서 모두가 알도록** — 각 프로젝트 채널에서 봇이 메시지를 남기고
  멘션하는 스타일" → 2026-09-25 결정 ①의 'DM' 부분 대체(봇은 유지) ④ 시안 v2 승인 + "**앞으로 채널의 스레드를 통해 하나의 프로젝트로 운영** — 프로젝트마다 스레드 주소를 설정하는 작업이 선행" → `projects.slack_thread_url`.
  Code 판단: ⓐ 스레드 링크 한 칸만 저장하고 채널·ts는 서버가 읽는다(`src/lib/slackThread.ts` — 링크 모양 3종, DM 채널 거부) ⓑ **멘션된 사람의 확인만 센다**(다른 사람은 나만 보기 안내 — 시안 v2 기본값, 이의 없음)
  ⓒ 24시간 미확인 = 원래 카드 링크 + 멘션 답글 한 번(스레드 안에 스레드가 없어 '원래 카드 스레드 답글' 안은 불가 — 같은 행사 스레드에 답글) ⓓ 스레드는 봇 토큰이 있을 때만 — 없으면 웹훅 → 공용(Phase 6 경로 그대로)
  ⓔ Slack ID는 이메일로 찾으면 **비어 있을 때만** 적는다(S-13에서 고친 값이 이긴다) · 이메일을 바꾸면 비운다 ⓕ 홈 '확인 안 한 지시 n' 행은 이번 PR에서 뺐다(§4 — 대시보드 조회 변경 동반). DataProvider v15.1 = 필드만(v13.1·v14.1 전례 — 착수 지시가 해제 승인을 겸함).
- **(2026-09-25 밤) Slack 알림 — 봇 전환 · DM 수신자 규칙 · 의뢰 확인 버튼(사용자 결정 3건, 새 챗에서 구현)** — 계기: 사용자 "많은 담당자들에게 메시지가 전달되게 하려면 결국 봇으로 운영되어야".
  판단 근거: ① Incoming Webhook은 채널 1개 고정·DM 불가·이메일→멤버 ID 조회 불가 ② 채널 글은 Slack 기본 알림 설정(DM·멘션·키워드만)에서 **푸시가 가지 않는다** ③ 웹훅도 Slack 앱을 만들어야 나오므로
  승인 절차는 같고, 봇은 같은 앱에 권한을 더하는 일 — 아직 웹훅 주소를 받지 않은 지금이 가장 싼 전환 시점.
  결정 ① **봇 전환**(선택지: 웹훅+멘션 / 봇 / 일단 웹훅) — 담당자 DM + 채널 기록, 이메일 자동 매칭, 웹훅은 폴백.
  결정 ② **DM 수신자 규칙 = 표대로**: 새 지시 → 항목 담당자 · 시안 올림·내부검토 요청·미등록 파일·파트너 제출 → PM · 발주처 승인·수정요청 → 담당자 + PM · 컨펌 발송 → DM 없음(PM이 한 일) · D-1 리마인드 → 그 역할 담당자들. 채널에는 전부 기록.
  (선택지 '전원 DM'은 알림 피로로 비추천, 'PM만'은 담당자 푸시가 없어 목적 미달.)
  결정 ③ **의뢰(제작요청·확인요청) 방식 = ② 링크 + '확인했어요' 1버튼**(선택지: ① 링크만 / ③ Slack에서 착수·완료·질문+스레드 동기화) — PM의 실제 질문은 "봤나?"이고 그건 버튼 하나로 답이 된다.
  작업 자체는 앱 '다음 단계' 카드가 이미 한 번에 처리하므로 Slack에 옮길 이유가 약하고, ③은 앱과 Slack 두 곳에 상태가 생겨 어긋남이 시작된다. 확인은 **상태가 아니라 표식**(`acknowledged_at`) — §5 전이표 무변경.
  Slack에서 의뢰를 시작하는 슬래시 명령은 제외(규격·참고 링크 없는 얇은 지시가 되기 쉽다). 발주처 확인요청은 Slack 밖(링크 → Phase 6b 이메일) 그대로.
  가정 3건(새 챗에서 확인): 리멤버 워크스페이스 커스텀 앱 설치 가능(관리자 승인 가능성) · 주소록 이메일 = Slack 이메일(다르면 수동 ID) · 행사 채널 = 공개 채널(비공개면 봇 초대 필수).
  범위 게이트 = **[C] 새 챗**(사용자 선택 — 이 챗은 UX 개편 8 PR로 컨텍스트가 큼). 설계서 §9·CLAUDE.md 개정은 구현 PR과 함께.
- **(2026-09-25) Phase 3.23 PR-8 행사 설정·온보딩 — Code 판단 6건(디자인지시서 v1.4 §7-2.12)** — ① **포맷 전환 링크(캔버스 `유형·연동 탭에서 바꾸기`) 미구현 — 결정 대기**:
  포맷 카드(FormatStep)는 행사 성격(kind)·모객 유형까지 다시 시드하고 WBS 재전개를 동반해, 진행 중인 행사의 설정에서 한 번에 여는 경로로 두면 한 번의 클릭이 행사를 크게 바꾼다. 읽기 전용 +
  이유 캡션으로 뒀다 — 필요하면 기획자님이 전환 규칙(무엇을 다시 시드하고 무엇을 남길지)을 정한 뒤 별도 ② **`저장됨 HH:mm` 캡션 추가**(캔버스에 없음) — 저장 바가 사라지기만 하면 저장됐는지
  알 수 없다(§10 저장 확인 표기 준용) ③ **바꾼 칸의 `원래 …`는 칸 안내를 대체하지 않고 이어 붙인다** — 유형을 바꾸는 순간 '데이터 보존 · WBS는 일정 화면에서' 안내가 사라지면 가장 필요한 때
  잃는다 ④ **온보딩 막대 = 필수 4 기준**(옛 = 단계 기준) — 캔버스의 `필수 4개 중 m개 입력` 옆 막대라 필수를 센다. 1단계는 저장 전 입력으로(칸을 채우는 대로) ⑤ **'나중에 하기'는 1단계에
  저장 안 한 입력이 있을 때만 묻는다** — 2·3단계는 동작마다 곧바로 저장돼('단계마다 저장돼요') 잃을 것이 없다. 행사는 지우지 않고 행사 목록 '먼저 확인할 행사'에 남는다 ⑥ 탭 이름 번호 제거 ·
  체크 스트립 퇴역(캔버스 그대로) — 테스트 17개 파일의 이름 계약을 따라 바꿨다(의미 동일).
- **(2026-09-25) Phase 3.23 PR-7 정산보드 — Code 판단 5건(디자인지시서 v1.4 §7-2.11)** — ① **초과 사유 = 발주 항목 메모(note)**: 캔버스는 버킷 한 칸에
  `초과 사유` 입력을 그렸지만 버킷에는 사유를 저장할 칸이 없다(스키마 + DataProvider 동결 해제 = 사용자 승인 사항). 항목 입력(`SettlementItemInput.note`)은 이미 있어 인터페이스 변경 0 —
  초과는 특정 항목이 만든 것이라 근거로도 맞다. 버킷 한 줄 사유가 꼭 필요하면 승인 후 별도 ② 원가 없는 버킷은 펼치지 않음(항목이 이미 있을 때만 — 자료를 숨기지 않게) — 캔버스와 같다 ③
  '초과 버킷만 보기' 필터 퇴역 — 알림이 버킷마다 `항목 보기`로 바로 연다(초과는 보통 1~2건) ④ 밀집 모드 이름은 공용 `DensityToggle` 그대로(캔버스 `촘촘하게`는 WBS·등록과 갈라진다) ⑤
  마진 구성 막대 색 = brown·steel·border-strong(캔버스 그대로 — 패턴 §07 '스택 막대 accent 3단'의 화면 예외: 금액 구성은 강조가 아니다, §7-2.11에 명시) · 참고 범위 밴드는 positive-tint.
- **(2026-09-25) Phase 3.23 PR-6 견적 목록·옵션 — Code 판단 5건(디자인지시서 v1.4 §7-2.10)** — ① 캔버스 요약의 `바꾼 이유는 적혀 있지 않습니다 · 이유 적기`는 넣지 않음 —
  견적에 변경 사유를 저장할 곳이 없다(스키마 + DataProvider 동결 해제 = 사용자 승인 사항). 할 수 있는 동작이 없는 안내라 줄도 빼고, 입력 스냅숏의 사실 차이(`바뀐 것`)만 남겼다
  ② **뒤 버전이 있는 견적의 고치기는 비활성 + 이유** — 저장하면 409(`이미 새 버전이 있는 견적입니다`)가 나는 경로라 누르기 전에 막는다(캔버스에 없던 상태 — 막힌 동작은
  숨기지 않고 이유를 적는 §7-2 규칙) ③ 확정·미연결 견적의 `이 견적으로 행사 만들기`는 요약 패널에 ghost로 유지(캔버스는 연결된 견적만 그렸다 — §16 핸드오프 진입점을 지키되
  머리의 새 견적이 채운 버튼이라 ghost) ④ 옵션 금액은 **엔진을 옵션 집합만 바꿔 여러 번 돌려** 읽는다 — 옵션 단가를 화면에 다시 적으면 엔진과 갈라질 수 있다(3.11 원칙
  '단가는 엔진 단일 출처'). 캔버스의 PCO 안내('운영비의 25%라 함께 오른다')는 엔진 산식(opCost × 25% · 만원 절사)과 맞아 그대로 썼다 ⑤ 캔버스의 '1단계에서 LED로 바꾸세요'는
  실제 위치(2단계 베뉴)로 고침 · 캔버스의 옵션 설명 축약은 하지 않고 카탈로그 설명(※ 줄 포함)을 그대로 — 중계 비용 분리 같은 안내가 빠지면 안 된다.
- **(2026-09-25) Phase 3.23 PR-4b 큐시트 — Code 판단 3건(디자인지시서 v1.4 §7-2.8)** — ① 캔버스 머리의 `인쇄용 보기`는 이번에 넣지 않음 — 인쇄 경로는 운영계획서 03 큐시트가
  이미 맡는다(A4 인쇄 CSS · DoD 9), 항목 화면에 두 번째 인쇄 경로를 만들면 두 모양이 갈린다 ② 메타 스트립은 큐시트에서만 걷고 시나리오·운영가이드 빌더는 유지(캔버스가 큐시트만
  다뤘다 — 3.16.3/3.16.4 계약 그대로) ③ 큐 추가 = 빈 큐를 바로 만들고 그 줄을 편집 상태로 연다(캔버스 문구 그대로) — 저장하지 않고 떠나면 번호만 있는 빈 큐가 남는다(지우기는 ⋯ 메뉴).
- **(2026-09-25) Phase 3.23 PR-4 항목 상세 — Code 판단 5건(디자인지시서 v1.4 §7-2.7)** — ① **PR-4를 둘로 나눔**: 항목 상세(이번) · 큐시트(PR-4b) —
  큐시트는 표·행 메뉴·끌어서 순서·대본 패널이 한꺼번에 바뀌고 운영 보드 인라인 빌더와 테스트 5개 파일이 함께 걸려, 한 PR에 넣으면 검증 단위가 너무 크다 ② **레일 6 → 5단계**(캔버스):
  수정요청은 늘 거치는 단계가 아니라 '발주처 컨펌'에서 되돌아온 표시로 ③ **막힌 상태의 업로드 카드는 그리지 않는다** — 이유는 다음 단계 카드 안 잠금 캡션 1개(Phase 4.3.1 계약
  = 파일 입력 0·업로드 버튼 0·안내 1은 유지) ④ 캔버스 '발주처에 다시 알리기'(채운) → PM `발주처 링크 복사`(ghost) — 보드와 같은 이유(보낼 수단 없음) ⑤ 캔버스 '전체 화면' →
  `새 탭에서 보기`, 'v1과 나란히 보기'는 이번에 넣지 않음(다음 개선).
- **(2026-09-25) Phase 3.23 PR-3 디자인 보드 — Code 판단 4건(디자인지시서 v1.4 §7-2.6)** — ① **캔버스 '다시 알리기' → 발주처 링크**: 발주처에게 보낼 수단이 없다
  (이메일 = Phase 6b, Slack은 내부 채널이고 서버 리마인드는 목록 단위 — §7-2.5). 살아 있는 링크 1개면 복사, 여럿·없음이면 행사 설정 ②(어느 링크인지 추측 금지)
  ② **캔버스 '끌어다 놓아 올리기' → 상세 업로드 카드로 이동**(`?upload=1`): 보드에서 바로 올리면 업로드 잠금 안내·Drive 미연결 경고(실서버에서 파일이 저장되지 않는
  상황)·진행률·여러 파일 처리를 두 곳에서 따로 지켜야 한다 — 한 곳으로 모은다 ③ **운영 보드는 이번 범위 밖**: 캔버스가 디자인 보드만 다뤘고, 운영 보드는 v2.5에서 승인된
  유형 우선 구조(유형 카드 + 인라인 빌더)라 표로 바꾸면 빌더 펼침 자리가 달라진다 — 필요하면 별도 시안 ④ 보드 상태 셀렉트는 차례 칩으로 대체(디자인 보드만 — 운영 보드
  필터 줄은 그대로). 승인(approved)은 사람이 할 일이 없어 '끝남'으로 센다.
- **(2026-09-25) Phase 3.23 UX 개편 — 사용자 결정 11건(버튼)** — ① 진행 방식 "Code 초안, 3턴 분할" ② 범위 = 운영 핵심(홈·보드·항목) + 돈(견적·정산) + 세팅(목록·설정·온보딩),
  외부 `/c`·`/p` 제외 ③ 방향 "웜 페이퍼 유지 + UX 개선" ④ 시험 입구 10/5 18:00 KST까지 연장 ⑤ 8화면 시안 승인 ⑥ 디자인 시스템 = 앱 토큰으로 새로 ⑦ 날짜 = 남은 날 D-n ·
  지난 날 'n일 지남' ⑧ 홈 = '오늘 할 일' 한 목록 ⑨ 3턴 = Code 전체 검수 + 디자인 보드 갤러리 보기 ⑩ 대비 = 보조 글자·빨강 둘 다 어둡게, 주황 채운 버튼 바탕 = 진한 주황
  ⑪ **바로 구현**, 범위 [A] 연속 · 순서 제안대로. 해석(Code): 비주얼 미리보기 = 항목 상세의 최신 시안(가정 — 보고에 명시) · 행사 당일 라벨은 업계 관행 'D-day' 유지,
  기한 당일은 '오늘' · 완료된 일의 기한 칸은 '—' · 발주처·운영계획서의 D+n은 외부 범위 밖이라 유지 · 흰 글자 주황 면 규칙을 버튼 외 배지·단계 표시에도 적용(대비 결정의 취지).
- **(2026-09-25) Phase 4.7 설계 판단 5건(Code — 설계서 v2.11 §19.5a)** — ① 파싱 자리 = **앱(브라우저)**: v2.2는 "파싱은 서버가 필요"라 적었지만 §22 견적서 임포트가
  동기 xlsx 리더로 브라우저 파싱을 이미 성립시켰다 — 같은 파서를 재사용하고 서버는 확정·원본 보관만 ② 확정 요청은 행 번호·버킷·제목만, **금액은 저장된 제안에서**
  (검토한 값과 저장 값의 일관성 — pm은 원래 항목 금액을 직접 입력할 수 있으므로 위변조 방어가 목적은 아니다) ③ 버킷 제안·선택지 = **원가 버킷만**(s5·rc·ld 제외 — R-S4),
  대행료·절사 행은 가장 큰 제안 버킷 + '확인 필요' ④ 부가세 = 4단 판정, 표기가 없으면 **제안하지 않는다**(§19.4 "자동 판정하지 않고 반드시 묻는다") ⑤ 원본 보관은
  best-effort(Drive 미연결·4MB 초과·오류에도 가져오기는 진행) + 인박스 대조에 포함. mock의 확인 큐는 메모리(픽스처 상태 불변 — Phase 4.1 금지 규칙).
  스모크에서 발견해 고침: 우리 견적서 임포트의 헤더 질문 경고('고객명·일시… 확인 큐에서 입력')가 협력사 확인 큐에 떠서 없는 칸을 찾게 했다 → 협력사 견적에서는 뺀다.
- **(2026-09-25) Phase 6 설계 판단 5건(Code — 설계서 v2.10.1 §9)** — ① 알림 원천을 새로 만들지 않고 activity_log를 쓴다(사건마다 훅을 새로 박지 않아
  경로가 늘어도 빠지지 않는다) ② 즉시 알림은 DB 트리거(pg_net)가 아니라 **앱의 신호 + 서버 선점** — 로컬 Postgres 검사 가능·Vercel Hobby 크론(하루 1회)으로는
  즉시성이 안 나와서. 놓친 신호는 매일 크론이 거둔다 ③ 보낼 곳 = §9 원문대로 **행사별 채널**(이미 있던 `projects.slack_webhook_url`) + 공용 env 폴백 —
  `ProjectPatch` 필드 1개 추가(v14.1, v13.1 전례대로 묶음 범위 승인이 동결 해제를 겸함) ④ 서버는 Slack Incoming Webhook 주소로만 POST(설정 값에 임의 URL이
  들어와도 서버가 그 주소로 요청하지 않게) ⑤ 채널이 없을 때 선점 행을 `skipped`로 남긴다 — 나중에 채널을 연결해도 지난 사건이 한꺼번에 가지 않게.
  홈 리마인드 버튼(원래 "Phase 6 예정" 안내)은 Slack 발송으로 채움(시간당 1회 — 서버가 막음). 발주처 이메일은 계속 Phase 6b.
- **(2026-09-25) 남은 기능 = 3묶음 순서 진행 · 검증 후 바로 머지 · 지우기 범위(사용자 버튼 3개 + 연속 진행 지시)** — "승인 — 묶음 1→2→3 순서" ·
  "바로 머지" · "모든 상태 + 이름 입력 확인" · 이어서 "입력 없이 수행 가능한 범위까지 쭉 진행". 이번에 하지 않는 7건(§1)은 사용자 확인 없이 착수하지 않는다.
  바로 머지 = 각 묶음 PR의 로컬 검증(vitest·tsc·build·deploy:check·demo 4단·스모크·로컬 Postgres)이 깨끗하면 Code가 머지 — 운영 반영은 머지 후 실주소로 확인.
- **(2026-09-25) Phase 4.5 설계 판단 4건(Code — 설계서 v2.10 §8·§7.8)** — ① 정형 문서(큐시트·시나리오·운영가이드) 종류 전환은 **양방향 409**: 빌더 데이터가
  카테고리에 매여 있어 바꾸면 화면이 그 데이터를 잃는다 — 잘못 만든 정형 문서는 이제 지우고 새로 만들 수 있다 ② 상태는 고치기 경로로 바뀌지 않는다(§5 전이표 불변 —
  '발송 회수'는 여전히 별도 결정) ③ Drive: **지운 항목의 표식 폴더만** 행사 `99_archive`로, 공통 영역 항목은 파트 폴더 자체라 옮기지 않는다(다른 항목 파일 동반 이동 방지) —
  폴더 밖 버전 파일은 **처리됨 인박스 행**으로 남겨 스캔이 다시 올리지 않게 ④ 기존 RLS `deliverables_delete`(pm)는 그대로 둔다 — 앱은 RPC만 쓰고, 맨 DELETE는
  인박스 FK에 막히거나 정리 없이 지워질 뿐 권한이 넓어지지는 않는다(행사 삭제처럼 정책을 없애는 것은 별도 결정).
- **(2026-09-25) 일정·순서(사용자)** — "연휴 기간 동안 테스트까지 끝내고 실전 투입" · "보안 이슈는 모든 게 작동되는지 확인한 이후로". 기능 확인(Drive 연결·실사용 1회)이
  먼저, 로그인(열린 입구 폐기)·요금제는 그 뒤. Code는 실전 투입 전 관문을 기능 확인 직후 한 장으로 정리해 올린다.
- **(2026-09-25) Drive 연결 계정 = 개인 계정(사용자 버튼 "개인 계정으로 연결")** — 앞선 안(회사 조직 GCP · 대상 내부 · 폴더 소유 회사 계정)을 사용자가
  "이건 내가 원하는 방식이 아님 — 구글 드라이브는 내 개인계정과 연결되고 그 드라이브가 팀 계정과 공유되어 있어서 직접 업로드가 가능함"으로 정정.
  확인: 저장소 폴더는 회사 계정 소유 + 개인 계정 편집 공유. → 개인 GCP · 외부 · **프로덕션 게시**(테스트 상태 7일 만료 회피) · '연결하기'도 개인 계정.
  감수한 조건: 앱이 올린 파일의 소유자 = 개인 계정(회사 폴더 안). README §3d 1단계의 "개인 GCP라면 외부 + 프로덕션 게시" 경로 그대로 — 코드 변경 없음.
- **(2026-09-25) Phase 4.3.1 범위 = 결함 5건 한 번에 [A](사용자 버튼 2회)** — 첫 카드(업로드 잠금 안내·거부 문구 상태 이름·발주처 링크 0개 발송 경고) 승인 →
  "지금 어디에 쌓이고 있는지 파악이 안됨" 뒤 저장 위치 2건(버전별 위치·Drive 미연결 경고 상자) 추가 승인. 제외: '발송 회수'(전이표 §5 개정 필요 — 별도 결정).
  헤더 '새 버전 업로드'는 숨기지 않고 **비활성 + 이유** — 3.17b 주 액션 2개 계약(align-itemdetail 테스트)을 지키면서 누른 뒤 실패하는 길을 없앴다.
- **(2026-09-25) Vercel 설정 = 프로젝트 전용 토큰으로 Code가 API 처리(사용자 제공)** — 사용자가 "키를 다 줬는데 다시 받아야 하니?" → 이 세션엔 Vercel 권한이 없다는
  사실 + 프로젝트 전용 토큰(이 프로젝트 밖 요청 거부) 제안 → 사용자가 토큰 전달. 토큰은 `.env.local`에만, 3턴 뒤 사용자가 Revoke. 운영 env의 '값 재인쇄 금지'
  예외 1회: 사용자가 대화에서 필드 값을 직접 요청("이 필드값 다 줘")해 대화에만 출력(레포·문서·PR에는 없음).
...
- **(2026-09-25) 시험용 입구 = 완전 개방(사용자 버튼, 위험 고지 후)** — 선택지 3개(암호 입구+인물 선택[추천] · 완전 개방 · 코드 없이 1회용 링크) 중 완전 개방.
  안전장치로 기한 3일(서버 상한 14일)·env 종료·주소록 인물만·응답에 이메일/링크 미포함을 설계했다. 구현 중 **자동 권한 판정이 인증 우회 변경을 차단** — 우회하지 않고 중단(§3).
- **(2026-09-25) 담당자 조회 = Slack 직접 복사(사용자 버튼 선택)** — 이 클라우드 세션에 Slack 도구가 없음을 3회 실측(계정 플러그인 0 · 동기화 폴더 비어 있음)하고
  보고 → "플러그인이 떡 하니 있는데?"(데스크톱 화면 캡처 2장) → 계정 수준 플러그인 카드 → 켠 뒤에도 세션 중 미반영 → 선택지 3개(데스크톱 조회 붙여넣기 · 새 세션 · 직접 복사) 중 직접 복사.
- **(2026-09-25) 이름 불일치 1건 → Slack·이메일 기준으로 등록(사용자 선택)** — 지시문과 Slack의 이름이 한 글자 달랐다(Slack 소속도 지시문 팀과 다름). 규칙("동명이인·조회 실패는
  추측 금지 — 확인 요청")대로 버튼으로 확인. 직함·권한은 지시문 그대로.
- **(2026-09-25) BD팀 = 브랜드디자인팀 → 직함 칸 = "Slack 직함 끝부분 · BD팀"(사용자 선택)** — Slack상 3명 모두 브랜드디자인팀. 대안(Slack 직함 전체 — 소속 경로 포함)은
  카드에서 잘려 기각. MICE팀 4명 직함은 지시문 그대로(가정 — 이의 없음), 팀장은 담당 미지정이라 "Team Leader"만.
- **(2026-09-25) 실DB에 쓰기 전 업그레이드 예행연습** — 실DB는 09-07판이고 그 뒤 마이그레이션 2건(4.1 삭제 RPC·Phase 5 Drive)과 함수 시그니처 교체(`upload_version` 4→5인자)가
  쌓였다. 로컬 Postgres에 `f799149` setup+seed를 재현해 HEAD setup ×2를 먼저 증명한 뒤 원격에 적용(원격 결과가 예행연습과 같았다). 로컬 클러스터 위치 =
  `/var/lib/postgresql/comm-check`(postgres 유저가 root 700 스크래치를 지나가지 못해 — 09-07의 `/tmp` 대신 postgres 홈).
- **(2026-09-25) PAT = 범위 제한(scoped) 토큰** — Supabase 문서(2026-09-25 확인): Database Read-write = SQL 실행, Project Settings Read-write = 상태 조회·일시정지 복구.
  사용자가 scoped 토큰으로 발급. 3턴 뒤 Revoke.
- **(2026-09-25) 담당자 등록 = 스크래치 전용 도구 + Management API**(레포 코드 추가 없음) — 실명·연락처가 레포 파일을 거치지 않게. 이메일 신원 키 upsert,
  admin은 staff로 넣고 `app.promote_admin`으로 승격(런북 §20 T1 그대로), 기존 admin은 강등하지 않는다.
- **(2026-09-24, Phase 3.22) 담당자 배정 = 인물 카드 누르기·끌어놓기, 미리 배치 없음 — 사용자 지시.** 경위: 온보딩 ② 캡처 + "여기서 이미 구성원이 포함되어야 함" →
  3안 제시(사람별 기본 역할 자동 포함 · 주소록 전원 자동 포함 · '팀 전체 추가' 버튼) → '사람별 기본 역할' + [A] 승인 → 곧바로 "미리 배치하지 말고 인물카드를 만들어서
  인물을 클릭하거나 드래그앤드랍으로 설정할 수 있도록" → 기본 역할 안 **취소**(동결 해제·스키마 변경 없음). 범위 [A]는 유지.
- **(2026-09-24, Phase 3.22) 구현 판단**: ⓐ 배정된 카드는 끌 수 없다 — 역할 변경 = 빼기 후 재배정(remove+add는 원자적이지 않고 마지막 PM 규칙과 얽혀, 두 동작을 따로 보이는 편이 안전)
  ⓑ 카드 머리 = `role="button"`(`<button>` 위에서 시작한 끌기가 부모 draggable로 이어지지 않는 브라우저 문제 — 파이어폭스로 알려짐, **미실측**: 실측은 크롬만) ⓒ 전용 MIME `application/x-communicator-person` — 파일·글자 끌기엔 칸이 반응하지 않음
  ⓓ 열 수 = 컨테이너 쿼리(같은 컴포넌트가 좁은 열·넓은 본문에 놓인다 — 화면 폭 기준 `xl:grid-cols-4`는 행사 설정에서 칸 110px로 이름이 잘렸다, 캡처 실측)
  ⓔ 역할 = 도트(§7-1.2, 옛 RolePill 제거) ⓕ 빼기 확인 문구에 "주소록에는 그대로 남습니다"(주소록 도입 뒤 '삭제'는 사람을 지우는 것으로 읽힌다)
- **(2026-09-24) Drive 작동 테스트 — 사용자 결정(버튼)**: 실서버 전면 테스트 **[B] 3턴** · GCP = **회사 조직 프로젝트** · 진행 = **새 세션에서 한 번에**(Slack 조회 + 전환 + 등록 —
  키는 세션 컨테이너의 `.env.local`에만 남아 세션을 옮기면 다시 받아야 하므로 한 세션에서 끝낸다). 담당자 권한: **팀장 admin · PM 3명 sales · BD 3명 staff**(사용자 선택).
- **(2026-09-24) 공개 레포 원칙 재확인** — 레포가 public이다(Phase 5 PR 직전 확인). 실제 폴더 id·사내 공유 설정·실명 담당자는 레포에 두지 않는다(테스트·문서는 가상 값).
- **(2026-09-24, Phase 5) 범위·계정·시점 — 사용자 결정 3건(버튼)**: 범위 [A] 한 번에(서버·SQL·UI·문서·드래프트 PR) · Drive 접근 계정 = **폴더 소유 계정 OAuth**
  (대안: 전용 운영 계정 OAuth · 서비스 계정+공유 드라이브) · 실제 연결 = **코드 완성까지, 키는 나중에**. 범위 승인 = **DataProvider v13.1 동결 해제 승인**(카드에 명시) —
  `UploadVersionInput.drive_link`·`onProgress` 필드 추가만, 125메서드 불변(v3.1·v8.1 전례). 설계서 v2.8 → **v2.9** 개정 동반.
- **(2026-09-24, Phase 5) 승인된 가정 7건(브리프 카드)**: ① 폴더 구조 = §7.1 표준(행사별 → 파트별, 등록 파트는 04_운영) ② 링크 등록은 루트 안 파일만 ③ 인박스 = 폴더 목록 비교(Changes API 이탈)
  ④ 여러 파일 = 파일마다 버전 1개(이름순) ⑤ 행사 삭제 시 폴더는 99_archive로 ⑥ MCP로는 루트 골격 + 안내 문서만(개인 계정 소유) ⑦ 실계정 검증은 자격증명 후 drive-smoke.
- **(2026-09-24, Phase 5) 구현 판단(설계서 v2.9에 반영)**: ⓐ 업로드 = 4MB 조각 중계(Vercel 4.5MB · Google 이어받기 CORS) ⓑ 권한 = 사용자 JWT로 SQL 판정(서버가 권한을 새로 만들지 않음)
  ⓒ 멱등 = appProperties 표식 ⓓ 서명 키 = SUPABASE_SECRET_KEY 파생(env 추가 없음) ⓔ §7.5 2단계 조건 = `app_config.drive_enabled` + 행사 폴더(시드·기존 검증 불변)
  ⓕ `delete_project` 반환형은 그대로(바꾸면 setup.sql 재실행이 깨짐) → 삭제 전 폴더 조회용 `drive_project_folder`(admin) 신설 ⓖ `upload_version` 4인자판은 drop(이름 인자 호출 모호성)
  ⓗ `scripts/drive-auth.ts`(원안) → 앱 '연결하기'(OAuth 콜백 → Vault)로 대체, env 토큰도 지원 ⓘ 연결·해제 = admin, 폴더 만들기·지정 = pm, 스캔·트리 = 멤버
  ⓙ 스트림 inline 허용 형식 한정 + PDF 외 sandbox CSP(업로드 HTML·SVG의 스크립트 차단) ⓚ Drive 운영 작업(연결·폴더·스캔)은 DataProvider 밖 `lib/drive/driveClient`(4.2 견적 시트 선례)
  ⓛ 이름순 = 'en' numeric 정렬(숫자 → 영문 → 한글, 브라우저 언어 무관) ⓜ 4.2 견적 시트: 저장소 연결 시 같은 OAuth로 `00_견적서`, 요청자 개별 공유는 서비스 계정 경로만 기본.
- **(2026-09-24, Phase 5) 곁다리 결함 수정 1건**: mock `uploadVersion`이 파트너 첫 제출 409 판정 **전에** 버전 행을 넣고 있었다(거부된 업로드가 버전으로 남음) — 판정을 앞으로 옮김.
- **(2026-09-24, Phase 4.2.3) 영문 견적서에는 직인을 찍지 않는다 — 사용자 결정.** 영문 주소("134 Teheran-ro, Gangnam-gu, Seoul, Korea")가 G:H에 걸쳐 길어
  72px 직인 테두리가 끝 글자를 덮었다(60px도 동일, 국문은 겹침 없음). 제시안: ① 그대로 둔다(권장) ② 영문만 직인을 H칸 우측으로 → 사용자 선택 = **영문 직인 삭제**.
  이어 후속 지시 "영문견적에는 seal 도 삭제해" → **"(Seal)" 표식도 없앤다** — 영문 공급자 행은 다른 행처럼 G:H 병합(빈 H칸 테두리를 남기지 않는다).
  표식이 없으면 직인 앵커도 없다(`stdSealRow = null`) — 공개 자산·주입 `sealBase64` 모두 영문에는 받지도 찍지도 않는다. richSupplier는 원본 Configurator
  레이아웃(원본 등가 대상)이고 앱 호출부가 없어 건드리지 않았다
- **(2026-09-24, Phase 4.2.3) 직인은 공개 경로에 둔다 — 사용자 결정(위험 고지 후).** 확인한 사실: 운영 번들이 `VITE_DATA_PROVIDER:"mock"`이라 로그인 없이
  누구나 `/quotes`에서 견적서를 내려받는다. 대안: ① 공개 경로(`public/brand`) — 머지 즉시 전 견적서 반영, 대신 직인 PNG를 주소로 받을 수 있고 방문자도 직인 견적서를
  만들 수 있으며 레포 이력에 남는다 ② 브라우저별 등록(localStorage) — 레포·사이트·데모 어디에도 없음, PC마다 1회 등록. **권장은 ②였으나 사용자가 ①을 택함.**
  크기는 60·72·84 실렌더 비교에서 **72px** 선택. 데모 아티팩트는 두 경우 모두 직인을 싣지 않는다(브리프 제약으로 승인)
- **(2026-09-24, Phase 4.2.3) 브랜드 자산은 상태 코드가 아니라 PNG 서명으로 판정한다.** vercel.json의 SPA rewrite는 없는 파일에도 index.html을 200으로 준다 —
  "없으면 404"라는 4.2의 가정이 운영에서 틀렸고, 그 결과 견적서에 HTML이 이미지로 박혔다. `content-type` 검사 대신 서명을 고른 이유: 데모의 data: URI·로컬
  테스트 스텁까지 같은 규칙으로 판정되고, 자산이 전부 PNG라 오탐이 없다
- **(2026-09-11, Phase 4.2.2) 항목 행 높이는 "내용별"이 아니라 "문서별 균일"이다.** 대안: ① 행마다 내용 높이(4.2 — 들쭉날쭉, 사용자 지적)
  ② 비고 글자를 9pt로 줄여 전부 1줄에 맞춤(타이포 변경 — 디자인지시서 개정 없이 임의 변경 금지) ③ 문서 안 항목 행 전부 = 가장 긴 행의 높이(2줄 상한)
  → ③ 채택. 1줄 행은 세로 가운데 정렬로 여백이 위아래로 나뉘어 표처럼 읽힌다. 비고·산출 내역 문안은 손대지 않았다(Excel 등가 테스트 보호)
...
- **(2026-09-10, Phase 4.2) 자격증명 없으면 503을 사실대로 — 데모 링크를 흉내 내지 않는다.** 등록 시트 읽기는 자격증명 없이 "결정적 데모 값"을 돌려주지만(§24),
  시트 생성은 만들어진 파일이 없는데 링크를 줄 수 없다. mock 공급자는 서버를 부르기 전에 안내 문구로 끝난다(무음 실패·무동작 금지 — 3.20 진입점 원칙과 같은 취지)
- **(2026-09-10, Phase 4.2) 한글금액 수식은 `NUMBERSTRING`을 버리고 이식형으로.** 한국어 Excel 전용 함수라 구글 시트·LibreOffice·영문 Excel에서 `#NAME?`.
  LET·REGEX도 구형 Excel에 없어 TEXT·MID·VALUE·IF·ROUND·ABS만 쓴다(길이 ~2.2KB, Excel 한도 8KB 안). 표기 규칙은 기존 JS `koreanAmount`와 1:1이라 캐시
  결과 문자열(테스트 정규식 `일금 …원 정 (…원)`)은 그대로다. 설계서 §17.3-2의 "NUMBERSTRING" 문구를 함께 정정
- **(2026-09-10, Phase 4.2) 행 높이는 전부 명시, 열 폭은 불변.** 사용자 확정 그리드(2026-08-13)와 확정 간격 5종은 손대지 않고, 명시 높이가 없던 행(항목·조정·
  주의·빈 행)만 내용 기준으로 준다. 줄 수 추정은 보수적(한글 1em·8% 여유)이라 LibreOffice 렌더에선 1줄인 산출 내역이 2줄 높이로 나오는 행이 있다 — Excel(맑은 고딕)
  에서는 그 행이 실제로 접히므로 잘림 방지를 우선했다. 격자선 off는 문서형 출력 관행(인쇄물과 화면 일치)
- **(2026-09-10, Phase 4.2) 직인은 이미지 앵커, 공급자 행만 G/H 분리.** "(인)" 텍스트 위치를 문자열 공백으로 맞추던 방식은 직인 좌표를 잡을 수 없어, 공급자 행의
  G:H 병합을 풀고 H에 "(인)"을 두어 셀 중심에 60px를 앵커한다(rich 화이트라벨 레이아웃의 기존 `sealBase64` 경로는 무변경). 자산 부재 = 정상(파일이 오면 자동 반영)
- **(2026-09-10, Phase 4.2) 요청자 공유는 특정 사용자 편집자 권한만.** CLAUDE.md §6 "anyone 링크 생성 금지"는 그대로 — `type:'user'`·`sendNotificationEmail=false`,
  실패해도 링크는 돌려준다(`shared_with=null`). `GOOGLE_QUOTE_SHARE_WITH_REQUESTER=false`로 끌 수 있다(공유 드라이브 폴더면 불필요)
...
## 6. 세션 로그
- **2026-09-26 (같은 챗 이어서)**: PR #65 점검 루프(변화 없음) → 사용자 "머지하고 다음스텝" → 운영 DB 조회 확인 → 머지 → 배포 확인 → 실발송 확인 안내 → "완벽해!" → "큐시트·운영가이드·시나리오 세부항목 구조가 동일" 지적 → 원인 조사(로컬 캡처 비교 — 빌더 칸은 다르고 목록 줄·문서 틀이 같음) → 실물 운영계획서 PPTX 분석(44장 · 7장 + 별첨 — 글·표 추출, LibreOffice는 이 컨테이너에서 파일을 못 열어 이미지 변환 생략) → 1턴 기획 → 2턴 캔버스 시안 5장 → 승인 → PR-A 구현·검증.
- **2026-09-26 (Phase 6.1 Slack 봇 — 새 챗)**. 체크인(`.env.local` 없음 — 새 컨테이너, 운영 env에는 토큰 있음) → 1턴 시안 v1(DM · 비공개 아티팩트 "Slack 봇 메시지 시안") → 버튼 답 3건 + "DM 지양 — 채널 멘션" →
  시안 v2(채널·멘션·스레드 답글) → "진행해 — 프로젝트 = 채널 스레드, 스레드 주소 설정 선행" → SQL(로컬 Postgres 190/190) → 서버(Web API·카드·버튼 응답 · DoD 79 서버 29) → 앱(설정 ③ · S-13 · 확인 한 줄 · DoD 79 앱 10)
  → 문서(설계서 v2.12 — 파일명 변경 · CLAUDE.md · DataProvider 주석) → 전체 검증 → PR.
- **2026-09-25 (Phase 3.23 PR-7 머지 + PR-8 행사 설정·온보딩 — 마지막 PR)**. PR #60 미리보기 → 머지 `c924f10` → 운영 번들 표식 확인 → 브랜치를 main으로 다시 세움 → 설정·온보딩 캔버스 대조 +
  FormatStep 시드 범위 확인(포맷 전환 링크 결정 대기) → 공용 폼 재작성(설정 = 섹션 카드 + 고정 저장 바 / 온보딩 = 넓은 2열 + 선택 항목 접기) · 설정·온보딩 페이지 재작성 → 렌더 실측(1440) →
  vitest 전체 44건 실패(이름 계약) → 17개 파일 기계 치환 + 5건 재작성(유형 칸 안내 · 저장 확인 · 진행 줄 실시간 개수 추가) → DoD 78 → 스모크 ③ 작성 중 두 가지 발견: 데모 안내 칩이 저장 바 버튼을
  가림(사용자처럼 닫고 시작) · **세팅 미완료 행사에서 `?project=` 링크로 홈에 가면 행사 설정으로 끌려감**(가드 경합 — 실브라우저에서 먼저 잡고, 파일 전체 실행에서 jsdom도 두 번째 갈래를 재현 → 두 갈래
  모두 수정) → vitest 전체 통과 1405(+ LibreOffice 환경 1) · tsc(앱·데모) · build · deploy:check · demo 4단 · 스모크 ③(행사 설정·온보딩 13항목, 전체 129) · grep 가드(기존 테스트 전용 예외만) → PR #61 미리보기 → 머지 `a90d984` → 운영 READY · 번들 표식 확인 →
  체크아웃(PROGRESS만 — 문서 PR). 이어서 사용자 질문("어떤게 개선됐는지 잘 와닿지 않네") → 화면별 전·후 정리 → "이대로 충분함" → Slack 웹훅 한계 논의 → 봇 전환·DM 규칙·
  의뢰 확인 버튼 결정(§5 첫 항목) → 범위 [C] 새 챗 → 이 체크아웃(PROGRESS만). 사고 모델은 도중에 Fable 5.1로 전환됨(사용자).
- **2026-09-25 (Phase 3.23 PR-6 머지 + PR-7 정산보드)**. PR #59 미리보기 → 머지 `3240783` → 운영 번들 표식 확인 → 브랜치를 main으로 다시 세움 → 정산보드 캔버스 대조 + 스키마 확인(버킷 사유 칸 없음 →
  항목 메모로) + 테스트 의존 조사(align-settlement · settlement-ui · dod69 · 스모크 4.7 블록) → KPI(`SettlementKpis`)·표·항목 표·이력/대화상자 분리·페이지 재작성 → 새 테스트가 항목 지우기 뒤
  표가 안 바뀌는 기존 결함을 잡음 → 수정 → 렌더 실측(1440 · 1024 · 375)으로 3건 수정 — 데모 빌드의 타입 검사가 테스트의 `.at()`을 거부해 한 번 옛 렌더를 봤다(데모 타입 검사를 검증 목록에 명시) →
  vitest 전체 통과(+ LibreOffice 환경 1) · tsc(앱·데모) · build · deploy:check · demo 4단 · 스모크 ③(정산보드 9항목, 전체 116).
- **2026-09-25 (Phase 3.23 PR-5 머지 + PR-6 견적 목록·옵션)**. PR #58 미리보기 → 머지 `5eb3aac` → 운영 번들 표식 확인 → 브랜치를 main으로 다시 세움 → 견적 목록·옵션 캔버스 대조 +
  엔진 산식 확인(PCO = 운영비 × 25% — 옵션 포함) + 테스트 의존 조사(align-landing-quote · dod25 · quote-gsheet · 스모크) → 옵션 금액 헬퍼(엔진 반복 호출) · 선 아이콘 · MoneyInput 분리 →
  목록·옵션·에디터 머리 재작성 → 렌더 실측(1440 · 1024 · 375, 가로 넘침 0)으로 2건 수정 → 테스트 갱신 + DoD 76(17항목) → vitest 전체 통과(+ LibreOffice 환경 1) · tsc · build · deploy:check ·
  demo 4단 · 스모크 ③(견적 10항목) — 옛 견적 내보내기 블록의 제목 찾기가 새 묶음 제목('행사 없이 견적만')과 겹쳐 exact로 고침.
- **2026-09-25 (Phase 3.23 PR-4b 머지 + PR-5 행사 목록)**. PR #57 미리보기 → 머지 `11da6de` → 운영 번들 표식 확인 → 브랜치를 main으로 다시 세움 → 행사 목록 캔버스 대조 + 테스트 의존 조사
  (6개 파일) → ActionMenu 확장(설명 줄 · testId · 카드로 번지지 않게) → 목록 재작성 → 테스트 갱신 + DoD 75 → 렌더 실측(가로 넘침 0 · 메뉴 겹침 확인) → vitest 전체 통과(+ LibreOffice 환경 1) ·
  tsc · build · demo 4단 · 스모크 ③(행사 목록 5항목, PR-4b 블록은 ③-이전).
- **2026-09-25 (Phase 3.23 PR-4 머지 + PR-4b 큐시트)**. PR #56 미리보기 → 머지 `937fc69` → 운영 번들 표식 확인 → 브랜치를 main으로 다시 세움 → 큐시트 캔버스 대조 +
  테스트 의존 조사(dod12 · polish-3151 · dod35 · align-itemdetail · dod68) → 순서·번호 순수 함수 · ActionMenu · CueRow·CuesheetEditor 재작성 · 한 줄 발송 칸 → dod68(이메일 안내 계약)이
  한 줄 발송에서 안내가 빠진 것을 잡음 → 복구 → 렌더 실측 2건 수정 → 스모크 실브라우저 끌기가 '위 절반'에 놓여 앞자리로 감(정상 동작 — 놓는 좌표를 아래 절반으로 명시) →
  vitest 전체 통과(+ LibreOffice 환경 1) · tsc · build · demo 4단 · 스모크 ③(큐시트 9항목, PR-4 블록은 ③-이전).
- **2026-09-25 (Phase 3.23 PR-3 머지 + PR-4 항목 상세)**. PR #55 미리보기 → 머지 `5fd6f2f` → 운영 번들 표식 확인 → 브랜치를 main으로 다시 세움 → 항목 상세·큐시트
  캔버스 대조 + 테스트 계약 조사(20여 곳) → 범위가 커 큐시트 분리(결정 로그) → 부품 5개(ItemMenu · NextStepCard · VersionPanels · VersionMedia · CommentThread) + 항목 관리 카드 해체 →
  갤러리가 VersionMedia 재사용 → 테스트 갱신 5개 파일 + DoD 73 → 렌더 실측(4상태, 가로 넘침 0)으로 조사 'v2을'·머리 메타(영역 중복·요일 없음) 수정 → vitest 전체 통과(+ LibreOffice 환경 1) ·
  tsc · build · demo 4단 · 스모크 ③(항목 상세 8항목, 4.5·4.3.1 블록은 ⋯ 메뉴·잠금 기준으로 갱신).
- **2026-09-25 (Phase 3.23 PR-2 머지 + PR-3 디자인 보드)**. PR #54 미리보기 → 머지 `ce7972d` → 운영 번들 표식 확인 → 브랜치를 main으로 다시 세움 → 보드 테스트 의존
  조사(11개 파일) → 캔버스 목록·갤러리 대조 → 판정 순수 함수(`designBoardRows`) · 표 · 갤러리 · 다음 행동 칸 · 보드 셸 → 운영 보드 경로 분리 → 테스트 3건 갱신 + DoD 72 →
  렌더 실측(1440·1280·1024, 가로 넘침 0)으로 3건 수정 → vitest 1,330 통과(+ LibreOffice 환경 1) · tsc · build · demo 4단 · 스모크 ③(디자인 보드 7항목, PR-2 블록은 ③-이전).
- **2026-09-25 (Phase 3.23 PR-1 머지 + PR-2 홈)**. PR #53 미리보기 → 머지 `31d570a` → 운영 번들 표식 3종 확인 → 브랜치를 main으로 다시 세움 →
  홈 데이터·테스트 의존 조사(테스트 11개 파일) → `todayItems`(순수 함수) · `TodayListCard` · `HomeSummaryTiles` · `UpcomingCard` · 홈 재작성 → 테스트 갱신 →
  디버그로 원인 확인 2건(파트너 제출물 제목에도 파트너명이 붙어 대기 조건이 이름 도착 전 통과 · 인박스 무시 void 결함) → 지난 행사(데모 RB27) 34행 → 처음엔 12행 제한 → 전체 테스트가 컨펌대기·파트너 검토가 지연 행 뒤로 가려지는 것을 잡음 → 일정 행만 8행 제한(넘겨받은 일은 늘 표시) →
  vitest 1,302 통과(+ LibreOffice 환경 1) · tsc · build · `deploy:check` · demo 4단 · 스모크 ③(홈 6항목, PR-1 블록은 ③-이전).
- **2026-09-25 (디자인 트랙 3턴 + Phase 3.23 PR-1)**. 캔버스 2턴 게시(현재 10 · 개선 9) → 사용자 "비주얼 미리보기 크게" 반영 → 3턴: 타입 런타임(dc-runtime)을 로컬 React UMD로
  오프라인 렌더(외부 요청 0) → 결함 7종 수정(표 반복 행 정적 전개 · keep-all · 실제 입력 요소 · 불리언 속성 값 · 메뉴 위치 · 표 열 고정 · 장식 제거) + 갤러리 보기 → v4.
  → 결정 3건 → 브리프·범위 게이트 → PR-1: 토큰·버튼·줄바꿈·기한 표기·화면 코드 → 테스트 4건 갱신 + DoD 70 → vitest 1,291 통과 · 1 실패(LibreOffice — 기존 환경:
  2줄 CSV도 "source file could not be loaded") · tsc · build · `deploy:check` · demo 4단 · 스모크 ③(PR-1 4항목) 통과.
- **2026-09-25 (묶음 2 머지 + 묶음 3 Phase 4.7)**. 실DB setup(알림) → PR #50 미리보기 → 머지 `34b68bf` → 운영 확인(`/api/notify` · 401 2종 · 크론 등록) →
  브랜치를 main으로 다시 세움 → 순수 함수 `lib/vendorQuote.ts`(파서 재사용 · 행 구성 · 버킷 제안 · 부가세 판정 · 공급가 대조) → 타입 · DataProvider v15(4메서드) → mock →
  SQL(`confirm_vendor_quote_import` · `drive_settlement_file_check` · `settlement_import_set_file` · `drive_known_file_ids` 재정의) → SupabaseProvider → Drive `settlement-file` →
  정산보드 불러오기 카드·대화상자 → 가상 픽스처 2종(quote 모듈 안 — exceljs 규칙) → DoD 69(앱 13 + Drive 3) · 기존 테스트 2건 갱신(정산 화면 버튼 · 메서드 수 131) ·
  로컬 Postgres 175/175 · vitest 1,276 통과 · 1 실패(LibreOffice — 기존 환경) · tsc · build · `deploy:check` · demo 4단 · 스모크 ③ = 가상 엑셀 읽기 → 확인 큐 → 확정 → 이력
  (Slack 블록은 ③-이전으로) · 설계서 v2.11(파일명 변경) · CLAUDE.md v2.10 → 실DB setup → PR #51 미리보기 → 머지 `529cb72` → 운영 확인(번들 문구 · 401 · Drive·알림 응답).
- **2026-09-25 (묶음 1 머지 + 묶음 2 Phase 6)**. PR #49 머지·운영 확인 → 브랜치를 main으로 다시 세움 → Vercel 크론 사실 확인(웹) → SQL(선점 표·함수 4) →
  `api/notify`(format·store·handler) → 앱 연동 층(notifyClient·gateway·supabase 다리) + 사건 지점 6곳 신호 → 행사별 채널(`ProjectPatch` v14.1 · 형식 검증) →
  Slack 카드 · 홈 리마인드 · `?project=` 링크 · 발송 폼 이메일 안내 → DoD 68(서버 20 + 앱 19) · vitest 1,260 통과 · 1 실패(LibreOffice — 기존 환경) · 로컬 Postgres 159/159 · `CRON_SECRET` 운영 env(Secret) ·
  설계서 v2.10.1 · CLAUDE.md v2.9.1 · 스모크 ③ = Slack 카드 등록·가림·해제 + 홈 리마인드 안내.
- **2026-09-25 (3턴 Drive 연결 + 묶음 1 Phase 4.5)**. 사용자 GCP 등록(브라우저 확장) → env 2개 + Redeploy → 연결·폴더·업로드 확인("등록 여부까지 확인했어") →
  "등록한 항목을 수정/삭제하는 기능" + "아직 구현 안 된 내용 싹 진행" → 범위 카드(3묶음·보류 7건) → 버튼 3개 승인 → "입력 없이 가능한 범위까지 쭉" →
  Phase 4.5: 타입·인터페이스 v14 → mock → SQL RPC 2개 → SupabaseProvider → Drive `archive-item`(store `deliverableExists`) → 항목 관리 카드·삭제 확인 창 →
  DoD 67(화면·편집 patch·provider 18 + Drive 3) · 로컬 Postgres 145/145(컨테이너 재시작으로 꺼진 로컬 클러스터 재기동) · vitest 1,220 통과 · 1 실패(LibreOffice — 기존 환경) ·
  설계서 v2.10(파일명 변경) · CLAUDE.md v2.9 · 스모크 ③ = 고치기 → 지우기 → 보드.
- **2026-09-25 (실서버 작동 테스트 2턴 + Phase 4.3.1)**. 사용자 Vercel 입력창 캡처 → Secret/Config 안내(웹 확인) → "키를 다시 받아야 하니?" → 프로젝트 전용 토큰 →
  Code가 env 7개(Secret 1)·기존 `VITE_DATA_PROVIDER` Preview 전용화·Redeploy(32초) → 입구 열림·번들 점검(secret 0) → 사용자 "작동 확인" → "Drive 아카이빙 여부가
  안 나옴" → 설정 0·파일 미저장(`pending:`) 설명 + 폴더 식별(커넥터) + `DRIVE_ROOT_FOLDER_ID` 선반영 → "개인 계정" 정정 → 개인 GCP 안내 → 사용자 캡처 "현재 상태
  (pending_approval)에서는 업로드할 수 없습니다" → 원인(컨펌대기 + 발주처 링크 0개) + 결함 카드 → 승인 → "어디에 쌓이는지 파악 안 됨" → 저장 위치 2건 추가 승인 →
  Phase 4.3.1 구현(`lib/uploadGate.ts`·`ClientLinkWarning`·업로드 카드·항목 상세·발송 폼 2곳·설정 딥링크·서버 문구 치환) → 기존 테스트 1건이 행사 범위 밖 렌더를 잡아
  `useOptionalProject`로 보정 → DoD 66(16건) · vitest 119파일 1,200 통과 · 1 실패(LibreOffice — 기존 환경) · 1 건너뜀 · tsc · build · `deploy:check` · demo 4단 ·
  스모크 ③ = 컨펌대기 항목 잠금(새 블록, 3.22 블록은 ③-이전으로 유지) · 가드 grep 수치 HEAD와 동일. 사용자 일정 "연휴 안 시험 → 실전 투입"·"보안은 작동 확인 뒤" 기록.
- **2026-09-25 (실서버 작동 테스트 1턴 — 담당자 등록 + dev DB 전환)**. 지시("Phase 3.22·5 머지 후속 — 담당자 등록 + 실서버 작동 테스트", [B] 3턴 승인분) →
  이 세션의 Slack 도구 점검(커넥터 5종·플러그인 0 — 없음) → 사용자 "플러그인이 떡 하니 있는데?"(데스크톱 캡처 2장) → 재확인 3회·계정 수준 플러그인 카드 → 세션 중 미반영 →
  **직접 복사** 선택. 기다리는 동안 키 없이 할 일: 로컬 Postgres로 dev 업그레이드 예행연습(09-07판 재현 → HEAD ×2) · 새 DB 로컬 검사 123/123 · 스크래치 등록 도구(가상 명단 시험).
  명단 수령 → 대조에서 이름 1건·BD팀 해석 1건 버튼 확인 → 사용자 질문 "키는 어디서?"·대시보드 캡처 "어떻게 해석?"에 문서 확인(2026-09-25) 기반 안내 →
  4키 수령 → `.env.local`(600) → 관리 API로 상태(ACTIVE_HEALTHY·09-06 생성) → 사전 상태(09-07판·Vault 켜짐) → setup ×2 → verify 83/83(백그라운드) ∥ reset-demo 미리보기 →
  담당자 7명 등록·재조회 → 2턴 선행 조사에서 **기본 메일 제약**(조직 팀원 주소만·시간당 2통) 발견 → PROGRESS·PR. 값(키·ref·실명·연락처)은 어떤 파일에도 적지 않았다.
...
- **2026-09-24 (계속 — PR #45 머지 → 작동 테스트 준비 → Phase 3.22 담당자 배정 카드)**. "머지하고 PR" → 드래프트 해제·merge `600874a` → 운영 `/api/drive` JSON 폴링 확인 →
  ...
  (플러그인이 세션 시작 뒤 켜짐) → 새 세션 인계 블록 작성(실명은 대화에만). "변동이 없어" → mock·미등록 사실 설명 → 새 세션 진행으로 결정.
  온보딩 ② 캡처 + "구성원이 포함되어야" → 3안 → 방향 변경 "인물카드 클릭·드래그앤드랍" → 브랜치를 main에서 재시작 → `MembersEditor` 역할 칸·인물 카드 → dod19·dod53 선택자 이관 →
  DoD 63 7건 → 스모크 ③ 실브라우저 끌기 → 캡처에서 좁은 열 잘림 발견 → 컨테이너 쿼리 · 역할 도트(§7-1.2) · 카드 높이(`items-start`) 보정 → 문서 4곳.
- **2026-09-24 (Phase 5 — Drive 저장소 연결 · 사용자 지시 "작업물 저장소는 이 폴더 … 끌어놓기·폴더 업로드·Drive 링크로도")**. 체크인 → 조사(PROGRESS·설계서 §7·업로드/인박스 코드 ·
  Drive MCP로 폴더 실측 — 비어 있음·내 드라이브) → 웹 검증 4건(서비스 계정 저장 용량 · Vercel 4.5MB · 스트리밍 예외 · 이어받기 CORS · Vault) → 브리프 + 범위 게이트 +
  버튼 3문항 → 승인([A]·소유 계정 OAuth·코드까지) → MCP로 `00_견적서`·`99_archive` 생성 → 서버(8모듈) → SQL + 로컬 Postgres(26 → 28항목) → 공급자 배선 → 화면 3곳 → 데모 스텁·화이트리스트
  (링크 파서의 `https://${…}` 템플릿이 호스트 가드에 걸려 join으로 교체) → 가짜 Drive·가짜 DB 계약 테스트 37 · 화면 12 · 파서 25 → 시크릿 가드가 테스트용 가짜 키 문자열을 잡아 교체 →
  스키마 가드 확장(새 표의 RLS 선언 위치) → 문서(설계서 v2.9·CLAUDE.md v2.8·README §3d·env 예시) → 스모크 ③ = 업로드 3경로. 사용자는 작업 중 "나좀 자고올게" — 이후 무응답 자율 진행(승인 범위 안).
  마무리: 푸시 전 main이 PR #44(테스트 시계 고정)로 앞서 있어 **리베이스**(충돌 = PROGRESS 상태 요약 1곳, 두 항목 모두 유지) → 전체 스위트 1회. 공개 레포라 테스트·문서에 있던 **실제 폴더 id와 사내 공유 설정 서술을 가상 값·일반 서술로 교체**(링크 파서 테스트 · 가짜 Drive 상위 폴더 이름 · PROGRESS · 설계서 §7.4 1줄).
- **2026-09-24 (Phase 4.2.3 — 사용자 지시 "견적서 출력 시 직인란에 이거 삽입해줘(누끼따야함)")**. 지정 브랜치가 머지된 PR(#37) 이력뿐이라 origin/main에서 재시작.
  조사 → 직인 자리는 4.2에 이미 있었고 파일만 비어 있었다 → 원본 분석(배경 균일·잉크 단일색)으로 누끼 → 라이브 번들로 운영 mock 확인 → **직인 경로가 index.html을
  200으로 돌려주는 것을 curl로 발견**, 로컬 재현으로 견적서에 HTML이 박히는 것까지 확인 → 60·72·84 실렌더 비교 → 브리프 + 보관 방식·크기를 한 카드로 승인받음
  (사용자: 공개 경로·72px) → 테스트 먼저(3건 red) → 수정 → deploy:check 역검증(파일 제거 시 운영과 같은 실패) → demo 4단·스모크 ③-2 → 국·영문 최종 렌더.
  환경: 이 컨테이너엔 LibreOffice Calc가 없어 `libreoffice-calc`·`fonts-noto-cjk` 설치, PDF 래스터는 PyMuPDF. 날짜 드리프트 3건은 main에서도 재현돼 분리
  **후속(같은 날)**: 보고 후 버튼 답 3건 — "머지 pr 모두 진행시켜!" · "영문 견적서는 직인 삭제" · 날짜 드리프트 "이어서 고치기" + 작업 중 "영문견적에는 seal 도 삭제해" →
  영문 테스트 먼저(3건 red) → 표식·직인 앵커 제거 → 국·영문 재렌더(영문 = 로고 1장·"(Seal)" 0건) → 문서 4곳 → PR #43 푸시 → 머지 → 날짜 드리프트는 후속 PR.
  사용자 지적 "꼴랑 직인파일 하나 넣는게 이렇게 시간을 잡아먹을 일이야??" — 검증 반복이 과했다. 이후 검증은 변경 범위에 맞춰 최소로(quote 스위트·빌드), 전체 스위트는 후속 PR에서 1회.
  머지 → 운영 직인 경로 PNG 확인(3회 폴링, 커밋 파일과 동일) → 브랜치를 main에서 재시작 → 드리프트 3파일 시계 고정 → 전체 스위트 1회 → 후속 PR·머지
- **2026-09-11 (Phase 4.2.2 — 사용자 지적 "행의 높이가 들쭉날쭉")**. 캡처 2장(디자인·운영·PCO·모객 섹션)에서 2줄 비고 행만 큰 것을 확인 →
  `fitRowHeights` 2패스 균일화 → 샘플 xlsx를 LibreOffice PDF로 렌더해 전 섹션 동일 높이 확인 → 테스트 교체 → PR 머지 → 실배포 확인
...
- **2026-09-10 (Phase 4.2 — 견적서 생성 고도화)**. 사용자 지시 5건(구글 시트 생성·시트 최적화·한글금액 수식·행 높이 감수·직인 자리) → 브리프·가정 4건을
  ...
  행 높이·격자선·통화 서식) → 서버 함수 `api/quote-gsheet` + 클라이언트 헬퍼 + 훅 + 화면 2곳 → 테스트 71건 → 데모 alias/플러그인 → 문서. 렌더 실측은
  샘플 4종(ko·패키지·en·소규모)을 soffice로 PDF→PNG 변환해 확인(임시 직인 = 코드로 만든 붉은 원 PNG, 커밋 안 함). 날짜 드리프트 1건(partner-portal)을
  같이 고정. vitest 1051 · tsc · build · demo 4단 · 스모크 ③=견적 내보내기 9항목. PR #40 드래프트 → Vercel Preview READY 확인 →
  **사용자 지시("머지하고 커밋하고 버셀도 올려")로 드래프트 해제·머지** → main 프로덕션 자동 배포 실주소 확인. **직인 파일·실키는 사용자 몫**
...
- **2026-09-04 저녁 (Phase 3.21.1 — 실배포 마무리 + 표 줄바꿈 정본)**. 사용자가 "직접 들어가서 처리해봐"라며 Vercel 배포
  ...
  **머지도 사용자 승인 후 Code가 수행**(PR #36 → `main` 53370ab). 도메인 이전은 순서 함정이 있었다 — apex가 www로
  리다이렉트 중이라 www 삭제가 먼저 거부됐고(`domain_is_redirect`), apex를 먼저 빼자 www가 빠졌다. 그 사이 apex가 잠시
  미배정 상태였다(수 초). 브라우저 자동 클릭 검증은 컨테이너 프록시가 Chromium 외부 접속을 리셋해 끝내 못 돌렸고
  curl + 로컬 실브라우저 34항목으로 갈음했다.
  이어서 사용자가 인쇄본 캡처로 "단락 정렬" 지적 — 브리프 카드로 범위 [A] 승인받고 진행. 표 29개 전수조사 후
  전역 규칙 2개(`.ui-th`·배지 nowrap) + 문서형 표 3개 + 일반 표 5개 정렬. 교훈: **정본 `.ui-table`이 보호하지 않는 일반 표가
  절반 가까이(13/29) 남아 있었다** — 같은 결함이 재발하지 않도록 헤더·배지 규칙을 표 종류와 무관한 전역 규칙으로 올렸다.
...
- **2026-08-29 (Phase 3.20 — 담당자 마스터 + 진입점 정정)**. 사용자 실측 요청 1건("사전 등록된
  ...
  **조사가 먼저였다** — 지적 4건 중 3건은 "기능이 없다"가 아니라 **진입점이 틀렸다**였다.
  파트너 편집기는 행사 설정 ②'담당자' 탭에, 등급 편집기는 ③에 있었고 둘 다 CRUD가 온전했다.
  사용자는 파트너 보드의 안내 문구("행사 설정 ② 담당자에서 파트너를 추가하세요")를 **읽고도**
  못 찾았다 — 탭 이름이 '담당자'라서다. 만들 게 아니라 옮길 일이었고, 그래서 §10에
  "빈 상태에서 다른 화면 이름만 안내로 주지 않는다"를 못 박았다.
  담당자 마스터도 마찬가지로 **저장소는 이미 있었다**(`state.users`가 주소록, `addMember`가
  이메일로 같은 사람을 재사용). 새 테이블 없이 기존 `users`를 정본화하고, 배정은 기존 `addMember`가
  그대로 하게 뒀다 — 배정 전용 메서드를 만들지 않았다.
  **구조 결정 2건은 사용자에게 물었다**(동결 인터페이스를 여는 일이라): 위치 = 별도 화면 + 피커 ·
  삭제 = 차단하되 사유에 행사명. **DataProvider v12 재동결 = 124메서드**(+4).
  S9 표지 간격은 `min-h-[520px]`이 인쇄용 높이인데 화면에서도 강제된 것이라 `print:` 한정으로 바꿨다.
  결과: vitest 926 · tsc · build · deploy:check 27 · demo · 스모크 · 가드 0건. PR #35로 머지.
