...
## 1. 상태 요약
- **진행: Phase 4.8 — AI(Claude)로 협력사 견적서 PDF·사진 읽기**(2026-09-26, 같은 챗 — 사용자 질문 "여기 시스템에 클로드 API 붙여볼 수 있어?"(예: 견적서·운영계획서 분석) → 버튼 답: 모델 **Claude Sonnet 5** · 키 **개인 Anthropic 계정으로 먼저 시험** · 범위 **로그인한 내부 사용자 + 1인 하루 30회** → "일단 아이디어 차원, 꼭 필요한 게 아니면 안 해도 됨 — 견적 나눠 뿌리고 WBS 짜는 데는 필요할 수도" → 버튼 **"협력사 견적서 PDF·사진만 먼저"**).
  ...
  **남은 것**: PR → 운영 DB 25번째(커넥터) → 머지 → **기획자님: Anthropic 콘솔에서 개인 키 발급 + 월 사용 한도(Spend limit) → Vercel env `ANTHROPIC_API_KEY`(Secret) 직접 입력 → Redeploy**(키는 대화에 붙이지 않는다) → 실물 협력사 견적서 PDF·사진 1~2건으로 시험.
