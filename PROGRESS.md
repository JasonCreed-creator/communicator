...
## 3. 미결
- **(Phase 4.2) 직인 파일 대기** — 사용자가 `remember-seal.png`(정사각 투명 PNG, 300px+)를 주면 `public/brand/`에 넣고 커밋만 하면 된다(코드 무변경).
  넣은 뒤 `npm run demo:build`로 데모 인라인 4건 확인. 직인 크기(60px≈16mm)·위치는 임시 원형으로 실측했으나 실직인 비율에 따라 ±10px 조정 여지.
