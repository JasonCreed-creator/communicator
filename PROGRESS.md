...
## 1. 상태 요약
- **진행 중: 실서버 작동 테스트 [B] 3턴 — 1턴 완료, 2턴(Vercel env·매직링크 로그인) 대기**(2026-09-25, 사용자 지시 "Phase 3.22·5 머지 후속 — 담당자 등록 +
  ...
  **2턴 차단 요인 발견**: Supabase 기본 메일은 **조직 팀원 주소로만 · 시간당 2통**(검증 2026-09-25) — 등록한 회사 주소로 매직링크가 가려면 그 주소를 Supabase 조직 팀원으로
  ...
  남은 것: 2턴 = Vercel env 5종 전환·재배포 → 매직링크 로그인(허용 도메인 결정 포함) / 3턴 = GCP OAuth 클라이언트 → Drive 연결 → `drive:smoke` → 실사용 1회.
...
- **(2026-09-25) `reset-demo --yes` 실행 여부 = 사용자 승인 대기** — 미리보기만 했다. 데모 8행사는 2턴 로그인 확인용으로 남겨 두는 편이 유용할 수 있어 실행 시점도 함께 결정.
...
## 4. 다음 스텝
- **(2026-09-25) 작동 테스트 1턴 이후**: ① `reset-demo --yes` 실행 시점 결정(지금 / 3턴 실사용 직전 / 보류 — 데모 행사는 멤버가 아니면 보이지 않으므로
  2턴 로그인 뒤 화면을 채워 보려면 `grant_demo_access`가 필요) ② **2턴**: 매직링크 메일 경로 결정(조직 팀원 초대 vs 사용자 지정 SMTP) → Vercel env 5종 → 재배포 →
  Supabase Auth URL(Site `https://www.rmb-mice.com` · Redirect `…/login`) → admin 로그인 → 이메일 자동 연결·admin 메뉴(행사 삭제·Drive 연결) 확인 ③ **3턴**(README §3d)
  ④ 끝나면 임시 PAT Revoke(사용자) ⑤ junior 1명 등록 ⑥ Free 플랜 유지/Pro 결정(실사용 전)
