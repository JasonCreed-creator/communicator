...
- **Phase 3.13.3 — 보드 항목 프리셋 (2026-08-23, 사용자 지적)**: 운영 보드에 제작물(디자인) 어휘가
  ...
  - `src/lib/boardPresets.ts` 신설(정본) — **스키마 무변경**. `spec_size|qty|location|type` 4열은
    ...
    design `규격·수량·위치·종류` / ops `규모·투입 인원·장소·구역·운영 구분` / common `분량·부수·보관 위치·문서 종류`
  - 카테고리 프리셋 — 근거는 **픽스처 실측 + 설계서 §15 WBS**:
    design 7종(키비주얼·배너·현수막·백월·리플렛·초청장·명찰) /
    ops 9종(큐시트·시나리오·존운영·운영안·안내문 + WBS 유래 현장답사 1.4·물류/셋팅 5.1·리허설 5.3·결과보고 6.3) /
    common 2종(회의록·계약·정산)
